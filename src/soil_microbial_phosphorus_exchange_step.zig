...
const history = @import("soil_microbial_phosphorus_state.zig");
...
pub fn applyTile(context: *ApplyContext, range: compute.CellRange) !void {
    ...
    for (range.first..range.end) |layer| {
        ...
        for (0..substrates) |substrate| for (0..populations) |population| {
            ...
            const activity = try metabolism.growthTemperatureResponse(context.model_grid.soil_temperature_k[layer], context.parameters.microbial_thermal_adaptation_offset_k) * @exp(context.parameters.heterotrophic_respiration.water_potential_sensitivity_per_mpa * context.matric_plus_osmotic_potential_mpa[layer]);
...
fn validate(context: ApplyContext, range: compute.CellRange) !void {
    ...
    if (range.first > range.end or range.end > layers or context.phosphorus_history.layer_count != layers or context.chemistry_state.cell_count != layers or context.microbial_state.cell_count * context.microbial_state.layer_count != layers or context.result.process_unit_count_per_layer != context.microbial_state.substrate_count * context.microbial_state.population_count or context.phosphorus_history.process_unit_count_per_layer != context.result.process_unit_count_per_layer or context.matric_plus_osmotic_potential_mpa.len != layers) return error.SoilMicrobialPhosphorusExchangeDimensionMismatch;
...
test "runtime microbial phosphate exchange immobilizes deficits and mineralizes surplus" {
    ...
    var context: ApplyContext = .{ .result = &result, .phosphorus_history = &phosphorus_history, .microbial_state = &microbial_state, .chemistry_state = &chemistry_state, .model_grid = &model_grid, .matric_plus_osmotic_potential_mpa = &.{0}, .zone_fractions = .{ .ammonium_non_band = 1, .ammonium_band = 0, .nitrate_non_band = 1, .nitrate_band = 0, .phosphate_non_band = 1, .phosphate_band = 0 }, .parameters = try nitrogen_parameters.parse(source), .timestep_h = 1, .negligible_amount = 1e-12 };
