...
pub const Outputs = struct {
    absorbed_shortwave_mj_m2_h: []f64,
    downward_shortwave_mj_m2_h: []f64,
...
pub const ResetTotals = struct {
    canopy_shortwave_mj_m2_h: f64 = 0,
    canopy_par_umol_m2_s: f64 = 0,
};

/// HOUR1 lines 1023--1032. Resets cell/species canopy radiation
...
pub fn apply(inputs: Inputs, outputs: Outputs) !ResetTotals {
    ...
    @memset(outputs.absorbed_shortwave_mj_m2_h, 0);
    @memset(outputs.downward_shortwave_mj_m2_h, 0);
...
fn validate(inputs: Inputs, outputs: Outputs) !void {
    ...
    if (species_count == 0 or
        ...
        outputs.absorbed_shortwave_mj_m2_h.len != species_count or
        outputs.downward_shortwave_mj_m2_h.len != species_count or
...
test "runtime species radiation accumulators reset and CFX follows source" {
    ...
    const totals = try apply(.{
        ...
        .absorbed_shortwave_mj_m2_h = &absorbed_sw,
        .downward_shortwave_mj_m2_h = &downward_sw,
    ...
    try std.testing.expectEqual(@as(f64, 0), totals.canopy_shortwave_mj_m2_h);
