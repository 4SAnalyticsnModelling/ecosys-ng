...
const priming = @import("organic_priming_exchange.zig");
    ...
    for (range.first..range.end) |layer| {
        ...
        for (0..organic.substrate_count) |substrate| {
            ...
            for (0..populations) |population| {
                ...
                response[substrate * populations + population] = growth_temperature * @exp(water_sensitivity * context.matric_plus_osmotic_potential_mpa[layer]);
            }
...
fn validate(context: ApplyContext, range: compute.CellRange) !void {
    ...
    if (range.first > range.end or range.end > layers or context.organic_state.layer_count != layers or context.microbial_state.cell_count * context.microbial_state.layer_count != layers or context.respiration_fluxes.layer_count != layers or context.soil_temperature_k.len != layers or context.matric_plus_osmotic_potential_mpa.len != layers or context.result.exchange.population_count != context.microbial_state.population_count) return error.SoilOrganicPrimingDimensionMismatch;
...
test "soil priming conserves dissolved activity and runtime microbial elements" {
    ...
    defer state.deinit();
    var context: ApplyContext = .{ .result = &state, .organic_state = &organic_state, .microbial_state = &microbial_state, .respiration_fluxes = &respiration, .soil_temperature_k = &.{298.15}, .matric_plus_osmotic_potential_mpa = &.{0}, .thermal_adaptation_offset_k = 0, .dissolved_priming_rate_per_h = 0.01, .microbial_priming_rate_per_h = 0.001, .timestep_h = 1, .negligible_carbon_g_c = 1e-12 };
