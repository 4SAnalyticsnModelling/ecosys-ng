...
fn validateState(state: State, domain_axes: usize, axis_layers: usize, domain_layers: usize, axes: usize) !void {
    ...
    inline for (.{ state.layer_protein_carbon_g_c, state.layer_total_root_carbon_g_c, state.mobile_carbon_g_c, state.mobile_nitrogen_g_n, state.mobile_phosphorus_g_p, state.total_water_potential_mpa, state.osmotic_water_potential_mpa, state.turgor_water_potential_mpa, state.primary_radius_m }) |values| if (values.len != domain_layers) return error.PrimaryRootPublicationDimensionMismatch;
...
fn applyAxis(state: State, inputs: Inputs, domain: usize, layer: usize, axis: usize) !void {
    ...
    if (crosses) {
        ...
        state.total_water_potential_mpa[target_domain_layer] = state.total_water_potential_mpa[domain_layer];
        state.osmotic_water_potential_mpa[target_domain_layer] = state.osmotic_water_potential_mpa[domain_layer];
        state.turgor_water_potential_mpa[target_domain_layer] = state.turgor_water_potential_mpa[domain_layer];
...
fn makeState(comptime layers: usize, v: *[19][layers]f64, deepest: *[1]usize) State {
    return .{ .axis_total_carbon_g_c = v[0][0..1], .axis_total_nitrogen_g_n = v[1][0..1], .axis_total_phosphorus_g_p = v[2][0..1], .axis_depth_m = v[3][0..1], .layer_carbon_g_c = &v[4], .layer_nitrogen_g_n = &v[5], .layer_phosphorus_g_p = &v[6], .layer_length_m = &v[7], .layer_protein_carbon_g_c = &v[8], .layer_total_root_carbon_g_c = &v[9], .mobile_carbon_g_c = &v[10], .mobile_nitrogen_g_n = &v[11], .mobile_phosphorus_g_p = &v[12], .total_water_potential_mpa = &v[13], .osmotic_water_potential_mpa = &v[14], .turgor_water_potential_mpa = &v[15], .primary_radius_m = &v[16], .deepest_rooted_layer_index_by_axis = deepest, .primary_root_length_accumulator_m = v[17][0..1], .primary_root_carbon_accumulator_g_c = v[18][0..1] };
}
