...
pub const Inputs = struct {
    ...
    bottom_forward_scattered_shortwave_mj_per_m2_h: f64,
...
pub const Result = struct {
    transmitted_direct_shortwave_mj_per_m2_h: f64,
    transmitted_diffuse_shortwave_mj_per_m2_h: f64,
...
pub fn compute(inputs: Inputs) !Result {
    try validate(inputs);
    const transmitted_direct_shortwave_mj_per_m2_h =
        inputs.direct_shortwave_mj_per_m2_h * inputs.bottom_direct_transmittance;
    const transmitted_diffuse_shortwave_mj_per_m2_h =
        inputs.working_diffuse_shortwave_mj_per_m2_h *
        inputs.bottom_diffuse_transmittance +
        inputs.bottom_forward_scattered_shortwave_mj_per_m2_h;
    ...
    var ground_shortwave_mj_per_m2_h =
        @abs(inputs.ground_direct_incidence_fraction) *
        transmitted_direct_shortwave_mj_per_m2_h;
    var ground_par_umol_per_m2_s =
        @abs(inputs.ground_direct_incidence_fraction) *
        transmitted_direct_par_umol_per_m2_s;
    for (inputs.ground_sky_incidence_fraction) |incidence| {
        ground_shortwave_mj_per_m2_h +=
            @abs(incidence) * transmitted_diffuse_shortwave_mj_per_m2_h;
        ground_par_umol_per_m2_s +=
            @abs(incidence) * transmitted_diffuse_par_umol_per_m2_s;
    }
    const ground_shortwave_mj_h =
        ground_shortwave_mj_per_m2_h * inputs.cell_area_m2;
    return .{
        .transmitted_direct_shortwave_mj_per_m2_h = transmitted_direct_shortwave_mj_per_m2_h,
        .transmitted_diffuse_shortwave_mj_per_m2_h = transmitted_diffuse_shortwave_mj_per_m2_h,
        ...
        .ground_shortwave_mj_per_m2_h = ground_shortwave_mj_per_m2_h,
        .ground_par_umol_per_m2_s = ground_par_umol_per_m2_s,
        .ground_shortwave_mj_h = ground_shortwave_mj_h,
    };
}

fn validate(inputs: Inputs) !void {
    inline for (.{
        inputs.direct_shortwave_mj_per_m2_h,
...
test "direct and runtime sky-zone radiation aggregate in source order" {
    const result = try compute(.{
        ...
        .bottom_forward_scattered_shortwave_mj_per_m2_h = 0.2,
        .bottom_forward_scattered_par_umol_per_m2_s = 8,
        .ground_direct_incidence_fraction = -0.5,
        .ground_sky_incidence_fraction = &.{ -0.25, 0.75 },
        .cell_area_m2 = 10,
    });
    try std.testing.expectEqual(@as(f64, 1), result.transmitted_direct_shortwave_mj_per_m2_h);
    try std.testing.expectEqual(@as(f64, 1), result.transmitted_diffuse_shortwave_mj_per_m2_h);
    try std.testing.expectEqual(@as(f64, 1.5), result.ground_shortwave_mj_per_m2_h);
    try std.testing.expectEqual(@as(f64, 65), result.ground_par_umol_per_m2_s);
    try std.testing.expectEqual(@as(f64, 15), result.ground_shortwave_mj_h);
}

test "empty sky-zone extent fails explicitly" {
    try std.testing.expectError(error.ZeroGroundSkyZoneExtent, compute(.{
        .direct_shortwave_mj_per_m2_h = 0,
        .direct_par_umol_per_m2_s = 0,
        .bottom_direct_transmittance = 1,
        .working_diffuse_shortwave_mj_per_m2_h = 0,
        .working_diffuse_par_umol_per_m2_s = 0,
        .bottom_diffuse_transmittance = 1,
        .bottom_forward_scattered_shortwave_mj_per_m2_h = 0,
