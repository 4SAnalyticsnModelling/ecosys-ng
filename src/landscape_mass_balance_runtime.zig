...
const mineral_nitrogen_module = @import("mineral_nitrogen_transport.zig");
...
const solute_module = @import("solute_transport.zig");
const zone_module = @import("solute_charge_classification.zig");
const litter_chemistry_module = @import("surface_litter_chemistry.zig");
const litter_fertilizer_module = @import("surface_litter_fertilizer.zig");
const surface_module = @import("surface_precipitation.zig");
const canopy_retention_module = @import("canopy_precipitation_retention.zig");
...
/// `soil_mass_Mg_scratch` is caller-owned runtime memory so reconstruction
...
pub const Inputs = struct {
    ...
    surface_litter_dry_mass_Mg: []const f64,
    canopy_retention: ?*const canopy_retention_module.State,
    cell_area_m2: []const f64,
    soil_mass_Mg_scratch: []f64,
...
pub fn reconstruct(
    ...
    try storage.add(try inventory.aggregateSnow(
        inputs.snow,
        inputs.parameters.snow_ice_density_Mg_per_m3,
    ));
    try storage.add(try inventory.aggregateSoilPhysicalAndGas(
        inputs.grid,
        inputs.soil_thermal.total_heat_capacity_mj_per_m3_k,
    ...
    try storage.add(try inventory.aggregateSurfaceChemistry(
        ...
        inputs.surface_litter_dry_mass_Mg,
        inputs.parameters.carbon_g_per_mol,
        inputs.parameters.nitrogen_g_per_mol,
        inputs.parameters.phosphorus_g_per_mol,
    ));
...
pub fn deriveSoilMass(
    ...
    if (matrix_bulk_volume_m3.len == 0 or
        bulk_density_Mg_per_m3.len != matrix_bulk_volume_m3.len or
        soil_mass_Mg.len != matrix_bulk_volume_m3.len)
        return error.RuntimeSoilMassDimensionMismatch;
    for (matrix_bulk_volume_m3, bulk_density_Mg_per_m3) |volume_m3, density_Mg_per_m3| {
        ...
        const mass_Mg = volume_m3 * density_Mg_per_m3;
        if (!std.math.isFinite(mass_Mg))
            return error.RuntimeSoilMassOverflow;
    }
    for (
        matrix_bulk_volume_m3,
        bulk_density_Mg_per_m3,
        soil_mass_Mg,
    ) |volume_m3, density_Mg_per_m3, *mass_Mg| {
        mass_Mg.* = volume_m3 * density_Mg_per_m3;
    }
}
...
fn validateDimensions(inputs: Inputs) !void {
    ...
    if (layers == 0 or cells == 0 or
        inputs.soil_thermal.total_heat_capacity_mj_per_m3_k.len != layers or
        ...
        inputs.soil_mass_Mg_scratch.len != layers or
        inputs.surface_ice_water_equivalent_m3.len != cells or
        inputs.surface_litter_dry_mass_Mg.len != cells or
