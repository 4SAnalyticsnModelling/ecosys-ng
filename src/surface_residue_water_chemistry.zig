...
pub const Inputs = struct {
    ...
    improved_matric_potential_mpa: f64,
...
/// Translates HOUR1 lines 4488-4525 while replacing the preceding residue
/// PSISM calculation with `improved_matric_potential_mpa`.
pub fn calculate(inputs: Inputs) CalculationError!Result {
    ...
    if (inputs.improved_matric_potential_mpa > 0.0) return error.InvalidMatricPotential;
    ...
    const gravitational_mpa = 0.0098 * residue_midpoint_elevation_m;
    const potentials = WaterPotentials{
        .matric_mpa = inputs.improved_matric_potential_mpa,
        .osmotic_mpa = osmotic_mpa,
        .gravitational_mpa = gravitational_mpa,
        .total_mpa = @min(
            0.0,
            inputs.improved_matric_potential_mpa + osmotic_mpa + gravitational_mpa,
        ),
    };
...
test "wet residue calculates elevation-aware potentials and nutrient concentrations" {
    const result = try calculate(.{
        ...
        .improved_matric_potential_mpa = -0.2,
    ...
    try std.testing.expect(result.potentials.gravitational_mpa > 0.0);
...
test "dry residue inherits soil potentials and zeros chemistry" {
    ...
    const result = try calculate(.{
        ...
        .volume_threshold_m3 = 0.0,
        .improved_matric_potential_mpa = 0.0,
