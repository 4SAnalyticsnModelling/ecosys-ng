...
pub const UreaParameters = struct {
    minimum_half_saturation_mol_n_per_Mg: f64,
...
pub const UreaResult = struct {
    ...
    effective_half_saturation_mol_n_per_Mg: f64,
...
pub fn ureaHydrolysis(inputs: UreaInputs, parameters: UreaParameters) !UreaResult {
    ...
    const coqck = if (inputs.biologically_active_water_volume_m3 > parameters.negligible_amount)
        ...
        0.1e6;
    const effective_half_saturation = parameters.minimum_half_saturation_mol_n_per_Mg * (1 + coqck / parameters.microbial_activity_inhibition_g_c_per_m3_h);
    ...
    const broadcast_concentration = fertilizerConcentration(inputs.broadcast_urea_mol_n, inputs.soil_mass_Mg, inputs.water_volume_m3, parameters.negligible_amount);
    const banded_concentration = fertilizerConcentration(inputs.banded_urea_mol_n, inputs.soil_mass_Mg, inputs.water_volume_m3, parameters.negligible_amount);
    ...
    return .{
        ...
        .effective_half_saturation_mol_n_per_Mg = effective_half_saturation,
...
fn fertilizerConcentration(amount_mol_n: f64, soil_mass_Mg: f64, water_volume_m3: f64, negligible: f64) f64 {
    if (amount_mol_n > negligible and soil_mass_Mg > negligible) return amount_mol_n / soil_mass_Mg;
...
fn validateUrea(inputs: UreaInputs, parameters: UreaParameters) !void {
    ...
    if (inputs.current_inhibitor_activity > 1 or inputs.timestep_h <= 0 or parameters.minimum_half_saturation_mol_n_per_Mg <= 0 or parameters.microbial_activity_inhibition_g_c_per_m3_h <= 0) return error.InvalidUreaHydrolysisInput;
}
...
test "urea hydrolysis and fertilizer dissolution conserve fertilizer nitrogen" {
    ...
    const hydrolysis = try ureaHydrolysis(.{ .broadcast_urea_mol_n = 1, .banded_urea_mol_n = 1, .soil_mass_Mg = 1, .water_volume_m3 = 1, .biologically_active_water_volume_m3 = 1, .total_microbial_respiration_activity_g_c_per_step = 0.1, .temperature_response = 1, .initial_inhibitor_activity = 1, .current_inhibitor_activity = 0.5, .timestep_h = 1 }, .{ .minimum_half_saturation_mol_n_per_Mg = 0.05, .microbial_activity_inhibition_g_c_per_m3_h = 50, .specific_hydrolysis_mol_n_per_g_c_h = 0.03, .inhibitor_decline_rate_per_h = 0.01, .negligible_amount = 1e-12 });
...
test "urea hydrolysis uses legacy TOQCK / VOLQ / TFNQ operands" {
    ...
    const broadcast_fraction = (broadcast_urea_mol_n / soil_mass_mg) / (broadcast_urea_mol_n / soil_mass_mg + expected_effective_half_saturation);
    const banded_fraction = (banded_urea_mol_n / soil_mass_mg) / (banded_urea_mol_n / soil_mass_mg + expected_effective_half_saturation);
    ...
    const result = try ureaHydrolysis(
        ...
        .{
            .minimum_half_saturation_mol_n_per_Mg = minimum_half_saturation,
    ...
    try std.testing.expectApproxEqAbs(expected_effective_half_saturation, result.effective_half_saturation_mol_n_per_Mg, 1e-12);
...
test "first fertilizer boundary commits legacy-formula hydrolysis and dissolution to aqueous state" {
    ...
    const fertilizer_parameters: UreaParameters = .{
        .minimum_half_saturation_mol_n_per_Mg = 1.5,
    ...
    const expected_effective_half_saturation = fertilizer_parameters.minimum_half_saturation_mol_n_per_Mg * (1 + coqck / fertilizer_parameters.microbial_activity_inhibition_g_c_per_m3_h);
    const broadcast_urea_concentration = broadcast_urea_mol_n / soil_mass_Mg;
    const banded_urea_concentration = banded_urea_mol_n / soil_mass_Mg;
...
test "zero biologically active water uses COQCK saturation cap" {
    ...
    const expected_fraction = (broadcast_urea_mol_n / soil_mass_mg) / (broadcast_urea_mol_n / soil_mass_mg + expected_effective_half_saturation);
    ...
    const result = try ureaHydrolysis(
        ...
        .{
            .minimum_half_saturation_mol_n_per_Mg = 0.2,
