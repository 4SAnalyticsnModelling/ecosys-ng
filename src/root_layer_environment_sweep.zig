...
/// GROSUB lines 5975--6034 runtime N,L traversal.
///
    ...
    for (0..inputs.biological_domain_count) |domain| {
        ...
        for (inputs.planting_layer_index..deepest_layer + 1) |layer| {
            ...
            _ = try metabolism.rootEnvironmentResponses(
                ...
                inputs.minimum_extension_water_potential_mpa,
...
fn validInputs(
    ...
    return .{
        ...
        .minimum_extension_water_potential_mpa = -1.5,
...
test "GROSUB environment sweep preserves plant-wide rooted bound and thin-layer scan" {
    ...
    try std.testing.expect(environments[6].scaled_penetration_resistance_mpa > environments[4].scaled_penetration_resistance_mpa);
}
...
test "GROSUB environment sweep is atomic on invalid late domain-layer input" {
    ...
    var environments = [_]metabolism.RootEnvironmentResponses{.{
        ...
        .maintenance_water = 4,
        .extension_water = 5,
        .scaled_penetration_resistance_mpa = 6,
    }} ** 8;
...
test "GROSUB environment sweep rejects non-finite admission geometry atomically" {
    ...
    var environments = [_]metabolism.RootEnvironmentResponses{.{ .maintenance_temperature = 1, .acidity = 2, .growth_water = 3, .maintenance_water = 4, .extension_water = 5, .scaled_penetration_resistance_mpa = 6 }} ** 8;
    const before_next = next_lower;
    const before_environment = environments;
