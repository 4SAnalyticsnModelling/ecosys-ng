...
const harvest = @import("plant_salt_harvest.zig");
const surface_chemistry = @import("surface_litter_chemistry.zig");
...
fn preflightExchangeLayer(
    ...
    soil: ?@import("solute_aqueous_network.zig").State,
) !void {
    for (0..salt_count) |salt| {
...
fn soilConcentration(cell: @import("solute_aqueous_network.zig").State, salt: harvest.Salt) f64 {
    return switch (salt) {
...
fn addSoilConcentration(cell: *@import("solute_aqueous_network.zig").State, salt: harvest.Salt, increment: f64) void {
    switch (salt) {
