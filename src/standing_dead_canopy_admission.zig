...
pub const Inputs = struct {
    ...
    minimum_water_solver_heat_capacity_mj_per_m2_k: f64,
    minimum_energy_solver_heat_capacity_mj_per_m2_k: f64,
    cell_area_m2: f64,
    ...
    absorbed_shortwave_rate_mj_per_h: f64,
    emissivity: f64,
    sky_longwave_rate_mj_per_h: f64,
    lateral_longwave_rate_mj_per_h: f64,
    combined_area_radiation_fraction: f64,
    latent_conductance_m2_per_h: f64,
    sensible_conductance_mj_per_m_k_h: f64,
};
...
pub const Result = struct {
    ...
    minimum_water_solver_heat_capacity_mj_per_k: f64,
    minimum_energy_solver_heat_capacity_mj_per_k: f64,
    surface_temperature_k: f64,
...
pub fn compute(inputs: Inputs) !Result {
    ...
    result.dry_heat_capacity_mj_per_k = 2.496 * @min(
    ...
    result.wet_heat_capacity_mj_per_k =
        result.dry_heat_capacity_mj_per_k +
        4.19 * @max(0, inputs.surface_water_m3);
    result.minimum_water_solver_heat_capacity_mj_per_k =
        inputs.minimum_water_solver_heat_capacity_mj_per_m2_k * inputs.cell_area_m2;
    result.minimum_energy_solver_heat_capacity_mj_per_k =
        inputs.minimum_energy_solver_heat_capacity_mj_per_m2_k * inputs.cell_area_m2;
    ...
    result.temperature_increment_k =
        if (result.dry_heat_capacity_mj_per_k >
        result.minimum_water_solver_heat_capacity_mj_per_k)
            inputs.large_temperature_increment_k
        else
            inputs.small_temperature_increment_k;

    if (result.wet_heat_capacity_mj_per_k <=
        result.minimum_energy_solver_heat_capacity_mj_per_k or
        inputs.radiation_fraction <= inputs.radiation_presence_threshold)
        return result;

    result.energy_balance_admitted = true;
    result.absorbed_shortwave_mj_per_step =
        inputs.absorbed_shortwave_rate_mj_per_h * inputs.timestep_h;
    result.emitted_longwave_coefficient_mj_per_step_k4 =
        inputs.emissivity * 2.04e-10 * inputs.radiation_fraction *
        inputs.cell_area_m2 * inputs.timestep_h;
    result.absorbed_sky_longwave_mj_per_step =
        inputs.sky_longwave_rate_mj_per_h * inputs.radiation_fraction *
        inputs.timestep_h;
    result.absorbed_lateral_longwave_mj_per_step =
        inputs.lateral_longwave_rate_mj_per_h *
    ...
    result.sensible_conductance_mj_per_m_k_step =
        inputs.radiation_fraction * inputs.sensible_conductance_mj_per_m_k_h;
...
fn validateActiveInputs(inputs: Inputs) !void {
    ...
    if (inputs.standing_dead_air_temperature_k <= 0 or
        ...
        inputs.minimum_water_solver_heat_capacity_mj_per_m2_k < 0 or
        inputs.minimum_energy_solver_heat_capacity_mj_per_m2_k < 0 or
        ...
        inputs.absorbed_shortwave_rate_mj_per_h < 0 or inputs.emissivity < 0 or
        inputs.sky_longwave_rate_mj_per_h < 0 or
        ...
        inputs.sensible_conductance_mj_per_m_k_h < 0)
        return error.InvalidStandingDeadCanopyAdmissionInput;
}
...
fn testInputs() Inputs {
    return .{
        ...
        .minimum_water_solver_heat_capacity_mj_per_m2_k = 0.01,
        .minimum_energy_solver_heat_capacity_mj_per_m2_k = 0.01,
        .cell_area_m2 = 10,
        ...
        .absorbed_shortwave_rate_mj_per_h = 4,
        .emissivity = 0.96,
        .sky_longwave_rate_mj_per_h = 2,
        .lateral_longwave_rate_mj_per_h = -0.5,
        .combined_area_radiation_fraction = 0.3,
        .latent_conductance_m2_per_h = 8,
        .sensible_conductance_mj_per_m_k_h = 6,
    };
}
...
test "standing-dead admission preserves source initialization order" {
    ...
    try std.testing.expectApproxEqAbs(@as(f64, 0.2496), result.dry_heat_capacity_mj_per_k, 1e-15);
    try std.testing.expectApproxEqAbs(@as(f64, 0.3334), result.wet_heat_capacity_mj_per_k, 1e-15);
    try std.testing.expectEqual(@as(f64, 0.1), result.temperature_increment_k);
    try std.testing.expectEqual(@as(f64, 1), result.absorbed_shortwave_mj_per_step);
...
test "low wet heat capacity rejects only energy-balance admission" {
    var inputs = testInputs();
    inputs.minimum_energy_solver_heat_capacity_mj_per_m2_k = 1;
    const result = try compute(inputs);
    try std.testing.expect(result.standing_dead_present);
    try std.testing.expect(!result.energy_balance_admitted);
    try std.testing.expectEqual(@as(f64, 275), result.surface_temperature_k);
    try std.testing.expectEqual(@as(f64, 0), result.absorbed_shortwave_mj_per_step);
}

test "dry heat-capacity threshold selects small temperature increment" {
    var inputs = testInputs();
    inputs.minimum_water_solver_heat_capacity_mj_per_m2_k = 1;
    const result = try compute(inputs);
