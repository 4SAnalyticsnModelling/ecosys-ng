...
pub fn apply(allocator: std.mem.Allocator, state: *snow.State, inputs: Inputs, parameters: Parameters) !void {
    ...
    inline for (.{ inputs.timestep_h, inputs.initial_snow_density_Mg_per_m3, inputs.ice_density_Mg_per_m3, parameters.maximum_temperature_metamorphism_density_Mg_per_m3, parameters.temperature_metamorphism_rate_per_h, parameters.temperature_metamorphism_exponent_per_c, parameters.viscosity_scale_Mg_h_per_m3, parameters.viscosity_temperature_exponent_per_c, parameters.viscosity_density_exponent_m3_per_Mg, parameters.minimum_snowfall_temperature_c, parameters.maximum_snowfall_temperature_c, parameters.snowfall_density_temperature_coefficient_Mg_per_m3_c_pow_1_5 }) |value| if (!std.math.isFinite(value)) return error.NonFiniteSnowCompactionParameter;
    if (inputs.timestep_h <= 0 or inputs.initial_snow_density_Mg_per_m3 <= 0 or inputs.ice_density_Mg_per_m3 <= 0 or parameters.maximum_temperature_metamorphism_density_Mg_per_m3 <= 0 or parameters.temperature_metamorphism_rate_per_h < 0 or parameters.viscosity_scale_Mg_h_per_m3 <= 0 or parameters.maximum_snowfall_temperature_c < parameters.minimum_snowfall_temperature_c or parameters.snowfall_density_temperature_coefficient_Mg_per_m3_c_pow_1_5 < 0) return error.InvalidSnowCompactionParameter;

    const density = try allocator.dupe(f64, state.snow_density_Mg_per_m3);
    ...
    for (0..state.cell_count) |cell| {
        ...
        for (0..state.layer_capacity) |layer| {
            ...
            const layer_water_equivalent_m3 = state.solid_snow_water_equivalent_m3[index] + state.liquid_water_volume_m3[index] + state.ice_volume_m3[index] * inputs.ice_density_Mg_per_m3;
            ...
            if (layer == 0 and snowfall > 0 and state.solid_snow_water_equivalent_m3[index] > 0) {
                ...
                const snowfall_density_Mg_per_m3 = inputs.initial_snow_density_Mg_per_m3 + parameters.snowfall_density_temperature_coefficient_Mg_per_m3_c_pow_1_5 * std.math.pow(f64, snowfall_temperature_c - parameters.minimum_snowfall_temperature_c, 1.5);
                const prior_solid = @max(0, state.solid_snow_water_equivalent_m3[index] - snowfall);
                const mixed_solid_volume_m3 = snowfall / snowfall_density_Mg_per_m3 + prior_solid / density[index];
            ...
            const temperature_rate = if (density[index] < parameters.maximum_temperature_metamorphism_density_Mg_per_m3)
                ...
                0;
            const viscosity_Mg_h_per_m3 = parameters.viscosity_scale_Mg_h_per_m3 * std.math.exp(parameters.viscosity_temperature_exponent_per_c * temperature_c + parameters.viscosity_density_exponent_m3_per_Mg * density[index]);
            const overburden_rate = density[index] * overburden_water_equivalent_m3 / (area_m2 * viscosity_Mg_h_per_m3);
    ...
    @memcpy(state.snow_density_Mg_per_m3, density);
...
test "empty capacity layers do not accumulate fictitious density" {
    ...
    const empty_density = state.snow_density_Mg_per_m3[1];
    try apply(std.testing.allocator, &state, .{ .snowfall_water_equivalent_m3 = &.{0}, .atmospheric_temperature_k = &.{268}, .timestep_h = 1, .initial_snow_density_Mg_per_m3 = 0.05, .ice_density_Mg_per_m3 = 0.92 }, .{ .maximum_temperature_metamorphism_density_Mg_per_m3 = 0.25, .temperature_metamorphism_rate_per_h = 1e-5, .temperature_metamorphism_exponent_per_c = 0.04, .viscosity_scale_Mg_h_per_m3 = 0.25, .viscosity_temperature_exponent_per_c = -0.08, .viscosity_density_exponent_m3_per_Mg = 23, .minimum_snowfall_temperature_c = -15, .maximum_snowfall_temperature_c = 2, .snowfall_density_temperature_coefficient_Mg_per_m3_c_pow_1_5 = 1.7e-3 });
    try std.testing.expectEqual(empty_density, state.snow_density_Mg_per_m3[1]);
}
