...
pub fn initialize(inputs: Inputs) !InitialState {
    ...
    if (!std.math.isFinite(inputs.diffuse_shortwave_mj_m2_h) or
        !std.math.isFinite(inputs.diffuse_par_umol_m2_s))
        return error.NonFiniteCanopyInterceptionInput;
    if (inputs.diffuse_shortwave_mj_m2_h < 0 or
    ...
    const working_diffuse_shortwave_mj_m2_h =
        inputs.diffuse_shortwave_mj_m2_h;
    ...
    const top_scattered_shortwave_mj_m2_h: f64 = 0.0;
    ...
    return .{
        ...
        .working_diffuse_shortwave_mj_m2_h = working_diffuse_shortwave_mj_m2_h,
        ...
        .top_scattered_shortwave_mj_m2_h = top_scattered_shortwave_mj_m2_h,
...
test "canopy interception accumulators preserve source initialization" {
    const state = try initialize(.{
        .cell_area_m2 = 20,
        .diffuse_shortwave_mj_m2_h = 1.25,
    ...
    try std.testing.expectEqual(
        @as(f64, 1.25),
        state.working_diffuse_shortwave_mj_m2_h,
    );
    try std.testing.expectEqual(
        @as(f64, 300),
    ...
    try std.testing.expectEqual(
        @as(f64, 0),
        state.top_scattered_shortwave_mj_m2_h,
    );
