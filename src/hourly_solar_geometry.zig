...
pub const Inputs = struct {
    ...
    solar_constant_mj_per_m2_h: f64 = 4.896,
};
...
pub const Result = struct {
    ...
    capped_incoming_shortwave_mj_per_m2_h: f64,
};
...
pub fn calculate(inputs: Inputs) !Result {
    ...
    if (inputs.solar_noon_hour < 1 or inputs.solar_noon_hour >= 25 or
        ...
        inputs.solar_constant_mj_per_m2_h <= 0)
        return error.InvalidHourlySolarGeometryInput;

    if (!inputs.outdoors) {
        return .{
            .current_solar_angle_sine = if (inputs.incoming_shortwave_mj_per_m2_h <= 0) 0 else 1,
            .next_hour_solar_angle_sine = 1,
            .extraterrestrial_shortwave_mj_per_m2_h = 0,
            .capped_incoming_shortwave_mj_per_m2_h = inputs.incoming_shortwave_mj_per_m2_h,
        };
    }
    ...
    if (inputs.incoming_shortwave_mj_per_m2_h <= 0)
        current_sine = 0;
    const extraterrestrial =
        inputs.solar_constant_mj_per_m2_h * @max(0.0, current_sine);
    const capped = @min(
        extraterrestrial,
    ...
    return .{
        ...
        .extraterrestrial_shortwave_mj_per_m2_h = extraterrestrial,
        .capped_incoming_shortwave_mj_per_m2_h = capped,
    };
}
...
fn outdoorInputs() Inputs {
    return .{
        ...
        .incoming_shortwave_mj_per_m2_h = 10,
    };
}
...
test "outdoor incoming shortwave is capped by extraterrestrial radiation" {
    ...
    try std.testing.expectEqual(
        result.extraterrestrial_shortwave_mj_per_m2_h,
        result.capped_incoming_shortwave_mj_per_m2_h,
    );
}

test "zero incoming radiation clears current but not next solar sine" {
    var inputs = outdoorInputs();
    inputs.incoming_shortwave_mj_per_m2_h = 0;
...
test "phytotron preserves incoming radiation and source angle flags" {
    ...
    try std.testing.expectEqual(
        inputs.incoming_shortwave_mj_per_m2_h,
        lit.capped_incoming_shortwave_mj_per_m2_h,
    );
