...
fn validate(context: ApplyContext, range: compute.CellRange) !void {
    ...
    if (range.first > range.end or range.end > layers or context.reactive_nitrogen.layer_count != layers or context.chemistry_state.cell_count != layers or context.microbial_state.cell_count * context.microbial_state.layer_count != layers or context.result.process_unit_count_per_layer != context.microbial_state.substrate_count * context.microbial_state.population_count or context.matric_plus_osmotic_potential_mpa.len != layers) return error.SoilMicrobialNitrogenExchangeDimensionMismatch;
...
test "runtime microbial nitrogen exchange immobilizes deficits and mineralizes surplus" {
    ...
    var context: ApplyContext = .{ .result = &result, .reactive_nitrogen = &reactive_state, .microbial_state = &microbial_state, .chemistry_state = &chemistry_state, .model_grid = &model_grid, .matric_plus_osmotic_potential_mpa = &.{0}, .zone_fractions = .{ .ammonium_non_band = 1, .ammonium_band = 0, .nitrate_non_band = 1, .nitrate_band = 0, .phosphate_non_band = 1, .phosphate_band = 0 }, .parameters = parsed, .nitrogen_molar_mass_g_per_mol = 14, .timestep_h = 1, .negligible_amount = 1e-12 };
