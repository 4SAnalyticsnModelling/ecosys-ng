...
pub fn writeCoupled(writer: anytype, grid: GridState, plants: PlantState) !void {
    ...
    inline for (.{ grid.soil_temperature_k, grid.liquid_water_m3, grid.ice_water_m3, grid.matrix_liquid_water_m3, grid.macropore_liquid_water_m3, grid.matrix_ice_water_m3, grid.macropore_ice_water_m3, grid.matrix_pore_capacity_m3, grid.macropore_pore_capacity_m3, grid.matrix_air_volume_m3, grid.macropore_air_volume_m3, grid.air_volume_m3, grid.water_vapor_volume_m3, grid.matric_potential_mpa, grid.surface_temperature_k, plants.canopy_temperature_k, plants.canopy_water_potential_mpa, plants.canopy_water_storage_m_per_m2, plants.leaf_area_index_m2_m2, plants.shoot_carbon_g_m2, plants.root_carbon_g_m2 }) |values| try writeF64Slice(writer, values);
}
...
pub fn readCoupledInto(reader: *std.Io.Reader, grid: *GridState, plants: *PlantState) !void {
    ...
    try readF64Slice(reader, "matric_potential_mpa", grid.matric_potential_mpa);
    ...
    try readF64Slice(reader, "canopy_water_potential_mpa", plants.canopy_water_potential_mpa);
...
pub fn write(writer: anytype, state: GridState) !void {
    ...
    try writeF64Slice(writer, state.matric_potential_mpa);
...
pub fn readInto(reader: *std.Io.Reader, state: *GridState) !void {
    ...
    try readF64Slice(reader, "matric_potential_mpa", state.matric_potential_mpa);
