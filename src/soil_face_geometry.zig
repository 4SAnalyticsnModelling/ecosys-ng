...
pub const State = struct {
    ...
    pub fn initMapped(
        ...
        result.wetting_depth_factor = try allocator.alloc(f64, count);
        errdefer allocator.free(result.wetting_depth_factor);
        @memset(result.wetting_depth_factor, 0);
        result.macropore_hydraulic_conductance_m_per_h_mpa = try allocator.alloc(f64, count);
        errdefer allocator.free(result.macropore_hydraulic_conductance_m_per_h_mpa);
        @memset(result.macropore_hydraulic_conductance_m_per_h_mpa, 0);
    ...
    pub fn deinit(self: *State) void {
        self.allocator.free(self.macropore_hydraulic_conductance_m_per_h_mpa);
        self.allocator.free(self.wetting_depth_factor);
    ...
    pub fn validateFinite(self: *const State) !void {
        ...
        inline for (.{ self.wetting_depth_factor, self.macropore_hydraulic_conductance_m_per_h_mpa }) |values| for (values) |value| if (!std.math.isFinite(value) or value < 0) return error.InvalidSoilFaceGeometry;
    }
};
...
test "mapped faces reproduce STARTS source AREA and full DLYR paths" {
    const cfg = try @import("config.zig").SimulationConfig.init(.{ .grid_columns = 2, .grid_rows = 1, .soil_layers = 2, .plant_populations = 1 }, .{ .worker_threads = 1, .tile_cells = 2 }, .{ .relative_tolerance = 1e-8, .absolute_tolerance = 1e-11, .max_nonlinear_iterations = 20 });
