...
pub const Inputs = struct {
    observed_longwave_megajoules_per_m2_h: f64,
    sky_emissivity_fraction: f64,
    air_temperature_k: f64,
    stefan_boltzmann_megajoules_per_m2_h_k4: f64 = 2.04e-10,
};

pub const Result = struct {
    longwave_megajoules_per_m2_h: f64,
...
pub fn select(inputs: Inputs) !Result {
    ...
    if (inputs.observed_longwave_megajoules_per_m2_h < 0 or
        inputs.sky_emissivity_fraction < 0 or
        inputs.sky_emissivity_fraction > 1 or
        inputs.air_temperature_k <= 0 or
        inputs.stefan_boltzmann_megajoules_per_m2_h_k4 <= 0)
        return error.InvalidSkyLongwaveInput;

    if (inputs.observed_longwave_megajoules_per_m2_h > 0) {
        return .{
            .longwave_megajoules_per_m2_h = inputs.observed_longwave_megajoules_per_m2_h,
    ...
    return .{
        .longwave_megajoules_per_m2_h = calculated,
...
test "strictly positive observed longwave bypasses calculation" {
    const result = try select(.{
        .observed_longwave_megajoules_per_m2_h = 0.8,
    ...
    try std.testing.expectEqual(
        @as(f64, 0.8),
        result.longwave_megajoules_per_m2_h,
    );
}

test "zero observation invokes exact emissivity temperature equation" {
    const result = try select(.{
        .observed_longwave_megajoules_per_m2_h = 0,
    ...
    try std.testing.expect(!result.used_observation);
    try std.testing.expectEqual(expected, result.longwave_megajoules_per_m2_h);
}
...
test "runtime radiation coefficient controls calculated result" {
    const result = try select(.{
        ...
        .stefan_boltzmann_megajoules_per_m2_h_k4 = 2.1e-10,
    });
    try std.testing.expectEqual(
...
test "invalid measured and derived inputs fail immediately" {
    try std.testing.expectError(
        ...
        select(.{
            .observed_longwave_megajoules_per_m2_h = -0.1,
