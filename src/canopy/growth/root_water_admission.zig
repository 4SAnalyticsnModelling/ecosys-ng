...
pub const Inputs = struct {
    ...
    wood_elastic_modulus_megapascal: f64,
...
pub const Result = struct {
    total_soil_root_conductance_m_per_h_megapascal: f64,
    hydraulic_canopy_height_m: f64,
    gravitational_canopy_water_potential_megapascal: f64,
            ...
            @max(
                ...
                1 +
                    inputs.canopy_total_water_potential_megapascal /
                        inputs.wood_elastic_modulus_megapascal,
            ),
            inputs.conducting_radius_exponent,
        );
    const result = Result{
        .total_soil_root_conductance_m_per_h_megapascal = total_conductance,
        .hydraulic_canopy_height_m = hydraulic_height,
        .gravitational_canopy_water_potential_megapascal = gravitational_potential,
...
fn validate(inputs: Inputs) !void {
    ...
    if (inputs.wet_canopy_heat_capacity_megajoules_per_k < 0 or
        ...
        inputs.wood_elastic_modulus_megapascal <= 0 or
...
fn admittedInputs() Inputs {
    return .{
        ...
        .canopy_height_m = 2,
        .canopy_total_water_potential_megapascal = -10,
        .wood_elastic_modulus_megapascal = 50,
...
test "UPTAKE admitted canopy initializes hydraulic preamble in source order" {
    ...
    try std.testing.expectEqual(@as(f64, 0), result.total_soil_root_conductance_m_per_h_megapascal);
    try std.testing.expectEqual(@as(f64, 1.6), result.hydraulic_canopy_height_m);
    try std.testing.expectApproxEqAbs(
        @as(f64, -0.01568),
        result.gravitational_canopy_water_potential_megapascal,
        1e-15,
    );
    try std.testing.expectApproxEqAbs(
...
test "conducting radius response retains source floor and fourth power" {
    var inputs = admittedInputs();
    inputs.canopy_total_water_potential_megapascal = -100;
