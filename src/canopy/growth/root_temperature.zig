...
pub const Inputs = struct {
    ...
    previous_daily_minimum_canopy_water_potential_megapascal: f64,
...
pub const Result = struct {
    ...
    daily_minimum_canopy_water_potential_megapascal: f64,
};
...
pub fn calculate(
    ...
    return .{
        ...
        .daily_minimum_canopy_water_potential_megapascal = @min(
            inputs.previous_daily_minimum_canopy_water_potential_megapascal,
...
test "pre-leafout deciduous canopy selects soil but Arrhenius retains TKC" {
    ...
    const result = try calculate(.{
        ...
        .first_soil_layer = 0,
        .end_soil_layer = 2,
        .previous_daily_minimum_canopy_water_potential_megapascal = -1,
    ...
    try std.testing.expectEqual(@as(f64, -2), result.daily_minimum_canopy_water_potential_megapascal);
}
...
test "evergreen canopy selects canopy temperature and partial root range" {
    ...
    const result = try calculate(.{
        ...
        .first_soil_layer = 1,
        .end_soil_layer = 2,
        .previous_daily_minimum_canopy_water_potential_megapascal = -3,
        .canopy_total_water_potential_megapascal = -2,
    }, &root_response);
    ...
    try std.testing.expectEqual(@as(f64, -3), result.daily_minimum_canopy_water_potential_megapascal);
}
