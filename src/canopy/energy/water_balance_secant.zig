...
pub const Result = struct {
    absolute_water_potential_change_megapascal: f64,
...
pub fn calculate(inputs: Inputs) !Result {
    ...
    const potential_change = @abs(
        inputs.current_canopy_water_potential_megapascal -
            inputs.previous_canopy_water_potential_megapascal,
    );
    ...
    const flux_resistance =
        ...
        potential_change > inputs.significant_change_threshold)
            ...
            inputs.fallback_resistance_mpa_per_m;
    const result = Result{
        .absolute_water_potential_change_megapascal = potential_change,
...
test "UPTAKE canopy water secants preserve source order" {
    const result = try calculate(.{
        .current_canopy_water_potential_megapascal = -1.5,
    ...
    try std.testing.expectEqual(@as(f64, 0.5), result.absolute_water_potential_change_megapascal);
