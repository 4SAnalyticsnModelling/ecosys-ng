...
/// STOMATE.F lines 420--461: C4 bundle-sheath C3 capacity in exact source
/// assignment order.
pub fn compute(inputs: Inputs) !Result {
    ...
    if (result.co2_unlimited_carboxylation_umol_per_m2_s <= 0)
        return error.ZeroBundleSheathRubiscoCapacity;
    ...
    result.co2_limited_carboxylation_umol_per_m2_s = @max(
        0,
        result.co2_unlimited_carboxylation_umol_per_m2_s *
            (inputs.bundle_sheath_co2_umol_per_l -
                result.co2_compensation_umol_per_l) /
            (inputs.bundle_sheath_co2_umol_per_l +
    ...
    inline for (@typeInfo(Result).@"struct".fields) |field|
        if (!std.math.isFinite(@field(result, field.name)))
            return error.NonFiniteBundleSheathC3CapacityResult;
    return result;
}
...
pub fn computeRuntimeNodes(
    ...
    if (inputs.len != scratch.len or inputs.len != destination.len)
        return error.BundleSheathC3CapacityDimensionMismatch;
...
fn validate(inputs: Inputs) !void {
    inline for (@typeInfo(Inputs).@"struct".fields) |field|
        if (!std.math.isFinite(@field(inputs, field.name)))
            return error.NonFiniteBundleSheathC3CapacityInput;
    if (inputs.leaf_protein_surface_density_g_per_m2 <= 0 or
        ...
        inputs.bundle_sheath_co2_umol_per_l < 0 or
        ...
        return error.InvalidBundleSheathC3CapacityInput;
}
...
fn compatibilityInputs() Inputs {
    return .{
        ...
        .bundle_sheath_co2_umol_per_l = 1000,
...
test "bundle sheath C3 capacity publishes every source field" {
    ...
    try std.testing.expect(result.co2_limited_carboxylation_umol_per_m2_s > 0);
}

test "bundle sheath shared outputs reconcile with existing C3 capacity" {
...
test "later invalid bundle sheath node leaves destination unchanged" {
    var invalid = compatibilityInputs();
    invalid.rubisco_o2_half_saturation_umol_per_l = 0;
    ...
    try std.testing.expectError(
        error.InvalidBundleSheathC3CapacityInput,
