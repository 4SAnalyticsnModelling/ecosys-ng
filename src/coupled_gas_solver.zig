...
test "coupled solve converges before NPH times NPG and conserves closed system" {
...
test "failed coupled solve rolls back all three phases" {
    var state = try gas.State.init(std.testing.allocator, 1);
    defer state.deinit();
    state.air_volume_m3[0] = 1;
    state.temperature_k[0] = 300;
