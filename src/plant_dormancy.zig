...
pub const Parameters = struct {
    ...
    combined_leafout_turgor_potential_mpa: f64,
    leafoff_total_water_potential_mpa: f64,
...
pub fn advanceRemobilization(
    ...
    inline for (.{ inputs.timestep_h, inputs.canopy_temperature_c, inputs.canopy_total_water_potential_mpa }) |value| if (!std.math.isFinite(value)) return error.InvalidRemobilizationState;
    ...
    if (shoot_enabled) {
        phenological_enabled = growth_habit == .annual or phenology_type == .evergreen or
            ...
            (phenology_type != .evergreen and phenology_type != .winter_deciduous and inputs.canopy_total_water_potential_mpa < parameters.leafoff_total_water_potential_mpa);
...
pub const Inputs = struct {
    ...
    canopy_total_water_potential_mpa: f64,
    surface_soil_water_potential_mpa: f64,
    seed_layer_soil_water_potential_mpa: f64,
    emerged: bool,
...
fn advanceDroughtDeciduous(state: *State, inputs: Inputs, parameters: Parameters, phenology_type: stages.PhenologyType) void {
    if (!state.leafout_disabled) {
        if (inputs.canopy_total_water_potential_mpa >= parameters.drought_leafout_total_water_potential_mpa) state.accumulated_leafout_h += inputs.timestep_h;
        if (state.accumulated_leafout_h < parameters.required_leafout_h and inputs.canopy_total_water_potential_mpa < parameters.leafoff_total_water_potential_mpa) state.accumulated_leafout_h = @max(0, state.accumulated_leafout_h - inputs.timestep_h);
    ...
    if (state.leafout_disabled and !state.leafoff_disabled) {
        const water_potential = if (inputs.emerged) inputs.canopy_total_water_potential_mpa else @min(inputs.seed_layer_soil_water_potential_mpa, inputs.surface_soil_water_potential_mpa);
        if (water_potential < parameters.leafoff_total_water_potential_mpa) state.accumulated_leafoff_h += inputs.timestep_h;
...
fn advanceCombinedDeciduous(state: *State, inputs: Inputs, parameters: Parameters, daylength_increasing: bool) void {
    if ((daylength_increasing or inputs.current_daylength_h >= inputs.maximum_seasonal_daylength_h - 2) and !state.leafout_disabled) {
        if (inputs.canopy_temperature_c >= parameters.leafout_temperature_threshold_c and inputs.canopy_turgor_potential_mpa > parameters.combined_leafout_turgor_potential_mpa) state.accumulated_leafout_h += inputs.timestep_h;
        if (state.accumulated_leafout_h < parameters.required_leafout_h and (inputs.canopy_temperature_c < parameters.chilling_temperature_c or inputs.canopy_turgor_potential_mpa < parameters.combined_leafout_turgor_potential_mpa)) state.accumulated_leafout_h = @max(0, state.accumulated_leafout_h - inputs.timestep_h);
        if (state.accumulated_leafout_h >= parameters.required_leafout_h) {
            state.accumulated_leafoff_h = 0;
            if (inputs.floral_initiated) state.leafoff_disabled = false;
        }
    ...
    if ((!daylength_increasing or inputs.current_daylength_h < 24 - inputs.maximum_seasonal_daylength_h + 2) and !state.leafoff_disabled) {
        if (inputs.canopy_temperature_c <= parameters.leafoff_temperature_threshold_c or inputs.canopy_total_water_potential_mpa < parameters.leafoff_total_water_potential_mpa) state.accumulated_leafoff_h += inputs.timestep_h;
        if (state.accumulated_leafoff_h >= parameters.required_leafoff_h and state.leafout_disabled) {
            state.accumulated_leafout_h = 0;
            state.leafout_disabled = false;
        }
...
fn testParameters() Parameters {
    return .{ .required_leafout_h = 2, .required_leafoff_h = 2, .leafout_temperature_threshold_c = 5, .leafoff_temperature_threshold_c = 0, .chilling_temperature_c = -5, .drought_leafout_total_water_potential_mpa = -0.1, .combined_leafout_turgor_potential_mpa = 0.1, .leafoff_total_water_potential_mpa = -1.5, .maximum_photoperiod_counter_h = 3600, .evergreen_leafoff_remobilization_start_fraction = 0.75, .deciduous_leafoff_remobilization_start_fraction = 0.5, .full_senescence_duration_h = 480 };
}

fn testInputs() Inputs {
    return .{ .day_of_year = 100, .execution_year = 2000, .latitude_deg_n = 53, .timestep_h = 1, .current_daylength_h = 13, .previous_daylength_h = 12.9, .maximum_seasonal_daylength_h = 17, .canopy_temperature_c = 10, .canopy_turgor_potential_mpa = 0.2, .canopy_total_water_potential_mpa = -0.05, .surface_soil_water_potential_mpa = -0.1, .seed_layer_soil_water_potential_mpa = -0.1, .emerged = true, .floral_initiated = false };
}
...
test "GROSUB remobilization gates preserve elapsed time and reset semantics" {
    ...
    var inputs: RemobilizationInputs = .{ .timestep_h = 1, .canopy_temperature_c = -1, .canopy_total_water_potential_mpa = -0.05 };
...
test "annual seed-set enables remobilization independently of leafoff hours" {
    const parameters = testParameters();
    var state: State = .{};
    const inputs: RemobilizationInputs = .{ .timestep_h = 1, .canopy_temperature_c = 10, .canopy_total_water_potential_mpa = -0.05 };
...
test "drought deciduous pre-emergence uses driest surface or seed layer" {
    ...
    inputs.surface_soil_water_potential_mpa = -2;
