...
pub const Parameters = struct {
    ...
    sensible_heat_conductivity_mj_per_m_h_k: f64,
    liquid_water_latent_heat_mj_per_m3: f64,
...
pub const State = struct {
    ...
    pub fn init(allocator: std.mem.Allocator, initial_temperature_k: []const f64, initial_vapor_volume_fraction: []const f64, cell_area_m2: []const f64, reference_height_m: f64, parameters: Parameters) !State {
        ...
        for (0..count) |cell| {
            ...
            capacity[cell] = volume[cell] * parameters.volumetric_air_heat_capacity_mj_per_m3_k;
        }
        return .{ .allocator = allocator, .cell_count = count, .temperature_k = temperature, .vapor_volume_fraction = vapor, .heat_capacity_mj_per_k = capacity, .air_volume_m3 = volume, .iteration_count = iterations };
    }

    pub fn deinit(self: *State) void {
        self.allocator.free(self.iteration_count);
        self.allocator.free(self.air_volume_m3);
        self.allocator.free(self.heat_capacity_mj_per_k);
    ...
    pub fn refreshGeometry(self: *State, cell_area_m2: []const f64, reference_height_m: []const f64, parameters: Parameters) !void {
        ...
        for (0..self.cell_count) |cell| {
            ...
            self.heat_capacity_mj_per_k[cell] = self.air_volume_m3[cell] * parameters.volumetric_air_heat_capacity_mj_per_m3_k;
        }
    }
};
...
pub const Inputs = struct {
    ...
    non_atmospheric_sensible_heat_mj_per_h: []const f64,
    non_atmospheric_vapor_flux_m3_per_h: []const f64,
    non_atmospheric_sensible_conductance_mj_per_h_k: []const f64,
...
pub fn deriveSurfaceSources(cell_area_m2: []const f64, surface_sensible_heat_flux_mj_per_m2_h: []const f64, surface_latent_heat_flux_mj_per_m2_h: []const f64, parameters: Parameters, sensible_heat_source_mj_per_h: []f64, vapor_source_m3_per_h: []f64) !void {
    const count = cell_area_m2.len;
    inline for (.{ surface_sensible_heat_flux_mj_per_m2_h.len, surface_latent_heat_flux_mj_per_m2_h.len, sensible_heat_source_mj_per_h.len, vapor_source_m3_per_h.len }) |length| if (length != count) return error.GroundAirDimensionMismatch;
    try validateParameters(parameters);
    for (0..count) |cell| {
        inline for (.{ cell_area_m2[cell], surface_sensible_heat_flux_mj_per_m2_h[cell], surface_latent_heat_flux_mj_per_m2_h[cell] }) |value| if (!std.math.isFinite(value)) return error.NonFiniteGroundAirSurfaceFlux;
        if (cell_area_m2[cell] <= 0) return error.InvalidGroundAirSurfaceArea;
        sensible_heat_source_mj_per_h[cell] = -surface_sensible_heat_flux_mj_per_m2_h[cell] * cell_area_m2[cell];
        vapor_source_m3_per_h[cell] = -surface_latent_heat_flux_mj_per_m2_h[cell] * cell_area_m2[cell] / parameters.liquid_water_latent_heat_mj_per_m3;
    }
}
...
const TemperatureContext = struct {
    ...
    source_heat_mj_per_h: f64,
    source_conductance_mj_per_h_k: f64,
...
pub fn solve(state: *State, inputs: Inputs, parameters: Parameters, solver_options: numerics.SolverOptions) !void {
    inline for (.{ inputs.atmospheric_temperature_k.len, inputs.atmospheric_vapor_volume_fraction.len, inputs.cell_area_m2.len, inputs.bulk_richardson_coefficient_k.len, inputs.neutral_atmospheric_resistance_h_per_m.len, inputs.canopy_resistance_h_per_m.len, inputs.non_atmospheric_sensible_heat_mj_per_h.len, inputs.non_atmospheric_vapor_flux_m3_per_h.len, inputs.non_atmospheric_sensible_conductance_mj_per_h_k.len, inputs.non_atmospheric_sensible_source_temperature_k.len, inputs.non_atmospheric_vapor_conductance_m3_per_h.len, inputs.non_atmospheric_vapor_source_fraction.len }) |length| if (length != state.cell_count) return error.GroundAirDimensionMismatch;
    try validateParameters(parameters);
    ...
    for (0..state.cell_count) |cell| {
        inline for (.{ state.temperature_k[cell], state.vapor_volume_fraction[cell], state.heat_capacity_mj_per_k[cell], state.air_volume_m3[cell], inputs.atmospheric_temperature_k[cell], inputs.atmospheric_vapor_volume_fraction[cell], inputs.cell_area_m2[cell], inputs.bulk_richardson_coefficient_k[cell], inputs.neutral_atmospheric_resistance_h_per_m[cell], inputs.canopy_resistance_h_per_m[cell], inputs.non_atmospheric_sensible_heat_mj_per_h[cell], inputs.non_atmospheric_vapor_flux_m3_per_h[cell], inputs.non_atmospheric_sensible_conductance_mj_per_h_k[cell], inputs.non_atmospheric_sensible_source_temperature_k[cell], inputs.non_atmospheric_vapor_conductance_m3_per_h[cell], inputs.non_atmospheric_vapor_source_fraction[cell] }) |value| if (!std.math.isFinite(value)) return error.NonFiniteGroundAirInput;
        if (state.temperature_k[cell] <= 0 or state.vapor_volume_fraction[cell] < 0 or state.heat_capacity_mj_per_k[cell] <= 0 or state.air_volume_m3[cell] <= 0 or inputs.atmospheric_temperature_k[cell] <= 0 or inputs.atmospheric_vapor_volume_fraction[cell] < 0 or inputs.cell_area_m2[cell] <= 0 or inputs.neutral_atmospheric_resistance_h_per_m[cell] < 0 or inputs.canopy_resistance_h_per_m[cell] < 0 or inputs.non_atmospheric_sensible_conductance_mj_per_h_k[cell] < 0 or inputs.non_atmospheric_sensible_source_temperature_k[cell] <= 0 or inputs.non_atmospheric_vapor_conductance_m3_per_h[cell] < 0 or inputs.non_atmospheric_vapor_source_fraction[cell] < 0) return error.InvalidGroundAirInput;
        const context: TemperatureContext = .{ .old_temperature_k = state.temperature_k[cell], .atmospheric_temperature_k = inputs.atmospheric_temperature_k[cell], .heat_capacity_mj_per_k = state.heat_capacity_mj_per_k[cell], .cell_area_m2 = inputs.cell_area_m2[cell], .bulk_richardson_coefficient_k = inputs.bulk_richardson_coefficient_k[cell], .neutral_resistance_h_per_m = inputs.neutral_atmospheric_resistance_h_per_m[cell], .canopy_resistance_h_per_m = inputs.canopy_resistance_h_per_m[cell], .source_heat_mj_per_h = inputs.non_atmospheric_sensible_heat_mj_per_h[cell], .source_conductance_mj_per_h_k = inputs.non_atmospheric_sensible_conductance_mj_per_h_k[cell], .source_temperature_k = inputs.non_atmospheric_sensible_source_temperature_k[cell], .parameters = parameters };
        var options = solver_options;
        options.residual_scale = @max(1, @abs(context.source_heat_mj_per_h) + context.heat_capacity_mj_per_k);
...
fn temperatureResidual(context: TemperatureContext, temperature_k: f64) f64 {
    ...
    const atmospheric_heat = context.parameters.sensible_heat_conductivity_mj_per_m_h_k * context.cell_area_m2 / resistance * (context.atmospheric_temperature_k - temperature_k);
    const source_heat = context.source_heat_mj_per_h + context.source_conductance_mj_per_h_k * (context.source_temperature_k - temperature_k);
    return source_heat + atmospheric_heat + context.heat_capacity_mj_per_k * (context.old_temperature_k - temperature_k);
}
...
fn temperaturePicard(context: TemperatureContext, temperature_k: f64) f64 {
    ...
    const conductance = context.parameters.sensible_heat_conductivity_mj_per_m_h_k * context.cell_area_m2 / resistance;
    return (context.source_heat_mj_per_h + context.source_conductance_mj_per_h_k * context.source_temperature_k + conductance * context.atmospheric_temperature_k + context.heat_capacity_mj_per_k * context.old_temperature_k) / (context.source_conductance_mj_per_h_k + conductance + context.heat_capacity_mj_per_k);
}
...
fn validateParameters(p: Parameters) !void {
    ...
    if (p.maximum_richardson_number < p.minimum_richardson_number or p.richardson_resistance_multiplier <= 0 or p.minimum_aerodynamic_resistance_h_per_m <= 0 or p.maximum_aerodynamic_resistance_h_per_m < p.minimum_aerodynamic_resistance_h_per_m or p.volumetric_air_heat_capacity_mj_per_m3_k <= 0 or p.minimum_air_column_height_m <= 0 or p.sensible_heat_conductivity_mj_per_m_h_k <= 0 or p.liquid_water_latent_heat_mj_per_m3 <= 0 or p.saturation_vapor_prefactor_k <= 0 or p.saturation_relative_humidity < 0 or p.saturation_relative_humidity > 1 or p.saturation_temperature_k <= 0 or p.saturation_reference_inverse_temperature_per_k <= 0) return error.InvalidGroundAirParameter;
}

test "ground air converges without repeating the full model" {
    const parameters: Parameters = .{ .minimum_richardson_number = -0.1, .maximum_richardson_number = 0.05, .richardson_resistance_multiplier = 10, .minimum_aerodynamic_resistance_h_per_m = 0.00139, .maximum_aerodynamic_resistance_h_per_m = 0.0139, .volumetric_air_heat_capacity_mj_per_m3_k = 1.25e-3, .minimum_air_column_height_m = 5, .sensible_heat_conductivity_mj_per_m_h_k = 1.2e-3, .liquid_water_latent_heat_mj_per_m3 = 2465, .saturation_vapor_prefactor_k = 2.173e-3, .saturation_relative_humidity = 0.61, .saturation_temperature_k = 5360, .saturation_reference_inverse_temperature_per_k = 3.661e-3 };
    var state = try State.init(std.testing.allocator, &.{280}, &.{0.005}, &.{10}, 5, parameters);
    defer state.deinit();
    try solve(&state, .{ .atmospheric_temperature_k = &.{285}, .atmospheric_vapor_volume_fraction = &.{0.004}, .cell_area_m2 = &.{10}, .bulk_richardson_coefficient_k = &.{20}, .neutral_atmospheric_resistance_h_per_m = &.{0.005}, .canopy_resistance_h_per_m = &.{0.002}, .non_atmospheric_sensible_heat_mj_per_h = &.{0.1}, .non_atmospheric_vapor_flux_m3_per_h = &.{0.001}, .non_atmospheric_sensible_conductance_mj_per_h_k = &.{0}, .non_atmospheric_sensible_source_temperature_k = &.{280}, .non_atmospheric_vapor_conductance_m3_per_h = &.{0}, .non_atmospheric_vapor_source_fraction = &.{0} }, parameters, .{ .absolute_tolerance = 1e-10, .relative_tolerance = 1e-8, .max_iterations = 20, .picard_relaxation = 0.5 });
