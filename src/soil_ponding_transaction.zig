...
const nitrogen_fertilizer_module = @import("fertilizer_nitrogen_inventory.zig");
const mineral_fertilizer_module = @import("mineral_fertilizer_inventory.zig");
const roots_module = @import("plant_root_system.zig");
const grid_module = @import("grid.zig");
const thermal_module = @import("soil_thermal.zig");
const mineral_remap = @import("soil_mineral_layer_remap.zig");
const organic_remap = @import("soil_organic_layer_remap.zig");
const chemistry_remap = @import("soil_chemistry_layer_remap.zig");
const nitrite_remap = @import("soil_nitrite_layer_remap.zig");
const gas_remap = @import("soil_gas_layer_remap.zig");
const fertilizer_remap = @import("soil_fertilizer_layer_remap.zig");
const root_remap = @import("plant_root_layer_remap.zig");
const water_heat_remap = @import("soil_water_heat_layer_remap.zig");
const pond_transition = @import("pond_layer_transition.zig");
...
pub const Inputs = struct {
    ...
    fraction: f64,
    source_soil_mass_before_Mg: f64,
    destination_soil_mass_before_Mg: f64,
...
pub fn apply(owners: Owners, inputs: Inputs) !void {
    ...
    try mineral_remap.validatePondedLayerIfComplete(owners.properties, source, destination, inputs.fraction, inputs.source_soil_mass_after_Mg, inputs.destination_soil_mass_after_Mg);
    try organic_remap.validateLayerFraction(owners.organic, source, destination, inputs.fraction);
    try chemistry_remap.validateSolidLayerFraction(owners.chemistry, source, destination, inputs.source_soil_mass_before_Mg, inputs.destination_soil_mass_before_Mg, inputs.source_water_before.shared_m3, inputs.destination_water_before.shared_m3, inputs.source_soil_mass_after_Mg, inputs.destination_soil_mass_after_Mg, inputs.source_water_after.shared_m3, inputs.destination_water_after.shared_m3, inputs.fraction);
    ...
    mineral_remap.transferPondedLayerIfComplete(owners.properties, source, destination, inputs.fraction, inputs.source_soil_mass_after_Mg, inputs.destination_soil_mass_after_Mg) catch unreachable;
    organic_remap.transferLayerFraction(owners.organic, source, destination, inputs.fraction) catch unreachable;
    chemistry_remap.transferSolidLayerFraction(owners.chemistry, source, destination, inputs.source_soil_mass_before_Mg, inputs.destination_soil_mass_before_Mg, inputs.source_water_before.shared_m3, inputs.destination_water_before.shared_m3, inputs.source_soil_mass_after_Mg, inputs.destination_soil_mass_after_Mg, inputs.source_water_after.shared_m3, inputs.destination_water_after.shared_m3, inputs.fraction) catch unreachable;
...
test "REDIST combined pool transaction rejects late root failure before earlier owners mutate" {
    ...
    var dry_heat_capacity_mj_per_m3_k = [_]f64{ 1, 1 };
    var total_heat_capacity_mj_per_m3_k = [_]f64{ 1, 1 };
    ...
    const inputs: Inputs = .{ .cell = 0, .species_count = 1, .plant_population_count = &.{5}, .root_structural_presence_g_per_plant = 0.1, .source_layer = 0, .destination_layer = 1, .fraction = 0.5, .source_soil_mass_before_Mg = 10, .destination_soil_mass_before_Mg = 10, .source_soil_mass_after_Mg = 5, .destination_soil_mass_after_Mg = 15, .source_water_before = water, .destination_water_before = water, .source_water_after = water, .destination_water_after = water, .dynamic_salts = false, .water_heat_parameters = .{ .liquid_water_heat_capacity_mj_per_m3_k = 4.19, .ice_heat_capacity_mj_per_m3_k = 1.9274, .minimum_heat_capacity_mj_per_k = 0 } };
