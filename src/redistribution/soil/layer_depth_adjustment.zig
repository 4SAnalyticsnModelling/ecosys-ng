...
/// Translates redist.f lines 7961–7962:
...
/// Translates redist.f lines 7898–7899:
...
/// Translates redist.f lines 7845–7856:
...
/// Translates redist.f line 7752–7753:
...
/// Per-layer pond depth increment (m step⁻¹) when BKDS(LX) ≤ 0 and BKDS(LX+1) ≤ 0
/// (pond on pond, uses initial layer depth DLYRI, redist.f lines 7758–7759):
...
/// Translates redist.f lines 7964–7995 for soil layers only (BKDS(LX) > 0).
...
/// one layer above NU (redist.f lines 7973–7976) if needed.
...
/// Translates redist.f lines 7896–7948 for soil layers only (BKDS(LX) > 0).
...
/// Propagation of the surface value above NU (redist.f lines 7913–7917) is the
