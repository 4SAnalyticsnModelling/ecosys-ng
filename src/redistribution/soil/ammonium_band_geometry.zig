...
/// Direct translation of the NH4 band-dimension block in REDIST 9490--9516.
pub fn redistribute(context: Context, state: *State) !void {
    if (!finiteStruct(context) or !finiteStruct(state.*) or context.redistribution_fraction < 0 or
        ...
        return error.InvalidSoilAmmoniumBandGeometryInput;
    ...
    if (!finiteStruct(next)) return error.NonFiniteSoilAmmoniumBandGeometryResult;
    state.* = next;
}

test "REDIST ammonium band geometry preserves transfer and update order" {
...
test "REDIST ammonium band geometry obeys source and thickness gates" {
