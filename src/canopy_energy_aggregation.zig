...
pub fn compute(inputs: Inputs) !Result {
    ...
    result.net_radiation_mj_per_step =
        inputs.live_canopy.net_radiation_mj_per_step +
        inputs.standing_dead.net_radiation_mj_per_step;
    result.latent_heat_mj_per_step =
        inputs.live_canopy.latent_heat_mj_per_step +
        inputs.standing_dead.latent_heat_mj_per_step;
    result.sensible_heat_mj_per_step =
        inputs.live_canopy.sensible_heat_mj_per_step +
        inputs.standing_dead.sensible_heat_mj_per_step;
    result.storage_heat_mj_per_step =
        inputs.live_canopy.storage_heat_mj_per_step +
        inputs.standing_dead.storage_heat_mj_per_step;
    result.vapor_convective_heat_mj_per_step =
        inputs.live_canopy.vapor_convective_heat_mj_per_step +
        inputs.standing_dead.vapor_convective_heat_mj_per_step;
    result.emitted_thermal_radiation_mj_per_step =
        inputs.live_canopy.emitted_thermal_radiation_mj_per_step +
        inputs.standing_dead.emitted_thermal_radiation_mj_per_step;
    ...
    result.energy_balance_residual_mj_per_step =
        ...
        (result.net_radiation_mj_per_step +
            ...
            result.sensible_heat_mj_per_step +
            result.vapor_convective_heat_mj_per_step +
            inputs.live_canopy.precipitation_convective_heat_mj_per_step +
            inputs.standing_dead.precipitation_convective_heat_mj_per_step);
...
fn testComponent(scale: f64) ComponentFluxes {
    return .{
        ...
        .emitted_thermal_radiation_mj_per_step = 0.3 * scale,
...
test "live plus standing-dead aggregation preserves eight source assignments" {
    ...
    try std.testing.expectEqual(@as(f64, -3), result.sensible_heat_mj_per_step);
    try std.testing.expectEqual(@as(f64, 4.5), result.storage_heat_mj_per_step);
    try std.testing.expectEqual(@as(f64, -1.5), result.vapor_convective_heat_mj_per_step);
...
test "runtime cell-species entries have independent ownership" {
    ...
    try std.testing.expectEqual(@as(f64, 150), destination[1].net_radiation_mj_per_step);
}
