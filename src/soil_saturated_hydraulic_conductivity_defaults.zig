...
pub const Inputs = struct {
    ...
    existing_vertical_m2_h_mpa: f64,
    existing_lateral_m2_h_mpa: f64,
...
pub const Result = struct {
    vertical_m2_h_mpa: f64,
    lateral_m2_h_mpa: f64,
};

/// HOUR1 lines 2197--2221. Vertical and lateral missing-value gates remain
/// independent and repeat the source default calculation in their order.
pub fn compute(inputs: Inputs) !Result {
    try validate(inputs);
    var vertical = inputs.existing_vertical_m2_h_mpa;
    var lateral = inputs.existing_lateral_m2_h_mpa;
    ...
    return .{ .vertical_m2_h_mpa = vertical, .lateral_m2_h_mpa = lateral };
}
...
fn validate(inputs: Inputs) !void {
    ...
    inline for (.{
        inputs.existing_vertical_m2_h_mpa,
        inputs.existing_lateral_m2_h_mpa,
...
test "mineral defaults independently replace missing directions" {
    const result = try compute(.{
        ...
        .existing_vertical_m2_h_mpa = 0,
        .existing_lateral_m2_h_mpa = 9,
    ...
    try std.testing.expect(result.vertical_m2_h_mpa >= 0);
    try std.testing.expectEqual(@as(f64, 9), result.lateral_m2_h_mpa);
}
...
test "organic default applies bulk density power then macropore factor" {
    ...
    try std.testing.expectApproxEqAbs(
        expected,
        result.vertical_m2_h_mpa,
        1e-15,
    );
    try std.testing.expectEqual(
        result.vertical_m2_h_mpa,
        result.lateral_m2_h_mpa,
    );
}
