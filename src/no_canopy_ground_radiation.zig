...
pub fn apply(inputs: Inputs, diagnostics: SpeciesDiagnostics) !f64 {
    ...
    for (inputs.ground_sky_incidence_fraction) |incidence|
        ground_shortwave_mj_per_m2_h +=
            @abs(incidence) * inputs.diffuse_shortwave_mj_per_m2_h;
    ...
    return ground_shortwave_mj_h;
}

fn validate(inputs: Inputs, diagnostics: SpeciesDiagnostics) !usize {
    inline for (.{
        inputs.direct_shortwave_mj_per_m2_h,
        inputs.diffuse_shortwave_mj_per_m2_h,
        inputs.cell_area_m2,
...
test "no canopy ground radiation accumulates sky zones then resets species" {
    ...
    const ground_shortwave_mj_h = try apply(.{
    ...
    try std.testing.expectEqual(@as(f64, 20), ground_shortwave_mj_h);
...
test "dimension mismatch leaves species diagnostics unchanged" {
    ...
    try std.testing.expectError(
        ...
        apply(.{
            ...
            .diffuse_shortwave_mj_per_m2_h = 0,
            .cell_area_m2 = 1,
        }, .{
            .live_shortwave_mj_h = &live_sw,
