...
pub const SoilProfile = struct {
    ...
    field_capacity_potential_mpa: f64,
    wilting_point_potential_mpa: f64,
    wet_soil_albedo: f64,
...
pub fn parsePhysicalProfile(allocator: std.mem.Allocator, source: []const u8) !SoilProfile {
    ...
    result.field_capacity_potential_mpa = try number(f64, &header);
    result.wilting_point_potential_mpa = try number(f64, &header);
