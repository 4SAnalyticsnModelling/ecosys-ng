...
//! Genuchten retention curve in `soil_water_retention.zig`.
//!
...
pub fn pressureHeadMFromPotentialMpa(potential_mpa: f64) f64 {
    return potential_mpa / megapascal_per_meter_of_water_head;
}
...
pub fn legacyPotentialDerivativeMpaPerVolumeFraction(
    ...
    const log_saturation_potential = @log(-curve.saturation_water_potential_mpa);
    const log_field_potential = @log(-curve.field_capacity_water_potential_mpa);
    const log_wilting_potential = @log(-curve.wilting_point_water_potential_mpa);
    // HOUR1 `FCD` and `PSD`.
    ...
    const potential_mpa = if (below_wilting)
    ...
    const derivative = potential_mpa * log_log_slope / water_fraction;
