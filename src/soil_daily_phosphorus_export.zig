        ...
        for (0..model_grid.cell_count) |cell| {
            ...
            for (0..model_grid.active_soil_layer_count[cell]) |local_layer| {
                ...
                for (0..organic.substrate_count) |substrate| {
                    ...
                    outward_g_p += @max(0, -transport.boundary_net_flux_g[base + @intFromEnum(organic_transport.Component.dissolved_organic_phosphorus)]);
                }
            }
        ...
        for (0..model_grid.cell_count) |cell| {
            ...
            for (0..model_grid.active_soil_layer_count[cell]) |local_layer| {
                ...
                inline for (@typeInfo(solute_species.AqueousSpecies).@"enum".fields) |field| {
                    ...
                    if (solute_species.diffusivityClass(species) == .phosphate) outward_mol_p += @max(0, -boundary_net_flux_mol[base + field.value]);
                }
            }
