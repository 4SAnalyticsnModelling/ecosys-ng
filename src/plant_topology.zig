...
const plant_growth_stages = @import("plant_growth_stages.zig");
const plant_dormancy = @import("plant_dormancy.zig");
...
pub const RootAxisContext = struct {
    ...
    minimum_root_turgor_potential_mpa: f64,
};
...
test "HFUNC root topology uses deepest rooted layer and runtime trait threshold" {
    ...
    roots.turgor_water_potential_mpa[try roots.layerIndex(0, 0, 0)] = 0;
    roots.turgor_water_potential_mpa[try roots.layerIndex(0, 0, 2)] = 0.2;
    try std.testing.expectEqual(@as(usize, 1), try advanceRootAxes(.{ .roots = &roots, .canopy = &canopy, .growth_stages = &growth, .branch_development = &development, .active_by_plant = &.{true}, .root_branching_carbon_fraction = &.{0.1}, .minimum_root_turgor_potential_mpa = 0.1 }));
...
test "HFUNC shoot predicate publishes a coordinated dynamic branch" {
    ...
    roots.turgor_water_potential_mpa[try roots.layerIndex(0, 0, 0)] = 0.2;
    const states: RuntimeStates = .{ .canopy = &canopy, .growth_stages = &growth, .dormancy = &dormancy, .branch_development = &development };
    growth.branches[0].emergence_day = 90;
    dormancy.branches[0].accumulated_leafout_h = 8;
    try std.testing.expectEqual(@as(usize, 1), try advanceShootBranches(.{ .states = states, .roots = &roots, .controls = &controls, .active_by_plant = &.{true}, .emerged_by_plant = &.{true}, .day_of_year = 123, .execution_year = 2000, .minimum_root_turgor_potential_mpa = 0.1 }));
    ...
    try std.testing.expectEqual(@as(usize, 0), try advanceShootBranches(.{ .states = states, .roots = &roots, .controls = &controls, .active_by_plant = &.{false}, .emerged_by_plant = &.{true}, .day_of_year = 366, .execution_year = 1900, .minimum_root_turgor_potential_mpa = 0.1 }));
    try std.testing.expectError(
        error.InvalidPlantTopologyDate,
        advanceShootBranches(.{ .states = states, .roots = &roots, .controls = &controls, .active_by_plant = &.{false}, .emerged_by_plant = &.{true}, .day_of_year = 366, .execution_year = 1901, .minimum_root_turgor_potential_mpa = 0.1 }),
    );
