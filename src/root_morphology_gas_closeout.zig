...
fn validateInputs(inputs: Inputs, domain_layers: usize) !void {
    ...
    if (inputs.population_count < 0 or inputs.minimum_active_layer_thickness_m < 0 or inputs.root_elastic_modulus_mpa <= 0 or inputs.minimum_average_secondary_length_m < 0 or inputs.presence_threshold < 0) return error.InvalidRootMorphologyInput;
...
pub fn apply(state: State, workspace: State, inputs: Inputs) !void {
    ...
    for (0..inputs.biological_domain_count) |domain| for (inputs.planting_layer_index..inputs.deepest_rooted_layer_index + 1) |layer| {
        ...
        if (total_length_m > inputs.presence_threshold and total_carbon_g_c > inputs.presence_threshold and inputs.population_count > inputs.presence_threshold) {
            ...
            workspace.primary_radius_m[index] = @max(inputs.minimum_primary_radius_m_by_domain[domain], (1 + inputs.root_turgor_water_potential_mpa_by_domain_layer[index] / inputs.root_elastic_modulus_mpa) * inputs.base_primary_radius_m_by_domain[domain]);
            workspace.secondary_radius_m[index] = @max(inputs.minimum_secondary_radius_m_by_domain[domain], (1 + inputs.root_turgor_water_potential_mpa_by_domain_layer[index] / inputs.root_elastic_modulus_mpa) * inputs.base_secondary_radius_m_by_domain[domain]);
