...
pub const State = struct {
    ...
    sensible_boundary_numerator_m2_mj_per_k_iteration: f64,
};
...
pub const Inputs = struct {
    ...
    volumetric_air_heat_capacity_mj_per_m3_k: f64 = 1.25e-3,
};
...
pub fn refresh(state: *State, inputs: Inputs) !bool {
    ...
    if (inputs.horizontal_cell_area_m2 <= 0 or
        ...
        inputs.volumetric_air_heat_capacity_mj_per_m3_k <= 0)
    ...
    const next: State = .{
        ...
        .sensible_boundary_numerator_m2_mj_per_k_iteration = inputs.horizontal_cell_area_m2 *
            inputs.volumetric_air_heat_capacity_mj_per_m3_k *
            inputs.water_heat_iteration_fraction_h,
    };
    inline for (@typeInfo(State).@"struct".fields) |field|
...
test "first subhour reproduces exact PAREX and PARSX equations" {
    var state: State = .{
        ...
        .sensible_boundary_numerator_m2_mj_per_k_iteration = 0,
    };
    ...
    try std.testing.expectEqual(
        @as(f64, 0.00625),
        state.sensible_boundary_numerator_m2_mj_per_k_iteration,
    );
}
...
test "later nonlinear iteration retains accepted hourly numerators" {
    var state: State = .{
        ...
        .sensible_boundary_numerator_m2_mj_per_k_iteration = 8,
    };
    const before = state;
    try std.testing.expect(!try refresh(&state, .{
...
test "runtime air heat capacity controls sensible numerator only" {
    ...
    _ = try refresh(&state, .{
        ...
        .volumetric_air_heat_capacity_mj_per_m3_k = 0.002,
    });
    try std.testing.expectEqual(
        @as(f64, 2),
        state.latent_boundary_numerator_m2_h_per_iteration,
...
test "overflow leaves state unchanged" {
    ...
    try std.testing.expectError(
        ...
        refresh(&state, .{
            ...
            .volumetric_air_heat_capacity_mj_per_m3_k = 2,
        }),
    );
    try std.testing.expectEqualDeep(before, state);
}
