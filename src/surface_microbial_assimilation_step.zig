...
pub fn applyTile(context: *ApplyContext, range: compute.CellRange) !void {
    ...
    for (range.first..range.end) |cell| for (0..respiration.litter_complex_count) |complex| for (0..respiration.source_population_count) |population| {
        ...
        const response = context.growth_temperature_response[cell] * @exp((if (population == 2) @as(f64, 0.05) else 0.10) * context.matric_plus_osmotic_potential_mpa[cell]);
...
fn validate(context: ApplyContext, range: compute.CellRange) !void {
    ...
    if (range.first > range.end or range.end > context.result.cell_count or context.surface_organic.layer_count != context.result.cell_count or context.growth_temperature_response.len != context.result.cell_count or context.matric_plus_osmotic_potential_mpa.len != context.result.cell_count or context.microbial_nitrogen_to_carbon_g_n_per_g_c.len != ratio_count or context.microbial_phosphorus_to_carbon_g_p_per_g_c.len != ratio_count) return error.SurfaceMicrobialAssimilationDimensionMismatch;
...
test "surface assimilation reproduces CGOMS split and conserves elements" {
    ...
    var context: ApplyContext = .{ .result = &state, .surface_organic = &organic_state, .growth_temperature_response = &.{1}, .matric_plus_osmotic_potential_mpa = &.{0}, .microbial_nitrogen_to_carbon_g_n_per_g_c = &n, .microbial_phosphorus_to_carbon_g_p_per_g_c = &p, .labile_structural_fraction = 0.55, .transfer_rate_per_h = 0.25, .timestep_h = 1 };
