...
/// Direct translation of the NO3 band-dimension block in REDIST 9528--9556.
pub fn redistribute(context: Context, state: *State) !void {
    if (!finiteStruct(context) or !finiteStruct(state.*) or context.redistribution_fraction < 0 or
        ...
        return error.InvalidSoilNitrateBandGeometryInput;
    ...
    if (!finiteStruct(next)) return error.NonFiniteSoilNitrateBandGeometryResult;
    state.* = next;
}

test "REDIST nitrate band geometry preserves transfer and update order" {
...
test "REDIST nitrate band geometry obeys source and thickness gates" {
