...
pub const Inputs = struct {
    top_mineral_layer_sand_mass_megagrams: f64,
    top_mineral_layer_silt_mass_megagrams: f64,
    top_mineral_layer_clay_mass_megagrams: f64,
    surface_noncharcoal_organic_carbon_g_c: f64,
    initial_litter_water_m3_per_g_c: f64,
};

pub const State = struct {
    top_mineral_layer_mass_megagrams: *f64,
...
pub fn initialize(state: State, inputs: Inputs) !void {
    ...
    const top_mineral_layer_mass_megagrams = @max(
        0.0,
        inputs.top_mineral_layer_sand_mass_megagrams +
            inputs.top_mineral_layer_silt_mass_megagrams +
            inputs.top_mineral_layer_clay_mass_megagrams,
    );
    const surface_litter_water_m3 =
        inputs.initial_litter_water_m3_per_g_c *
        inputs.surface_noncharcoal_organic_carbon_g_c;
    if (!std.math.isFinite(top_mineral_layer_mass_megagrams) or
        !std.math.isFinite(surface_litter_water_m3))
        return error.NonFiniteSurfaceTopsoilInterfaceResult;

    state.top_mineral_layer_mass_megagrams.* = top_mineral_layer_mass_megagrams;
...
test "STARTS final block refreshes mineral mass and noncharcoal litter water" {
    var mineral_mass_megagrams: f64 = 0.0;
    var litter_water_m3: f64 = 0.0;
    try initialize(.{
        .top_mineral_layer_mass_megagrams = &mineral_mass_megagrams,
        .surface_litter_water_m3 = &litter_water_m3,
    }, .{
        .top_mineral_layer_sand_mass_megagrams = 2.0,
        .top_mineral_layer_silt_mass_megagrams = 3.0,
        .top_mineral_layer_clay_mass_megagrams = 4.0,
        .surface_noncharcoal_organic_carbon_g_c = 100.0,
        .initial_litter_water_m3_per_g_c = 8.0e-6,
    });
    try std.testing.expectEqual(@as(f64, 9.0), mineral_mass_megagrams);
...
test "overflow fails before either state field mutates" {
    var mineral_mass_megagrams: f64 = 7.0;
    ...
    try std.testing.expectError(
        ...
        initialize(.{
            ...
            .top_mineral_layer_sand_mass_megagrams = std.math.floatMax(f64),
            .top_mineral_layer_silt_mass_megagrams = std.math.floatMax(f64),
            .top_mineral_layer_clay_mass_megagrams = 0.0,
            .surface_noncharcoal_organic_carbon_g_c = 1.0,
            .initial_litter_water_m3_per_g_c = 1.0,
        }),
    );
    try std.testing.expectEqual(@as(f64, 7.0), mineral_mass_megagrams);
