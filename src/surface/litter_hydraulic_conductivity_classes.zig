...
pub const Inputs = struct {
    ...
    saturated_conductivity_m2_h_megapascal: f64,
...
/// `hour1.f` lines 1986--2034. Runtime hydraulic classes replace source JK
...
pub fn compute(inputs: Inputs, outputs: Outputs) !?AirEntry {
    ...
    for (0..class_count) |class| {
        ...
        const potential = if (water_content < inputs.wilting_point_m3_m3)
            @max(
                inputs.minimum_potential_megapascal,
            ...
            inputs.saturation_potential_megapascal;
        outputs.water_potential_megapascal[class] = potential;
        denominator_sum +=
    ...
    for (0..class_count) |class| {
        ...
        for (class..class_count) |summed_class| {
            ...
            const potential = outputs.water_potential_megapascal[summed_class];
            numerator_sum +=
                (2.0 * x_m + 1.0 - 2.0 * x_k) /
                (potential * potential);
        }
        outputs.vertical_conductivity_m2_h_megapascal[class] =
            inputs.saturated_conductivity_m2_h_megapascal * conductivity_shape *
            numerator_sum / denominator_sum;
        outputs.lateral_x_conductivity_m2_h_megapascal[class] = 0.0;
        outputs.lateral_y_conductivity_m2_h_megapascal[class] = 0.0;
        if (class > 0 and
            outputs.vertical_conductivity_m2_h_megapascal[class] <
                inputs.air_entry_conductivity_fraction *
                    inputs.saturated_conductivity_m2_h_megapascal and
            outputs.vertical_conductivity_m2_h_megapascal[class - 1] >=
                inputs.air_entry_conductivity_fraction *
                    inputs.saturated_conductivity_m2_h_megapascal)
            air_entry = .{
...
fn validate(inputs: Inputs, outputs: Outputs) !usize {
    ...
    if (count == 0 or outputs.water_potential_megapascal.len != count or
        outputs.vertical_conductivity_m2_h_megapascal.len != count or
        outputs.lateral_x_conductivity_m2_h_megapascal.len != count or
        outputs.lateral_y_conductivity_m2_h_megapascal.len != count)
    ...
    if (inputs.porosity_m3_m3 <= 0 or inputs.field_capacity_m3_m3 <= 0 or
        ...
        inputs.saturation_potential_megapascal >= 0 or
        inputs.saturated_conductivity_m2_h_megapascal < 0 or
...
test "runtime hydraulic classes produce finite vertical and zero lateral conductivity" {
    ...
    _ = try compute(.{
        ...
        .saturated_conductivity_m2_h_megapascal = 0.01,
        .air_entry_conductivity_fraction = 0.1,
    }, .{
        .water_content_m3_m3 = &water,
        .water_potential_megapascal = &potential,
