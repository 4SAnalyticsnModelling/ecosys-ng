...
pub const Thresholds = struct {
    ...
    area_normalized_primary_m2_inv: f64,
...
/// Translates STARTQ lines 886-888 for one plant species.
pub fn calculate(inputs: Inputs) CalculationError!Thresholds {
    ...
    const area_normalized_primary_m2_inv =
        inputs.primary_threshold_per_plant * inputs.plant_population_count /
        inputs.cell_area_m2;
    ...
    const result = Thresholds{
        ...
        .area_normalized_primary_m2_inv = area_normalized_primary_m2_inv,
...
test "thresholds scale with population and cell area in STARTQ order" {
    ...
    try std.testing.expectEqual(@as(f64, 4.0e-12), result.area_normalized_primary_m2_inv);
