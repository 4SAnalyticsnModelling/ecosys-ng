...
const hydraulic_conductivity = @import("soil_hydraulic_conductivity.zig");
const water_boundary = @import("soil_water_boundary.zig");
...
pub const Face = struct {
    ...
    macropore_hydraulic_conductance_m_per_h_mpa: f64,
};
...
pub const FaceGeometry = struct {
    ...
    wetting_depth_factor: []const f64,
    macropore_hydraulic_conductance_m_per_h_mpa: []const f64,
};
...
pub const Properties = struct {
    ...
    matrix_air_entry_water_fraction: []const f64,
    ...
    gravitational_potential_mpa: []const f64,
    osmotic_potential_mpa: []const f64,
    matrix_hydraulic_conductivity_m2_per_h_mpa: []const f64,
    ...
    boundary_macropore_hydraulic_conductivity_m2_per_h_mpa: []const f64 = &.{},
...
pub fn solveAndBindTransportFaces(allocator: std.mem.Allocator, grid: *grid_module.GridState, hydrology: *transport_hydrology.State, shared_faces: *transport_hydrology.SoilFaces, geometry: FaceGeometry, properties: Properties, options: Options) !Result {
    ...
    if (shared_faces.macropore_faces.len != count or shared_faces.direction_axis.len != count or geometry.source_path_length_m.len != count or geometry.destination_path_length_m.len != count or geometry.face_area_m2.len != count or geometry.wetting_depth_factor.len != count or geometry.macropore_hydraulic_conductance_m_per_h_mpa.len != count) return error.SoilWaterFaceGeometryDimensionMismatch;
    ...
    for (faces, 0..) |*face, index| {
        ...
        face.* = .{ .source_cell = shared_faces.micropore_faces[index].first_cell, .destination_cell = shared_faces.micropore_faces[index].second_cell, .axis = axis, .direction = if (axis == .z) .vertical else .horizontal, .source_path_length_m = geometry.source_path_length_m[index], .destination_path_length_m = geometry.destination_path_length_m[index], .face_area_m2 = geometry.face_area_m2[index], .wetting_depth_factor = geometry.wetting_depth_factor[index], .macropore_hydraulic_conductance_m_per_h_mpa = geometry.macropore_hydraulic_conductance_m_per_h_mpa[index] };
    }
...
fn residualAt(grid: *const grid_module.GridState, faces: []const Face, properties: Properties, base: []const f64, trial: []const f64, target: []f64, residual: []f64, scratch: []f64, micro_fluxes: []f64, macro_fluxes: []f64) !void {
    ...
    for (faces, 0..) |face, face_index| {
        ...
        const source_total = source_matric + properties.gravitational_potential_mpa[source] + properties.osmotic_potential_multiplier * properties.osmotic_potential_mpa[source];
        const destination_total = destination_matric + properties.gravitational_potential_mpa[destination] + properties.osmotic_potential_multiplier * properties.osmotic_potential_mpa[destination];
        ...
        const matrix = try water_flux.calculateMatrixFaceFlux(.{ .direction = face.direction, .source_water_m3 = source_water, .destination_water_m3 = destination_water, .source_air_m3 = source_air, .destination_air_m3 = destination_air, .source_micropore_volume_m3 = properties.matrix_bulk_volume_m3[source], .destination_micropore_volume_m3 = properties.matrix_bulk_volume_m3[destination], .source_water_fraction = source_fraction, .destination_water_fraction = destination_fraction, .source_air_entry_water_fraction = properties.matrix_air_entry_water_fraction[source], .destination_air_entry_water_fraction = properties.matrix_air_entry_water_fraction[destination], .source_total_water_potential_mpa = source_total, .destination_total_water_potential_mpa = destination_total, .source_hydraulic_conductivity_m2_per_h_mpa = source_conductivity, .destination_hydraulic_conductivity_m2_per_h_mpa = destination_conductivity, .source_path_length_m = face.source_path_length_m, .destination_path_length_m = face.destination_path_length_m, .face_area_m2 = face.face_area_m2, .wetting_depth_factor = 0, .time_fraction = properties.nonlinear_time_fraction });
        ...
        if (grid.macropore_pore_capacity_m3[source] > 0 and grid.macropore_pore_capacity_m3[destination] > 0) {
            ...
            const calculated_macro_flux_m3 = richards: {
                ...
                const flux = try water_flux.calculateMatrixFaceFlux(.{
                    ...
                    .source_air_entry_water_fraction = 0,
                    .destination_air_entry_water_fraction = 0,
                    .source_total_water_potential_mpa = source_head_m * properties.gravitational_water_potential_mpa_per_m + properties.gravitational_potential_mpa[source],
                    .destination_total_water_potential_mpa = destination_head_m * properties.gravitational_water_potential_mpa_per_m + properties.gravitational_potential_mpa[destination],
                    .source_hydraulic_conductivity_m2_per_h_mpa = source_macropore_conductivity,
                    .destination_hydraulic_conductivity_m2_per_h_mpa = destination_macropore_conductivity,
    ...
    if (properties.boundary_topology) |topology| for (topology.faces) |boundary_face| {
        ...
        if (boundary_face.is_lower_boundary) {
            ...
            const matric_mpa = try matricPotentialMpaAt(properties, layer, matrix_fraction);
            const base_fraction = base[layer] / properties.matrix_bulk_volume_m3[layer];
            const base_matric_mpa = try matricPotentialMpaAt(properties, layer, base_fraction);
            const current_layer_discharge_candidate = base_matric_mpa > grid.matric_potential_mpa[layer];
            ...
            for ([_]bool{ false, true }) |artificial| {
                ...
                if (matrix_discharge_enabled and midpoint_m < external_depth_m) {
                    ...
                    while (lower_local < grid.active_soil_layer_count[boundary_face.cell_index]) : (lower_local += 1) {
                        ...
                        const lower_matric_mpa = try matricPotentialMpaAt(properties, lower, lower_fraction);
                        if (lower_matric_mpa <= grid.matric_potential_mpa[lower] or properties.boundary_layer_midpoint_depth_m[lower] > topology.active_layer_depth_m[boundary_face.cell_index]) {
                ...
                if (midpoint_m < external_depth_m and matrix_discharge_enabled) {
                    const oriented = try water_boundary.matrixDischarge(.{ .direction_sign = boundary_face.direction_sign, .slope_sine = boundary_face.slope_sine, .directional_layer_width_m = boundary_face.directional_layer_width_m, .water_table_slope = table_slope, .matric_potential_mpa = matric_mpa, .saturation_water_potential_mpa = saturationMatricPotentialMpa(properties, layer), .layer_midpoint_depth_m = midpoint_m, .external_water_table_depth_m = external_depth_m, .internal_water_table_depth_m = topology.internal_water_table_depth_m[boundary_face.cell_index], .hydraulic_conductivity_m2_per_h_mpa = try conductivityAt(properties, layer, axis, matrix_fraction, grid.matrix_ice_water_m3[layer]), .face_area_m2 = face_area_m2, .fraction_face_below_water_table = fraction_below, .recharge_frequency_divisor = distance_m, .recharge_time_multiplier = exchange_fraction, .time_fraction = properties.nonlinear_time_fraction, .source_temperature_k = grid.soil_temperature_k[layer] }, artificial);
                ...
                if (macro_capacity > 0 and base_macro_water_depth_m < external_depth_m and macropore_water > 0) {
                    ...
                    const oriented = try water_boundary.recharge(.{ .direction_sign = boundary_face.direction_sign, .slope_sine = boundary_face.slope_sine, .directional_layer_width_m = boundary_face.directional_layer_width_m, .water_table_slope = table_slope, .matric_potential_mpa = 0, .layer_or_macropore_water_depth_m = macro_water_depth_m, .external_water_table_depth_m = external_depth_m, .hydraulic_conductivity_m2_per_h_mpa = properties.boundary_macropore_hydraulic_conductivity_m2_per_h_mpa[layer] * macroporeFrozenHydraulicImpedance(properties, grid, layer), .face_area_m2 = face_area_m2, .fraction_face_below_water_table = fraction_below, .recharge_frequency_divisor = @max(1, distance_m), .recharge_time_multiplier = exchange_fraction, .time_fraction = properties.nonlinear_time_fraction, .available_air_volume_m3 = @max(0, macro_capacity - grid.macropore_ice_water_m3[layer] - macropore_water), .source_temperature_k = grid.soil_temperature_k[layer] }, true);
...
pub fn conductivityAt(properties: Properties, cell: usize, axis: usize, water_fraction: f64, ice_water_equivalent_m3: f64) !f64 {
    ...
    const bounded_water_content = std.math.clamp(water_fraction, parameters.residual_water_content_m3_per_m3, parameters.saturated_water_content_m3_per_m3);
    ...
    const multiplier = if (properties.rainfall_conductivity_multiplier.len == 0) 1 else properties.rainfall_conductivity_multiplier[cell];
    const ice_content_m3_per_m3 = ice_water_equivalent_m3 / properties.matrix_bulk_volume_m3[cell];
    return conductivity_m_per_h / properties.gravitational_water_potential_mpa_per_m *
        multiplier * frozenHydraulicImpedance(properties.frozen_hydraulic_impedance_exponent, ice_content_m3_per_m3, parameters);
}
...
pub fn interpolatedConductivity(table: []const f64, class_count: usize, porosity: f64, cell: usize, axis: usize, water_fraction: f64) !f64 {
    if (class_count == 0 or axis >= 3 or !std.math.isFinite(porosity) or porosity <= 0 or table.len % (3 * class_count) != 0 or cell >= table.len / (3 * class_count)) return error.SoilWaterSolverDimensionMismatch;
    const lower_class = try hydraulic_conductivity.classIndex(class_count, water_fraction, porosity);
    const base_index = (cell * 3 + axis) * class_count;
    if (lower_class + 1 >= class_count) return table[base_index + lower_class];
    // Interpolate the exact HOUR1 HCND table rather than introducing a hard
    // discontinuity at every class boundary. Table nodes and end points remain
    // identical to Fortran, while the nonlinear residual becomes continuous.
    const dry_coordinate = @as(f64, @floatFromInt(class_count)) * std.math.clamp((porosity - water_fraction) / porosity, 0, 1);
    const fraction = std.math.clamp(dry_coordinate - @as(f64, @floatFromInt(lower_class)), 0, 1);
    const wetter = table[base_index + lower_class];
    const drier = table[base_index + lower_class + 1];
    return wetter + fraction * (drier - wetter);
}
...
fn commit(grid: *grid_module.GridState, properties: Properties, state: []const f64) !void {
    ...
    for (0..cells) |cell| {
        ...
        grid.matric_potential_mpa[cell] = try matricPotentialMpaAt(
            properties,
            cell,
...
fn validateInputs(grid: *const grid_module.GridState, faces: []const Face, properties: Properties, micro_flux: []const f64, macro_flux: []const f64, options: Options) !void {
    const cells = grid.layer_count;
    if (properties.matrix_bulk_volume_m3.len != cells or properties.matrix_air_entry_water_fraction.len != cells or properties.retention_curve.len != cells or properties.mualem_van_genuchten_parameters.len != cells or properties.macropore_mualem_van_genuchten_parameters.len != cells or properties.dual_domain_exchange_enabled.len != cells or properties.macropore_spacing_m.len != cells or properties.macropore_radius_m.len != cells or properties.gravitational_potential_mpa.len != cells or properties.osmotic_potential_mpa.len != cells or properties.vertical_thickness_m.len != cells or (properties.rainfall_conductivity_multiplier.len != 0 and properties.rainfall_conductivity_multiplier.len != cells) or (properties.matrix_external_source_m3_per_step.len != 0 and properties.matrix_external_source_m3_per_step.len != cells) or micro_flux.len != faces.len or macro_flux.len != faces.len) return error.SoilWaterSolverDimensionMismatch;
    ...
    for (0..cells) |cell| {
        if (!std.math.isFinite(properties.matrix_bulk_volume_m3[cell]) or properties.matrix_bulk_volume_m3[cell] <= 0 or !std.math.isFinite(properties.matrix_air_entry_water_fraction[cell]) or properties.matrix_air_entry_water_fraction[cell] < 0 or !std.math.isFinite(properties.vertical_thickness_m[cell]) or properties.vertical_thickness_m[cell] <= 0 or grid.matrix_liquid_water_m3[cell] < 0 or grid.macropore_liquid_water_m3[cell] < 0 or grid.matrix_liquid_water_m3[cell] + grid.matrix_ice_water_m3[cell] > grid.matrix_pore_capacity_m3[cell] + poreCapacityRoundoffToleranceM3(grid.matrix_pore_capacity_m3[cell]) or grid.macropore_liquid_water_m3[cell] + grid.macropore_ice_water_m3[cell] > grid.macropore_pore_capacity_m3[cell] + poreCapacityRoundoffToleranceM3(grid.macropore_pore_capacity_m3[cell])) {
            if (!builtin.is_test) std.log.err("invalid soil water input: layer_cell={d} bulk_m3={e} air_entry_fraction={e} thickness_m={e} matrix_water_m3={e} matrix_ice_m3={e} matrix_capacity_m3={e} macropore_water_m3={e} macropore_ice_m3={e} macropore_capacity_m3={e}", .{ cell, properties.matrix_bulk_volume_m3[cell], properties.matrix_air_entry_water_fraction[cell], properties.vertical_thickness_m[cell], grid.matrix_liquid_water_m3[cell], grid.matrix_ice_water_m3[cell], grid.matrix_pore_capacity_m3[cell], grid.macropore_liquid_water_m3[cell], grid.macropore_ice_water_m3[cell], grid.macropore_pore_capacity_m3[cell] });
            return error.InvalidSoilWaterSolverInput;
        }
    }
    for (faces) |face| if (face.source_cell >= cells or face.destination_cell >= cells or face.source_cell == face.destination_cell or !std.math.isFinite(face.source_path_length_m) or face.source_path_length_m <= 0 or !std.math.isFinite(face.destination_path_length_m) or face.destination_path_length_m <= 0 or !std.math.isFinite(face.face_area_m2) or face.face_area_m2 < 0 or !std.math.isFinite(face.wetting_depth_factor) or face.wetting_depth_factor < 0 or !std.math.isFinite(face.macropore_hydraulic_conductance_m_per_h_mpa) or face.macropore_hydraulic_conductance_m_per_h_mpa < 0) return error.InvalidSoilWaterFace;
}
...
test "runtime NPH hybrid water solve exits early and conserves both pore domains" {
    ...
    const result = try solve(std.testing.allocator, &grid, &.{.{ .source_cell = 0, .destination_cell = 1, .direction = .horizontal, .source_path_length_m = 1, .destination_path_length_m = 1, .face_area_m2 = 1, .wetting_depth_factor = 0, .macropore_hydraulic_conductance_m_per_h_mpa = 0.01 }}, .{ .matrix_bulk_volume_m3 = &bulk, .matrix_air_entry_water_fraction = &air_entry, .retention_curve = &curves, .mualem_van_genuchten_parameters = &matrix_parameters, .macropore_mualem_van_genuchten_parameters = &macropore_parameters, .macropore_spacing_m = &macropore_spacing_m, .macropore_radius_m = &macropore_radius_m, .dual_domain_exchange_enabled = &dual_domain_disabled, .gravitational_potential_mpa = &gravity, .osmotic_potential_mpa = &osmotic, .matrix_hydraulic_conductivity_m2_per_h_mpa = &conductivity, .vertical_thickness_m = &thickness, .osmotic_potential_multiplier = 1 }, &micro_flux, &macro_flux, .{ .max_iterations = 40 });
...
test "subsurface irrigation is an external source in the Richards residual" {
    ...
    const properties: Properties = .{
        ...
        .matrix_air_entry_water_fraction = &.{0},
        ...
        .gravitational_potential_mpa = &.{0},
        .osmotic_potential_mpa = &.{0},
        .matrix_hydraulic_conductivity_m2_per_h_mpa = &.{ 0, 0, 0 },
...
test "simultaneous Richards residual is independent of face ordering" {
    ...
    const properties: Properties = .{
        ...
        .matrix_air_entry_water_fraction = &zero,
        ...
        .gravitational_potential_mpa = &zero,
        .osmotic_potential_mpa = &zero,
        .matrix_hydraulic_conductivity_m2_per_h_mpa = &conductivity,
    ...
    const face_01: Face = .{
        ...
        .face_area_m2 = 1,
        .wetting_depth_factor = 0,
        .macropore_hydraulic_conductance_m_per_h_mpa = 0,
    };
    const face_12: Face = .{
        .source_cell = 1,
        .destination_cell = 2,
...
test "lower drainage and lateral water-table recharge converge inside NPH residual" {
    ...
    var site = try @import("site.zig").parse(std.testing.allocator, @import("test_fixtures.zig").site_source, 1, 1);
    defer site.deinit();
    var topography = try @import("topography.zig").parse(std.testing.allocator, "1 1 1 1 0 0 0 0\nsoil\n");
    defer topography.deinit();
    var terrain = try @import("terrain_hydrology.zig").State.initMapped(std.testing.allocator, topography, &.{0}, &.{1}, &.{1}, 1, 1);
    ...
    _ = try solve(std.testing.allocator, &grid, &.{}, .{ .matrix_bulk_volume_m3 = &one, .matrix_air_entry_water_fraction = &zero, .retention_curve = &curves, .mualem_van_genuchten_parameters = &matrix_parameters, .macropore_mualem_van_genuchten_parameters = &macropore_parameters, .macropore_spacing_m = &macropore_spacing_m, .macropore_radius_m = &macropore_radius_m, .dual_domain_exchange_enabled = &dual_domain_disabled, .gravitational_potential_mpa = &zero, .osmotic_potential_mpa = &zero, .matrix_hydraulic_conductivity_m2_per_h_mpa = &conductivity, .vertical_thickness_m = &thickness, .osmotic_potential_multiplier = 1, .boundary_topology = &topology, .boundary_face_area_m2 = &one, .boundary_macropore_hydraulic_conductivity_m2_per_h_mpa = &macro_conductivity, .boundary_layer_volume_m3 = &one, .boundary_layer_midpoint_depth_m = &thickness, .boundary_layer_bottom_depth_m = &one }, &.{}, &.{}, .{ .max_iterations = 20 });
    ...
    grid.macropore_liquid_water_m3[0] = 0.05;
    grid.matric_potential_mpa[0] = -1.0e300;
    var artificial_drainage = [_]f64{0};
    const bottom = [_]f64{0.3};
    _ = try solve(std.testing.allocator, &grid, &.{}, .{ .matrix_bulk_volume_m3 = &one, .matrix_air_entry_water_fraction = &zero, .retention_curve = &curves, .mualem_van_genuchten_parameters = &matrix_parameters, .macropore_mualem_van_genuchten_parameters = &macropore_parameters, .macropore_spacing_m = &macropore_spacing_m, .macropore_radius_m = &macropore_radius_m, .dual_domain_exchange_enabled = &dual_domain_disabled, .gravitational_potential_mpa = &zero, .osmotic_potential_mpa = &zero, .matrix_hydraulic_conductivity_m2_per_h_mpa = &conductivity, .vertical_thickness_m = &thickness, .osmotic_potential_multiplier = 1, .boundary_topology = &topology, .boundary_face_area_m2 = &one, .boundary_macropore_hydraulic_conductivity_m2_per_h_mpa = &macro_conductivity, .boundary_layer_volume_m3 = &one, .boundary_layer_midpoint_depth_m = &thickness, .boundary_layer_bottom_depth_m = &bottom, .artificial_drainage_outflow_m3_per_step = &artificial_drainage }, &.{}, &.{}, .{ .max_iterations = 20 });
...
test "rejected water solve leaves grid and published flux unchanged" {
    ...
    try std.testing.expectError(error.InvalidSoilWaterSolverInput, solve(std.testing.allocator, &grid, &.{.{ .source_cell = 0, .destination_cell = 1, .direction = .horizontal, .source_path_length_m = 1, .destination_path_length_m = 1, .face_area_m2 = 1, .wetting_depth_factor = 0, .macropore_hydraulic_conductance_m_per_h_mpa = 0 }}, .{ .matrix_bulk_volume_m3 = &pair, .matrix_air_entry_water_fraction = &zeros, .retention_curve = &curves, .mualem_van_genuchten_parameters = &matrix_parameters, .macropore_mualem_van_genuchten_parameters = &macropore_parameters, .macropore_spacing_m = &macropore_spacing_m, .macropore_radius_m = &macropore_radius_m, .dual_domain_exchange_enabled = &dual_domain_disabled, .gravitational_potential_mpa = &zeros, .osmotic_potential_mpa = &zeros, .matrix_hydraulic_conductivity_m2_per_h_mpa = &conductivity, .vertical_thickness_m = &thickness, .osmotic_potential_multiplier = 1 }, &micro_flux, &macro_flux, .{ .max_iterations = 1, .absolute_tolerance_m3 = 1.0e-30, .relative_tolerance = 1.0e-30 }));
...
test "converged WATSUB flux binds to shared hydrology and solute faces" {
    ...
    const thickness = [_]f64{ 0.1, 0.1 };
    const one = [_]f64{1};
    const no_enhancement = [_]f64{0};
    const no_macro = [_]f64{0};
    _ = try solveAndBindTransportFaces(std.testing.allocator, &grid, &hydrology, &shared_faces, .{ .source_path_length_m = &one, .destination_path_length_m = &one, .face_area_m2 = &one, .wetting_depth_factor = &no_enhancement, .macropore_hydraulic_conductance_m_per_h_mpa = &no_macro }, .{ .matrix_bulk_volume_m3 = &bulk, .matrix_air_entry_water_fraction = &air_entry, .retention_curve = &curves, .mualem_van_genuchten_parameters = &matrix_parameters, .macropore_mualem_van_genuchten_parameters = &macropore_parameters, .macropore_spacing_m = &macropore_spacing_m, .macropore_radius_m = &macropore_radius_m, .dual_domain_exchange_enabled = &dual_domain_disabled, .gravitational_potential_mpa = &zeros, .osmotic_potential_mpa = &zeros, .matrix_hydraulic_conductivity_m2_per_h_mpa = &conductivity, .vertical_thickness_m = &thickness, .osmotic_potential_multiplier = 1 }, .{ .max_iterations = 40 });
...
test "original Mualem conductivity is isotropic before runtime directional geometry" {
    ...
    const properties: Properties = .{ .matrix_bulk_volume_m3 = &scalar, .matrix_air_entry_water_fraction = &scalar, .retention_curve = &curves, .mualem_van_genuchten_parameters = &parameters, .gravitational_potential_mpa = &scalar, .osmotic_potential_mpa = &scalar, .matrix_hydraulic_conductivity_m2_per_h_mpa = &compatibility_table, .vertical_thickness_m = &scalar, .osmotic_potential_multiplier = 1 };
...
test "rainfall damage scales Mualem conductivity without changing parameters" {
    ...
    const unscaled: Properties = .{ .matrix_bulk_volume_m3 = &scalar, .matrix_air_entry_water_fraction = &scalar, .retention_curve = &curves, .mualem_van_genuchten_parameters = &parameters, .gravitational_potential_mpa = &scalar, .osmotic_potential_mpa = &scalar, .matrix_hydraulic_conductivity_m2_per_h_mpa = &compatibility_table, .vertical_thickness_m = &scalar, .osmotic_potential_multiplier = 1 };
    var scaled = unscaled;
...
test "Richards residual uses runtime Mualem van Genuchten head and conductivity" {
    ...
    const properties: Properties = .{
        .matrix_bulk_volume_m3 = &scalar,
        .matrix_air_entry_water_fraction = &scalar,
        .retention_curve = &curve,
        .mualem_van_genuchten_parameters = &parameter_slice,
        .gravitational_potential_mpa = &scalar,
        .osmotic_potential_mpa = &scalar,
        .matrix_hydraulic_conductivity_m2_per_h_mpa = &compatibility_table,
        .vertical_thickness_m = &scalar,
        .osmotic_potential_multiplier = 1,
    };
    try std.testing.expectApproxEqAbs(
...
test "Dall'Amico ice impedance scales runtime Mualem conductivity" {
    ...
    const unfrozen: Properties = .{ .matrix_bulk_volume_m3 = &scalar, .matrix_air_entry_water_fraction = &scalar, .retention_curve = &curves, .mualem_van_genuchten_parameters = &parameters_slice, .gravitational_potential_mpa = &zero, .osmotic_potential_mpa = &zero, .matrix_hydraulic_conductivity_m2_per_h_mpa = &conductivity, .vertical_thickness_m = &scalar, .osmotic_potential_multiplier = 1, .frozen_hydraulic_impedance_exponent = 7 };
    const water_content_m3_per_m3 = 0.25;
...
test "macropore Richards flow uses runtime shape and conserves pore water" {
    ...
    _ = try solve(
        std.testing.allocator,
        &grid,
        &.{.{ .source_cell = 0, .destination_cell = 1, .direction = .horizontal, .source_path_length_m = 1, .destination_path_length_m = 1, .face_area_m2 = 1, .wetting_depth_factor = 0, .macropore_hydraulic_conductance_m_per_h_mpa = 999 }},
        .{ .matrix_bulk_volume_m3 = &pair, .matrix_air_entry_water_fraction = &zeros, .retention_curve = &curves, .mualem_van_genuchten_parameters = &matrix_parameters, .macropore_mualem_van_genuchten_parameters = &macro_parameters, .macropore_spacing_m = &spacing, .macropore_radius_m = &radius, .dual_domain_exchange_enabled = &exchange_disabled, .gravitational_potential_mpa = &zeros, .osmotic_potential_mpa = &zeros, .matrix_hydraulic_conductivity_m2_per_h_mpa = &conductivity, .vertical_thickness_m = &pair, .osmotic_potential_multiplier = 1 },
        &matrix_flux,
        &macropore_flux,
        .{ .max_iterations = 40 },
    );
