...
pub const Result = struct {
    ...
    solid_heat_capacity_mj_k: f64,
    weighted_thermal_conductivity_m_mj_h_k: f64,
...
pub fn calculate(inputs: Inputs) CalculationError!Result {
    ...
    var particle_density_mg_m3: f64 = 0.0;
    var organic_volume_fraction: f64 = 0.0;
    var mineral_volume_fraction: f64 = 0.0;
    var sand_volume_fraction: f64 = 0.0;
    var solid_heat_capacity_mj_k = inputs.existing_solid_heat_capacity_mj_k;
    var weighted_thermal_conductivity: f64 = 0.0;
    var thermal_conductivity_weight: f64 = 0.0;
    var porosity_m3_m3: f64 = 1.0;

    if (inputs.soil_mass_mg > inputs.positive_mass_threshold_mg) {
        organic_matter_fraction = @max(
            0.0,
            @min(1.0, 1.82e-6 * inputs.organic_carbon_concentration_g_mg),
        );
        particle_density_mg_m3 =
            1.30 * organic_matter_fraction + 2.66 * (1.0 - organic_matter_fraction);
        const bulk_to_particle_ratio = inputs.bulk_density_mg_m3 / particle_density_mg_m3;
        const composition_total = organic_matter_fraction +
            inputs.silt_concentration_g_mg +
            inputs.clay_concentration_g_mg +
            inputs.sand_concentration_g_mg;
        ...
        mineral_volume_fraction =
            (inputs.silt_concentration_g_mg + inputs.clay_concentration_g_mg) *
            normalization * bulk_to_particle_ratio;
        sand_volume_fraction =
            inputs.sand_concentration_g_mg * normalization * bulk_to_particle_ratio;
    ...
    const result = Result{
        ...
        .solid_heat_capacity_mj_k = solid_heat_capacity_mj_k,
        .weighted_thermal_conductivity_m_mj_h_k = weighted_thermal_conductivity,
...
test "positive soil mass calculates thermal properties and retains VHCM" {
    const result = try calculate(.{
        .soil_mass_mg = 100.0,
        .organic_carbon_concentration_g_mg = 100_000.0,
        .silt_concentration_g_mg = 0.3,
        .clay_concentration_g_mg = 0.2,
        .sand_concentration_g_mg = 0.4,
        .bulk_density_mg_m3 = 1.2,
        .micropore_volume_fraction = 0.8,
        .rock_volume_fraction = 0.1,
        .existing_porosity_m3_m3 = 0.6,
        .existing_solid_heat_capacity_mj_k = 12.0,
        .effective_soil_volume_m3 = 80.0,
        .total_layer_volume_m3 = 100.0,
        .macropore_volume_fraction = 0.05,
        .positive_mass_threshold_mg = 0.0,
        .layer_position = .surface_mineral,
    });
    try std.testing.expectEqual(@as(f64, 12.0), result.solid_heat_capacity_mj_k);
...
test "zero soil mass clears thermal state and uses unit porosity" {
    const result = try calculate(.{
        .soil_mass_mg = 0.0,
        .organic_carbon_concentration_g_mg = 0.0,
        .silt_concentration_g_mg = 0.0,
        .clay_concentration_g_mg = 0.0,
        .sand_concentration_g_mg = 0.0,
        .bulk_density_mg_m3 = 0.0,
        .micropore_volume_fraction = 0.0,
        .rock_volume_fraction = 0.0,
        .existing_porosity_m3_m3 = 0.0,
        .existing_solid_heat_capacity_mj_k = 9.0,
    ...
    try std.testing.expectEqual(@as(f64, 0.0), result.solid_heat_capacity_mj_k);
