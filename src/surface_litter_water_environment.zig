...
const geometry = @import("surface_litter_geometry_step.zig");
...
pub const State = struct {
    ...
    matric_water_potential_mpa: []f64,
    osmotic_water_potential_mpa: []f64,
    matric_plus_osmotic_water_potential_mpa: []f64,
...
pub const ApplyContext = struct {
    ...
    field_capacity_potential_mpa: []const f64,
    wilting_point_potential_mpa: []const f64,
...
pub fn applyTile(context: *ApplyContext, range: compute.CellRange) !void {
    ...
    for (range.first..range.end) |cell| {
        ...
        const inactive = try @import("surface_litter_geometry.zig").waterFractionAtPotentialBelowWilting(field_capacity, wilting_point, context.field_capacity_potential_mpa[cell], context.wilting_point_potential_mpa[cell], context.parameters.hygroscopic_water_potential_mpa);
        ...
        const curve: retention.ResolvedCurve = .{ .porosity_fraction = porosity, .curve = .{
            ...
            .saturation_water_potential_mpa = context.parameters.saturation_water_potential_mpa,
            .field_capacity_water_potential_mpa = context.field_capacity_potential_mpa[cell],
            .wilting_point_water_potential_mpa = context.wilting_point_potential_mpa[cell],
            .minimum_water_potential_mpa = context.parameters.minimum_water_potential_mpa,
        ...
        const matric = if (retained_water_fraction > 0) try curve.waterPotentialMpa(retained_water_fraction) else context.parameters.minimum_water_potential_mpa;
        ...
        context.result.matric_water_potential_mpa[cell] = matric;
        context.result.osmotic_water_potential_mpa[cell] = osmotic;
        context.result.matric_plus_osmotic_water_potential_mpa[cell] = matric + osmotic;
...
fn validate(context: ApplyContext, range: compute.CellRange) !void {
    ...
    inline for (.{ context.litter_water_m3, context.litter_temperature_k, context.field_capacity_potential_mpa, context.wilting_point_potential_mpa }) |values| if (values.len != cells) return error.SurfaceLitterWaterEnvironmentDimensionMismatch;
    ...
    if (context.mean_annual_temperature_c.len != context.result.cell_count or context.parameters.saturation_water_potential_mpa >= 0 or context.parameters.minimum_water_potential_mpa >= context.parameters.saturation_water_potential_mpa or context.parameters.hygroscopic_water_potential_mpa >= 0 or context.parameters.saturation_to_field_shape <= 0 or context.parameters.below_wilting_shape <= 0) return error.InvalidSurfaceLitterWaterEnvironmentParameter;
...
test "surface litter water environment reproduces HOUR1 potentials and ATCS adaptation" {
    ...
    defer state.deinit();
    var context: ApplyContext = .{ .result = &state, .litter_geometry = &litter_geometry, .litter_chemistry = &litter_chemistry, .litter_water_m3 = &.{0.2}, .litter_temperature_k = &.{293.15}, .field_capacity_potential_mpa = &.{-0.033}, .wilting_point_potential_mpa = &.{-1.5}, .mean_annual_temperature_c = &.{5}, .parameters = .{ .saturation_water_potential_mpa = -0.0005, .minimum_water_potential_mpa = -1.5e12, .hygroscopic_water_potential_mpa = -1.5e4, .saturation_to_field_shape = 0.5, .below_wilting_shape = 0.5 } };
    try applyTile(&context, .{ .first = 0, .end = 1 });
    try std.testing.expect(state.matric_water_potential_mpa[0] < -0.033);
    try std.testing.expect(state.osmotic_water_potential_mpa[0] < 0);
