...
const partition_module = @import("redist_fire_combustion_partition.zig");

pub const Partition = @import("redist_fire_microbial_biomass_combustion.zig").Partition;
...
/// Direct translation of active REDIST 10876--10897 statements for one layer.
pub fn combust(layer: usize, partition: Partition, rates: Rates, state: State) !void {
    ...
    if (rates.layer_count == 0 or layer >= rates.layer_count or
        ...
        rates.phosphorus_g_p.len != rate_len or state.charcoal_carbon_g_c.len != rate_len or
        state.ammonium_g_n.len != rates.layer_count or
        state.dihydrogen_phosphate_g_p.len != rates.layer_count)
        return error.FireCharcoalDimensionMismatch;
    if (!validPartition(partition) or !finiteSlice(rates.carbon_g_c) or
        ...
        return error.InvalidFireCharcoalInput;
    inline for (.{ rates.carbon_g_c, rates.nitrogen_g_n, rates.phosphorus_g_p, state.charcoal_carbon_g_c, state.ammonium_g_n, state.dihydrogen_phosphate_g_p }) |values|
        for (values) |value| if (value < 0) return error.InvalidFireCharcoalInput;
    ...
    for (0..5) |k| {
        ...
        inline for (.{ next_charcoal[k], next_nh4, next_po4, next.carbon_dioxide_emission_g_c, next.methane_emission_g_c, next.charcoal_carbon_g_c, next.gaseous_nitrogen_g_n, next.gaseous_phosphorus_g_p, next.mineral_nitrogen_g_n, next.mineral_phosphorus_g_p }) |value|
            if (!std.math.isFinite(value)) return error.NonFiniteFireCharcoalResult;
    }
...
test "REDIST charcoal combustion preserves K order and conserves C N P" {
    const layers = 2;
    ...
    try std.testing.expectApproxEqAbs(10, accumulators.charcoal_carbon_g_c + accumulators.carbon_dioxide_emission_g_c + accumulators.methane_emission_g_c, 1e-12);
    try std.testing.expectApproxEqAbs(15, accumulators.mineral_nitrogen_g_n + accumulators.gaseous_nitrogen_g_n, 1e-12);
    try std.testing.expectApproxEqAbs(20, accumulators.mineral_phosphorus_g_p + accumulators.gaseous_phosphorus_g_p, 1e-12);
    for (0..5) |k| try std.testing.expectEqual(@as(f64, 0.5), charcoal[index(layers, k, 1)]);
    try std.testing.expectEqual(@as(f64, 0), charcoal[0]);
}

test "REDIST charcoal combustion validation failure is atomic" {
