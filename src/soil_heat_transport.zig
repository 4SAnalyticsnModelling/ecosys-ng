...
const SurfaceTemperatureState = @import("surface_temperature_solver.zig").State;
...
pub const State = struct {
    ...
    downward_conductive_heat_flux_mj_per_m2: []f64,
    heat_balance_residual_mj_per_m2: []f64,
    ...
    pub fn init(allocator: std.mem.Allocator, cell_count: usize, soil_layer_capacity: usize) !State {
        ...
        @memset(flux, 0);
        @memset(residual, 0);
        return .{ .allocator = allocator, .cell_count = cell_count, .soil_layer_capacity = soil_layer_capacity, .downward_conductive_heat_flux_mj_per_m2 = flux, .heat_balance_residual_mj_per_m2 = residual };
    }

    pub fn deinit(self: *State) void {
        self.allocator.free(self.downward_conductive_heat_flux_mj_per_m2);
        self.allocator.free(self.heat_balance_residual_mj_per_m2);
        self.* = undefined;
    }

    pub fn validateFinite(self: State) !void {
        for (self.downward_conductive_heat_flux_mj_per_m2, 0..) |value, index| if (!std.math.isFinite(value)) {
            std.log.err("non-finite soil heat flux: layer_slot={d} value={e}", .{ index, value });
            return error.NonFiniteSoilHeatFlux;
        };
        for (self.heat_balance_residual_mj_per_m2, 0..) |value, cell| if (!std.math.isFinite(value)) {
            std.log.err("non-finite soil heat balance: cell={d} residual_mj_per_m2={e}", .{ cell, value });
...
pub fn updateTile(context: *UpdateContext, range: CellRange) !void {
    ...
    for (range.first..range.end) |cell| {
        ...
        var initial_sensible_heat_mj_per_m2: f64 = 0;
        for (0..active_layers) |layer| {
            const index = base + layer;
            initial_sensible_heat_mj_per_m2 += arealHeatCapacity(context.thermal, index) * context.grid.soil_temperature_k[index];
        }
        ...
        const requested_surface_to_soil = -context.surface.conductive_heat_flux_mj_per_m2[cell];
        ...
        for (0..active_layers) |layer| transport.downward_conductive_heat_flux_mj_per_m2[base + layer] = 0;
        for (0..active_layers - 1) |layer| {
            ...
            const resistance_h_k_m2_per_mj = upper_half_distance_m / context.thermal.thermal_conductivity_m_mj_per_h_k[upper] + lower_half_distance_m / context.thermal.thermal_conductivity_m_mj_per_h_k[lower];
            if (!std.math.isFinite(resistance_h_k_m2_per_mj) or resistance_h_k_m2_per_mj <= 0) return error.InvalidSoilThermalResistance;
            const temperature_difference_k = context.grid.soil_temperature_k[upper] - context.grid.soil_temperature_k[lower];
            const requested_flux = context.timestep_hours * temperature_difference_k / resistance_h_k_m2_per_mj;
            ...
            transport.downward_conductive_heat_flux_mj_per_m2[upper] = std.math.clamp(requested_flux, -equilibration_limit, equilibration_limit);
        }

        var incoming_mj_per_m2 = surface_to_soil;
        for (0..active_layers) |layer| {
            const index = base + layer;
            const outgoing_mj_per_m2 = transport.downward_conductive_heat_flux_mj_per_m2[index];
            context.grid.soil_temperature_k[index] += (incoming_mj_per_m2 - outgoing_mj_per_m2) / arealHeatCapacity(context.thermal, index);
            ...
            incoming_mj_per_m2 = outgoing_mj_per_m2;
        }
        var final_sensible_heat_mj_per_m2: f64 = 0;
        for (0..active_layers) |layer| {
            const index = base + layer;
            final_sensible_heat_mj_per_m2 += arealHeatCapacity(context.thermal, index) * context.grid.soil_temperature_k[index];
        }
        const residual_mj_per_m2 = final_sensible_heat_mj_per_m2 - initial_sensible_heat_mj_per_m2 - surface_to_soil;
        transport.heat_balance_residual_mj_per_m2[cell] = residual_mj_per_m2;
        const balance_scale = @max(1.0, @max(@abs(initial_sensible_heat_mj_per_m2), @abs(final_sensible_heat_mj_per_m2)));
        if (@abs(residual_mj_per_m2) > 1.0e-12 * balance_scale) {
            std.log.err("soil heat conservation failure: cell={d} residual_mj_per_m2={e} scale_mj_per_m2={e}", .{ cell, residual_mj_per_m2, balance_scale });
...
fn arealHeatCapacity(thermal: *const SoilThermalState, index: usize) f64 {
    return thermal.total_heat_capacity_mj_per_m3_k[index] * thermal.layer_thickness_m[index];
}
