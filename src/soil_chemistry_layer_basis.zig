...
pub const Basis = struct {
    ...
    ammonium_extract_mol_n_per_Mg: f64,
    nitrate_extract_mol_n_per_Mg: f64,
    phosphate_extract_mol_p_per_Mg: f64,
};

/// Direct translation of STARTE lines 179--191 and 208--211. This establishes
...
pub fn derive(inputs: Inputs, parameters: Parameters) !Basis {
    ...
    if (inputs.soil_mass_Mg < 0 or
        inputs.pore_volume_m3 <= 0 or
        inputs.field_capacity_water_m3 < 0 or
        inputs.negligible_soil_mass_Mg < 0 or
        inputs.cation_exchange_capacity_mol_charge_per_Mg < 0 or
        inputs.anion_exchange_capacity_mol_charge_per_Mg < 0 or
        inputs.ammonium_mol_n_per_Mg < 0 or
        inputs.nitrate_mol_n_per_Mg < 0 or
        inputs.phosphate_mol_p_per_Mg < 0 or
    ...
    const effective_soil_mass_Mg =
        if (inputs.soil_mass_Mg > inputs.negligible_soil_mass_Mg)
            inputs.soil_mass_Mg
        else
            inputs.pore_volume_m3;
    const mass_to_field_capacity =
        if (inputs.field_capacity_water_m3 > inputs.negligible_soil_mass_Mg)
            @min(1.0, effective_soil_mass_Mg / inputs.field_capacity_water_m3)
        else
            1.0;
    ...
    const result: Basis = .{
        .effective_soil_mass_Mg = effective_soil_mass_Mg,
        .soil_mass_to_field_capacity_ratio = mass_to_field_capacity,
        .cation_exchange_capacity_mol_charge = inputs.cation_exchange_capacity_mol_charge_per_Mg *
            effective_soil_mass_Mg,
        .anion_exchange_capacity_mol_charge = inputs.anion_exchange_capacity_mol_charge_per_Mg *
            effective_soil_mass_Mg,
        .hydrogen_mol_per_m3 = hydrogen_mol_per_m3,
        .hydroxide_mol_per_m3 = hydroxide_mol_per_m3,
        .ammonium_extract_mol_n_per_Mg = parameters.ammonium_extract_multiplier *
            inputs.ammonium_mol_n_per_Mg,
        .nitrate_extract_mol_n_per_Mg = inputs.nitrate_mol_n_per_Mg,
        .phosphate_extract_mol_p_per_Mg = parameters.phosphate_extract_multiplier *
            inputs.phosphate_mol_p_per_Mg,
    };
...
test "STARTE layer basis preserves source operation order and units" {
    const result = try derive(.{
        ...
        .negligible_soil_mass_Mg = 1.0e-12,
        .cation_exchange_capacity_mol_charge_per_Mg = 10,
        .anion_exchange_capacity_mol_charge_per_Mg = 3,
        .soil_ph = 7,
        .ammonium_mol_n_per_Mg = 8,
        .nitrate_mol_n_per_Mg = 6,
        .phosphate_mol_p_per_Mg = 4,
    }, sourceParameters());
    try std.testing.expectEqual(@as(f64, 2), result.effective_soil_mass_Mg);
    ...
    try std.testing.expectEqual(@as(f64, 0.8), result.ammonium_extract_mol_n_per_Mg);
    try std.testing.expectEqual(@as(f64, 6), result.nitrate_extract_mol_n_per_Mg);
    try std.testing.expectEqual(@as(f64, 0.04), result.phosphate_extract_mol_p_per_Mg);
}
...
test "STARTE layer basis uses pore volume for a negligible soil mass" {
    const result = try derive(.{
        ...
        .anion_exchange_capacity_mol_charge_per_Mg = 1,
        .soil_ph = 6,
        .ammonium_mol_n_per_Mg = 0,
        .nitrate_mol_n_per_Mg = 0,
        .phosphate_mol_p_per_Mg = 0,
    }, sourceParameters());
    try std.testing.expectEqual(@as(f64, 0.75), result.effective_soil_mass_Mg);
...
test "STARTE layer basis rejects invalid state before publishing a result" {
    var inputs: Inputs = .{
        ...
        .nitrate_mol_n_per_Mg = 1,
        .phosphate_mol_p_per_Mg = 1,
    };
    inputs.soil_mass_Mg = std.math.nan(f64);
