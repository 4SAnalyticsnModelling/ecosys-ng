...
const manifest_module = @import("checkpoint_manifest.zig");
const bundle_io = @import("checkpoint_bundle_io.zig");
...
fn validateShape(shape: manifest_module.RuntimeShape, live: LiveView) !void {
    ...
    if (live.soil_geometry_and_hydrology.delayed_live_canopy_combustion_heat_mj.len != plants or live.soil_geometry_and_hydrology.delayed_standing_dead_combustion_heat_mj.len != plants or live.soil_geometry_and_hydrology.delayed_subsurface_combustion_heat_mj.len != cells * shape.soil_layers or live.soil_geometry_and_hydrology.delayed_surface_combustion_heat_mj.len != cells or live.soil_geometry_and_hydrology.surface_litter_ice_m3.len != cells) return error.CheckpointLiveShapeMismatch;
