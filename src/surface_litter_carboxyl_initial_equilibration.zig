...
pub const Parameters = struct {
    ...
    maximum_exchange_mol_per_Mg_iteration: f64,
    negligible_open_sites_mol_per_Mg: f64,
};

pub const Result = struct {
    open_carboxyl_sites_mol_per_Mg: f64,
    equilibrium_open_carboxyl_sites_mol_per_Mg: f64,
    protonation_change_mol_per_Mg_iteration: f64,
    protonated_carboxyl_sites_mol_per_Mg: f64,
};

/// Direct single-iteration translation of STARTE lines 1896--1900. This
...
pub fn iterate(inputs: Inputs, parameters: Parameters) !Result {
    ...
    const change = @max(
        -parameters.maximum_exchange_mol_per_Mg_iteration *
            inputs.protonated_carboxyl_sites_mol_per_Mg,
        -parameters.maximum_exchange_mol_per_Mg_iteration,
        @min(
            parameters.maximum_exchange_mol_per_Mg_iteration,
    ...
    const protonated_sites =
        inputs.protonated_carboxyl_sites_mol_per_Mg + change;
    const result: Result = .{
        .open_carboxyl_sites_mol_per_Mg = open_sites,
        .equilibrium_open_carboxyl_sites_mol_per_Mg = equilibrium_open_sites,
        .protonation_change_mol_per_Mg_iteration = change,
        .protonated_carboxyl_sites_mol_per_Mg = protonated_sites,
    };
    inline for (@typeInfo(Result).@"struct".fields) |field|
        if (!std.math.isFinite(@field(result, field.name)))
            return error.NonFiniteSurfaceCarboxylInitializationResult;
    if (protonated_sites < 0 or
        protonated_sites > inputs.total_carboxyl_sites_mol_per_Mg)
...
fn sourceParameters() Parameters {
    return .{
        ...
        .maximum_exchange_mol_per_Mg_iteration = 1.0e-2,
        .negligible_open_sites_mol_per_Mg = 1.0e-48,
    };
}
...
test "STARTE litter carboxyl iteration preserves nested source bounds" {
    const result = try iterate(.{
        ...
        .protonated_carboxyl_sites_mol_per_Mg = 0.4,
        .hydrogen_activity_mol_per_m3 = 0.1,
    }, sourceParameters());
    try std.testing.expectEqual(@as(f64, 0.6), result.open_carboxyl_sites_mol_per_Mg);
    try std.testing.expectEqual(
        @as(f64, 0.04),
        result.equilibrium_open_carboxyl_sites_mol_per_Mg,
    );
    try std.testing.expectEqual(
        @as(f64, 0.01),
        result.protonation_change_mol_per_Mg_iteration,
    );
...
test "STARTE litter carboxyl desorption is limited by occupied fraction" {
    const result = try iterate(.{
        .total_carboxyl_sites_mol_per_Mg = 1,
        .protonated_carboxyl_sites_mol_per_Mg = 0.05,
        .hydrogen_activity_mol_per_m3 = 1.0e-6,
    }, .{
        .carboxyl_dissociation_mol_per_m3 = 1,
        .maximum_exchange_mol_per_Mg_iteration = 0.2,
        .negligible_open_sites_mol_per_Mg = 1.0e-48,
    });
    try std.testing.expectApproxEqAbs(
        @as(f64, -0.01),
...
test "STARTE litter carboxyl iteration rejects invalid occupancy" {
    try std.testing.expectError(
        ...
        iterate(.{
            .total_carboxyl_sites_mol_per_Mg = 1,
            .protonated_carboxyl_sites_mol_per_Mg = 2,
