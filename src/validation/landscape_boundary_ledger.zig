...
const atmospheric_solutes = @import("../atmosphere/atmospheric_solute_input_ledger.zig");
...
pub const State = struct {
    ...
    ///
    /// HEAT-001 fourth layer, measured by A6b. The adjustment above was
    /// `(C_i - C_l) * d_ice * T`, and that is the wrong sensible consequence
    /// by exactly the constant `(C_l - C_i) * Tm = 618.03 MJ m-3` per cubic
    /// metre of ice formed. It is the same reference-state drift that was
    /// corrected for boundary snowfall at lines 167--186 above, in the same
    /// pairing, left uncorrected here.
    ///
    /// The derivation, against the census this must pair with. The surface
    /// census (`landscape_mass_inventory.zig:1350`) carries
    ///   `E = C_s*T + froz(T)*I`,  `froz(T) = C_l*Tm - L + C_i*(T - Tm)`
    /// with `C_s` including `C_l * liquid`, and `liquid + I` conserved under a
    /// repartition. So a repartition at fixed `T` changes the census by
    ///   `[froz(T) - C_l*T] * d_ice = [-L + (C_l - C_i)*(Tm - T)] * d_ice`.
    /// The surface flux booking in `accumulateAcceptedSurfaceAndCanopyHeat`
    /// already carries `-L*d_ice`, because by the surface partition identity
    ///   `net_rad + sensible + latent + vapor == -(conduction + storage + phase)`
    /// (verified to 6.7e-8 relative by `tools/a6b_surface_partition_audit.py`)
    /// that booking equals `C_s*dT - L*d_ice - conduction*A`, and the
    /// conduction half is separately paired with the soil. Subtracting the
    /// booked part from the census change leaves the required adjustment
    ///   `required = (C_l - C_i) * (Tm - T) * d_ice`
    /// against the superseded `booked = (C_i - C_l) * T * d_ice`, which differ
    /// by `-(C_l - C_i) * Tm * d_ice`. The two agree only at `T = 0 K`.
    ///
    /// Note the physical check the superseded form fails: at `T = Tm` a
    /// repartition costs no sensible energy at all, because both phases have
    /// the same enthalpy at the melting point on this reference state. The old
    /// form booked `-(C_l - C_i)*Tm*d_ice = -618.03 MJ` per cubic metre there;
    /// the corrected form books exactly `0`, and it is antisymmetric in
    /// `Tm - T` as a sensible-heat difference must be.
    ///
    /// Measured on Ottawa day one. Surface litter ice grows from `0` to
    /// `5.172315618591871e4 m3` over hours 2--9 and the mis-booking is
    /// `-3.576892e7 MJ = -0.410335 MJ m-2`. Because the ledger under-credits
    /// input, `balance = storage - input + output` rises, giving a *positive*
    /// deviation, and the audited abort is `+5.261519631191418e-1 MJ m-2`.
    /// This single defect accounts for `78.0%` of it, and it is confined to
    /// hours 2--10, which is exactly the window carrying the largest per-hour
    /// residual injection once the closure instrument's one-hour surface lag
    /// is corrected for.
...
test "surface phase audit books only the sensible capacity rebase, not latent" {
    ...
    //
    // HEAT-001 fourth layer (A6b): that rebase is `(C_l - C_i)*(Tm - T)*d_ice`,
    // not the superseded `(C_i - C_l)*T*d_ice`. Here that is
    // `(4 - 2) * (273.15 - 280) * 0.25 = -3.425 MJ`, an input of `0` and an
    // output of `3.425`. Above the melting point, freezing costs *negative*
    // sensible energy because ice has the smaller capacity, so the sign is
    // right. The superseded form gave `-140 MJ`, wrong by
    // `(C_l - C_i)*Tm*d_ice = 2*273.15*0.25 = 136.575 MJ`.
    try std.testing.expectEqual(@as(f64, 0), state.cumulative.heat_input_megajoules);
    try std.testing.expectApproxEqAbs(
        @as(f64, 3.425),
        state.cumulative.heat_output_megajoules,
...
test "surface phase rebase vanishes at the melting point and is antisymmetric" {
    // The property the superseded `(C_i - C_l)*T*d_ice` form violated, and the
    // reason HEAT-001 lost `-0.410335 MJ m-2` on Ottawa day one. On the census
    // reference state "liquid water at 0 K" both phases carry the same enthalpy
    // at `Tm`, so a repartition exactly at `Tm` costs no sensible energy at all
    // and the audit adjustment must be identically zero. The old form booked
    // `(C_l - C_i)*Tm*d_ice = 618.03 MJ` per cubic metre there.
    var at_melting: State = .{};
    try at_melting.accumulateAcceptedSurfacePhaseSensibleAdjustment(
        &.{333},
        &.{1},
        &.{inventory.default_pure_water_melting_temperature_k},
        &.{1},
    ...
    try std.testing.expectEqual(@as(f64, 0), at_melting.cumulative.heat_input_megajoules);
    try std.testing.expectEqual(@as(f64, 0), at_melting.cumulative.heat_output_megajoules);

    // A sensible-heat difference is antisymmetric about `Tm`: freezing the same
    // volume the same number of kelvin below and above the melting point must
    // book equal and opposite adjustments. `(C_i - C_l)*T*d_ice` is not.
    const offset_k = 10.0;
    var below: State = .{};
    try below.accumulateAcceptedSurfacePhaseSensibleAdjustment(
        &.{333},
        &.{1},
        &.{inventory.default_pure_water_melting_temperature_k - offset_k},
        &.{1},
        4.19,
        1.9274,
    );
    var above: State = .{};
    try above.accumulateAcceptedSurfacePhaseSensibleAdjustment(
        &.{333},
        &.{1},
        &.{inventory.default_pure_water_melting_temperature_k + offset_k},
        &.{1},
    ...
    const expected = (4.19 - 1.9274) * offset_k;
    try std.testing.expectApproxEqAbs(expected, below.cumulative.heat_input_megajoules, 1e-12);
    try std.testing.expectEqual(@as(f64, 0), below.cumulative.heat_output_megajoules);
    try std.testing.expectEqual(@as(f64, 0), above.cumulative.heat_input_megajoules);
    try std.testing.expectApproxEqAbs(expected, above.cumulative.heat_output_megajoules, 1e-12);
}

test "surface phase rebase pairs exactly with the surface enthalpy census" {
    // The decisive conservation property, stated as the identity the ledger and
    // the census must jointly satisfy. Freezing `d_ice` at temperature `T`
    // changes the surface census by `[froz(T) - C_l*T]*d_ice`. The surface flux
    // booking already carries `-L*d_ice` of that. Whatever remains is exactly
    // what this routine must book, or the audit opens by the difference. This
    // test asserts that equality directly against
    // `landscape_mass_inventory.frozenWaterEnthalpyPerM3`, so the two sides
    // cannot drift again the way the precipitation boundary did.
    ...
    for ([_]f64{ 250, 260, 270, melting, 280, 290 }) |temperature_k| {
        for ([_]f64{ 0.5, -0.25 }) |ice_change_m3| {
            var ledger: State = .{};
            try ledger.accumulateAcceptedSurfacePhaseSensibleAdjustment(
            ...
            const booked = ledger.cumulative.heat_input_megajoules -
                ledger.cumulative.heat_output_megajoules;
            // What the census does when the same water repartitions at fixed T.
            const census_change = (inventory.frozenWaterEnthalpyPerM3(
                temperature_k,
                liquid_capacity,
                ice_capacity,
                latent,
            ...
            // What the surface flux booking already carries for the same event.
            const already_booked_by_surface_flux = -latent * ice_change_m3;
            try std.testing.expectApproxEqAbs(
                census_change - already_booked_by_surface_flux,
                booked,
                1e-12,
            );
        }
