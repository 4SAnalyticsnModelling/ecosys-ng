...
const SurfaceEnergyState = @import("surface_energy.zig").State;
const ExposureState = @import("canopy_exposure.zig").State;
const SoilThermalState = @import("soil_thermal.zig").State;
const phase_change = @import("soil_water_phase_change.zig");
const retention = @import("soil_water_retention.zig");

pub const Settings = struct {
    sensible_heat_conductance_mj_per_m2_h_k: f64,
    latent_heat_conductance_mj_per_m2_h_kpa: f64,
...
pub const State = struct {
    ...
    vapor_sensible_heat_flux_mj_per_m2: []f64,
    conductive_heat_flux_mj_per_m2: []f64,
    storage_heat_flux_mj_per_m2: []f64,
    phase_heat_flux_mj_per_m2: []f64,
    ...
    pub fn init(allocator: std.mem.Allocator, cell_count: usize) !State {
        ...
        var result: State = .{
            ...
            .energy_residual_mj_per_m2 = undefined,
            .residual_tolerance_mj_per_m2 = undefined,
            .sensible_heat_flux_mj_per_m2 = undefined,
            .latent_heat_flux_mj_per_m2 = undefined,
            .vapor_sensible_heat_flux_mj_per_m2 = undefined,
            .conductive_heat_flux_mj_per_m2 = undefined,
            .storage_heat_flux_mj_per_m2 = undefined,
            .phase_heat_flux_mj_per_m2 = undefined,
        ...
        result.energy_residual_mj_per_m2 = try allocator.alloc(f64, cell_count);
        errdefer allocator.free(result.energy_residual_mj_per_m2);
        result.residual_tolerance_mj_per_m2 = try allocator.alloc(f64, cell_count);
        errdefer allocator.free(result.residual_tolerance_mj_per_m2);
        result.sensible_heat_flux_mj_per_m2 = try allocator.alloc(f64, cell_count);
        errdefer allocator.free(result.sensible_heat_flux_mj_per_m2);
        result.latent_heat_flux_mj_per_m2 = try allocator.alloc(f64, cell_count);
        errdefer allocator.free(result.latent_heat_flux_mj_per_m2);
        result.vapor_sensible_heat_flux_mj_per_m2 = try allocator.alloc(f64, cell_count);
        errdefer allocator.free(result.vapor_sensible_heat_flux_mj_per_m2);
        result.conductive_heat_flux_mj_per_m2 = try allocator.alloc(f64, cell_count);
        errdefer allocator.free(result.conductive_heat_flux_mj_per_m2);
        result.storage_heat_flux_mj_per_m2 = try allocator.alloc(f64, cell_count);
        errdefer allocator.free(result.storage_heat_flux_mj_per_m2);
        result.phase_heat_flux_mj_per_m2 = try allocator.alloc(f64, cell_count);
        errdefer allocator.free(result.phase_heat_flux_mj_per_m2);
        ...
        @memset(result.energy_residual_mj_per_m2, 0);
        @memset(result.residual_tolerance_mj_per_m2, 0);
        @memset(result.sensible_heat_flux_mj_per_m2, 0);
        @memset(result.latent_heat_flux_mj_per_m2, 0);
        @memset(result.vapor_sensible_heat_flux_mj_per_m2, 0);
        @memset(result.conductive_heat_flux_mj_per_m2, 0);
        @memset(result.storage_heat_flux_mj_per_m2, 0);
        @memset(result.phase_heat_flux_mj_per_m2, 0);
    ...
    pub fn deinit(self: *State) void {
        ...
        self.allocator.free(self.storage_heat_flux_mj_per_m2);
        self.allocator.free(self.phase_heat_flux_mj_per_m2);
        self.allocator.free(self.liquid_water_change_m3);
        self.allocator.free(self.ice_water_equivalent_change_m3);
        self.allocator.free(self.conductive_heat_flux_mj_per_m2);
        self.allocator.free(self.vapor_sensible_heat_flux_mj_per_m2);
        self.allocator.free(self.latent_heat_flux_mj_per_m2);
        self.allocator.free(self.sensible_heat_flux_mj_per_m2);
        self.allocator.free(self.residual_tolerance_mj_per_m2);
        self.allocator.free(self.energy_residual_mj_per_m2);
    ...
    pub fn validateFinite(self: State) !void {
        ...
        for (self.energy_residual_mj_per_m2, 0..) |value, cell| if (!std.math.isFinite(value)) {
            std.log.err("non-finite surface energy residual: cell={d} value={e}", .{ cell, value });
            return error.NonFiniteSurfaceTemperatureResidual;
        };
        inline for (.{ self.sensible_heat_flux_mj_per_m2, self.latent_heat_flux_mj_per_m2, self.vapor_sensible_heat_flux_mj_per_m2, self.conductive_heat_flux_mj_per_m2, self.storage_heat_flux_mj_per_m2, self.phase_heat_flux_mj_per_m2, self.liquid_water_change_m3, self.ice_water_equivalent_change_m3 }) |values| for (values, 0..) |value, cell| {
    ...
    pub const Diagnostics = struct {
        maximum_absolute_residual_mj_per_m2: f64,
    ...
    pub fn validateConvergence(self: State) !Diagnostics {
        var diagnostics: Diagnostics = .{
            .maximum_absolute_residual_mj_per_m2 = 0,
            .total_iterations = 0,
        ...
        for (0..self.cell_count) |cell| {
            const absolute_residual = @abs(self.energy_residual_mj_per_m2[cell]);
            const tolerance = self.residual_tolerance_mj_per_m2[cell];
            if (!std.math.isFinite(absolute_residual) or absolute_residual > tolerance) {
                std.log.err("surface nonlinear convergence audit failed: cell={d} residual_mj_per_m2={e} tolerance={e} temperature_k={e}", .{ cell, absolute_residual, tolerance, self.equilibrium_temperature_k[cell] });
                return error.SurfaceTemperatureResidualTooLarge;
            }
            diagnostics.maximum_absolute_residual_mj_per_m2 = @max(diagnostics.maximum_absolute_residual_mj_per_m2, absolute_residual);
...
pub const ApplyContext = struct {
    ...
    surface_energy: *const SurfaceEnergyState,
    soil_thermal: *const SoilThermalState,
    exposure: ?*const ExposureState,
    external_heat_mj_per_m2: []const f64,
...
const ResidualContext = struct {
    ...
    atmospheric_longwave_mj_per_m2: f64,
    ...
    conductive_heat_conductance_mj_per_m2_h_k: f64,
    storage_heat_conductance_mj_per_m2_h_k: f64,
    external_heat_mj_per_m2: f64 = 0,
...
pub fn applyTile(context: *ApplyContext, range: CellRange) !void {
    ...
    for (range.first..range.end) |cell| {
        ...
        const residual_context: ResidualContext = .{
            .absorbed_shortwave_mj_per_m2 = context.ground_radiation.absorbed_shortwave_mj_per_m2[cell],
            .atmospheric_longwave_mj_per_m2 = context.atmosphere.longwave_radiation_mj_per_m2[cell],
            ...
            .sensible_heat_conductance_mj_per_m2_h_k = context.settings.sensible_heat_conductance_mj_per_m2_h_k,
            .latent_heat_conductance_mj_per_m2_h_kpa = context.settings.latent_heat_conductance_mj_per_m2_h_kpa,
            ...
            .conductive_heat_conductance_mj_per_m2_h_k = context.soil_thermal.thermal_conductivity_m_mj_per_h_k[surface_layer_index] / (0.5 * layer_thickness_m),
            .storage_heat_conductance_mj_per_m2_h_k = context.soil_thermal.total_heat_capacity_mj_per_m3_k[surface_layer_index] * layer_thickness_m / context.settings.timestep_hours,
            .external_heat_mj_per_m2 = context.external_heat_mj_per_m2[cell],
            .phase = if (context.surface_phase) |surface_phase| phase: {
                ...
                break :phase .{
                    ...
                    .latent_heat_of_fusion_mj_per_m3 = surface_phase.latent_heat_of_fusion_mj_per_m3,
                    .pure_water_melting_temperature_k = surface_phase.pure_water_melting_temperature_k,
                };
            } else null,
        };
        if (!std.math.isFinite(residual_context.external_heat_mj_per_m2)) return error.NonFiniteSurfaceExternalHeat;
        ...
        result.energy_residual_mj_per_m2[cell] = solved.residual;
        result.residual_tolerance_mj_per_m2[cell] = cell_solver_options.absolute_tolerance + cell_solver_options.relative_tolerance * cell_solver_options.residual_scale;
        result.sensible_heat_flux_mj_per_m2[cell] = residual_context.sensible_heat_conductance_mj_per_m2_h_k * (residual_context.air_temperature_k - solved.root);
        result.latent_heat_flux_mj_per_m2[cell] = latentHeatFlux(residual_context, solved.root);
        result.vapor_sensible_heat_flux_mj_per_m2[cell] =
            try vaporSensibleHeatFlux(
                result.latent_heat_flux_mj_per_m2[cell],
                residual_context.air_temperature_k,
                solved.root,
                context.settings.liquid_water_heat_capacity_mj_per_m3_k,
                context.settings.latent_heat_of_vaporization_mj_per_m3,
            );
        result.conductive_heat_flux_mj_per_m2[cell] = residual_context.conductive_heat_conductance_mj_per_m2_h_k * (residual_context.subsurface_temperature_k - solved.root);
        result.storage_heat_flux_mj_per_m2[cell] = residual_context.storage_heat_conductance_mj_per_m2_h_k * (residual_context.previous_surface_temperature_k - solved.root);
        if (context.surface_phase) |surface_phase| if (residual_context.phase != null) {
            ...
            result.ice_water_equivalent_change_m3[cell] =
                ...
                surface_phase.ice_water_equivalent_m3[cell];
            result.phase_heat_flux_mj_per_m2[cell] =
                residual_context.phase.?.latent_heat_of_fusion_mj_per_m3 *
                result.ice_water_equivalent_change_m3[cell] /
            ...
            result.phase_heat_flux_mj_per_m2[cell] = 0;
        } else {
            result.liquid_water_change_m3[cell] = 0;
            result.ice_water_equivalent_change_m3[cell] = 0;
            result.phase_heat_flux_mj_per_m2[cell] = 0;
...
fn residual(context: ResidualContext, temperature_k: f64) f64 {
    const downward_longwave = context.atmospheric_longwave_mj_per_m2 * context.ground_exposure_fraction;
    const emitted_longwave = context.emissivity * 2.04e-10 * std.math.pow(f64, temperature_k, 4) * context.ground_exposure_fraction;
    const sensible_heat = context.sensible_heat_conductance_mj_per_m2_h_k * (context.air_temperature_k - temperature_k);
    const conductive_heat = context.conductive_heat_conductance_mj_per_m2_h_k * (context.subsurface_temperature_k - temperature_k);
    const storage_heat = context.storage_heat_conductance_mj_per_m2_h_k * (context.previous_surface_temperature_k - temperature_k);
    return context.absorbed_shortwave_mj_per_m2 + context.external_heat_mj_per_m2 + downward_longwave - emitted_longwave + sensible_heat + latentHeatFlux(context, temperature_k) + conductive_heat + storage_heat + phaseHeatFlux(context, temperature_k);
}
...
fn derivative(context: ResidualContext, temperature_k: f64) f64 {
    ...
    return -4.0 * context.emissivity * 2.04e-10 * std.math.pow(f64, temperature_k, 3) * context.ground_exposure_fraction - context.sensible_heat_conductance_mj_per_m2_h_k - context.latent_heat_conductance_mj_per_m2_h_kpa * context.surface_vapor_activity_fraction * saturation_derivative - context.conductive_heat_conductance_mj_per_m2_h_k - context.storage_heat_conductance_mj_per_m2_h_k + phaseHeatDerivative(context, temperature_k);
}

fn picard(context: ResidualContext, temperature_k: f64) f64 {
    const downward_longwave = context.atmospheric_longwave_mj_per_m2 * context.ground_exposure_fraction;
    const emitted_longwave = context.emissivity * 2.04e-10 * std.math.pow(f64, temperature_k, 4) * context.ground_exposure_fraction;
    const total_linear_conductance = context.sensible_heat_conductance_mj_per_m2_h_k + context.conductive_heat_conductance_mj_per_m2_h_k + context.storage_heat_conductance_mj_per_m2_h_k;
    return (context.sensible_heat_conductance_mj_per_m2_h_k * context.air_temperature_k + context.conductive_heat_conductance_mj_per_m2_h_k * context.subsurface_temperature_k + context.storage_heat_conductance_mj_per_m2_h_k * context.previous_surface_temperature_k + context.absorbed_shortwave_mj_per_m2 + context.external_heat_mj_per_m2 + downward_longwave - emitted_longwave + latentHeatFlux(context, temperature_k) + phaseHeatFlux(context, temperature_k)) / total_linear_conductance;
}
...
fn latentHeatFlux(context: ResidualContext, temperature_k: f64) f64 {
    return context.latent_heat_conductance_mj_per_m2_h_kpa * (context.atmospheric_vapor_pressure_kpa - context.surface_vapor_activity_fraction * saturationVaporPressureKpa(temperature_k));
}
...
fn vaporSensibleHeatFlux(
    ...
    inline for (.{ latent_heat_flux_mj_per_m2, air_temperature_k, surface_temperature_k, liquid_water_heat_capacity_mj_per_m3_k, latent_heat_of_vaporization_mj_per_m3 }) |value|
        if (!std.math.isFinite(value))
            return error.NonFiniteSurfaceVaporSensibleHeatInput;
    if (air_temperature_k <= 0 or surface_temperature_k <= 0 or
        liquid_water_heat_capacity_mj_per_m3_k <= 0 or
        latent_heat_of_vaporization_mj_per_m3 <= 0)
        return error.InvalidSurfaceVaporSensibleHeatInput;
    const water_flux_into_surface_m3_per_m2 =
        latent_heat_flux_mj_per_m2 / latent_heat_of_vaporization_mj_per_m3;
    const donor_temperature_k = if (water_flux_into_surface_m3_per_m2 >= 0)
        air_temperature_k
    else
        surface_temperature_k;
    const heat_flux_mj_per_m2 = water_flux_into_surface_m3_per_m2 *
        liquid_water_heat_capacity_mj_per_m3_k * donor_temperature_k;
    if (!std.math.isFinite(heat_flux_mj_per_m2))
        return error.NonFiniteSurfaceVaporSensibleHeatResult;
    return heat_flux_mj_per_m2;
}
...
fn surfacePhaseEquilibrium(
    ...
    return phase_change.dallAmicoEquilibrium(.{
        ...
        .latent_heat_of_fusion_mj_per_m3 = context.latent_heat_of_fusion_mj_per_m3,
...
fn phaseHeatFlux(context: ResidualContext, temperature_k: f64) f64 {
    ...
    return phase.latent_heat_of_fusion_mj_per_m3 *
        (equilibrium.ice_water_equivalent_m3 -
...
fn phaseHeatDerivative(context: ResidualContext, temperature_k: f64) f64 {
    ...
    const liquid_water_change_m3_per_k =
        ...
        phase.latent_heat_of_fusion_mj_per_m3 /
...
fn validateSurfacePhaseContext(
    ...
    inline for (.{
        ...
        context.latent_heat_of_fusion_mj_per_m3,
    ...
    if (context.residual_water_content_m3_per_m3 < 0 or
        ...
        context.latent_heat_of_fusion_mj_per_m3 <= 0 or
...
fn residualScale(context: ResidualContext, temperature_k: f64) f64 {
    ...
    return @max(1.0e-12, @abs(context.absorbed_shortwave_mj_per_m2) + @abs(context.external_heat_mj_per_m2) + @abs(downward_longwave) + @abs(emitted_longwave) + @abs(sensible_heat) + @abs(latent_heat) + @abs(conductive_heat) + @abs(storage_heat) + @abs(phase_heat));
}

fn validateSettings(settings: Settings) !void {
    if (!std.math.isFinite(settings.sensible_heat_conductance_mj_per_m2_h_k) or settings.sensible_heat_conductance_mj_per_m2_h_k <= 0 or !std.math.isFinite(settings.latent_heat_conductance_mj_per_m2_h_kpa) or settings.latent_heat_conductance_mj_per_m2_h_kpa < 0 or !std.math.isFinite(settings.liquid_water_heat_capacity_mj_per_m3_k) or settings.liquid_water_heat_capacity_mj_per_m3_k <= 0 or !std.math.isFinite(settings.latent_heat_of_vaporization_mj_per_m3) or settings.latent_heat_of_vaporization_mj_per_m3 <= 0 or !std.math.isFinite(settings.surface_vapor_activity_fraction) or settings.surface_vapor_activity_fraction < 0 or settings.surface_vapor_activity_fraction > 1 or !std.math.isFinite(settings.timestep_hours) or settings.timestep_hours <= 0 or !std.math.isFinite(settings.minimum_temperature_k) or !std.math.isFinite(settings.maximum_temperature_k) or settings.minimum_temperature_k <= 0 or settings.minimum_temperature_k >= settings.maximum_temperature_k) return error.InvalidSurfaceTemperatureSettings;
}

test "hybrid Newton closes radiative sensible surface balance" {
    const context: ResidualContext = .{ .absorbed_shortwave_mj_per_m2 = 1.2, .atmospheric_longwave_mj_per_m2 = 0.9, .air_temperature_k = 285, .subsurface_temperature_k = 280, .previous_surface_temperature_k = 285, .ground_exposure_fraction = 1, .emissivity = 0.97, .sensible_heat_conductance_mj_per_m2_h_k = 0.43, .latent_heat_conductance_mj_per_m2_h_kpa = 0.1, .conductive_heat_conductance_mj_per_m2_h_k = 0.1, .storage_heat_conductance_mj_per_m2_h_k = 0.2, .atmospheric_vapor_pressure_kpa = 1.0, .surface_vapor_activity_fraction = 1.0 };
...
test "delayed litter combustion heat is closed inside the hybrid surface solve" {
    const baseline: ResidualContext = .{
        .absorbed_shortwave_mj_per_m2 = 0.8,
        .atmospheric_longwave_mj_per_m2 = 0.7,
        .air_temperature_k = 280,
        .subsurface_temperature_k = 278,
        .previous_surface_temperature_k = 281,
        .ground_exposure_fraction = 1,
        .emissivity = 0.97,
        .sensible_heat_conductance_mj_per_m2_h_k = 0.43,
        .latent_heat_conductance_mj_per_m2_h_kpa = 0.1,
        .conductive_heat_conductance_mj_per_m2_h_k = 0.1,
        .storage_heat_conductance_mj_per_m2_h_k = 0.2,
        .atmospheric_vapor_pressure_kpa = 0.9,
        .surface_vapor_activity_fraction = 0.8,
    };
    var heated = baseline;
    heated.external_heat_mj_per_m2 = 2;
...
test "analytic residual derivative agrees with finite difference" {
    const context: ResidualContext = .{ .absorbed_shortwave_mj_per_m2 = 0.8, .atmospheric_longwave_mj_per_m2 = 0.7, .air_temperature_k = 280, .subsurface_temperature_k = 278, .previous_surface_temperature_k = 281, .ground_exposure_fraction = 0.75, .emissivity = 0.97, .sensible_heat_conductance_mj_per_m2_h_k = 0.43, .latent_heat_conductance_mj_per_m2_h_kpa = 0.2, .conductive_heat_conductance_mj_per_m2_h_k = 0.1, .storage_heat_conductance_mj_per_m2_h_k = 0.2, .atmospheric_vapor_pressure_kpa = 0.9, .surface_vapor_activity_fraction = 0.85 };
...
test "surface residue Dall'Amico enthalpy converges without a sub-hour cycle" {
    const phase: CellPhaseContext = .{
        ...
        .latent_heat_of_fusion_mj_per_m3 = 333,
        .pure_water_melting_temperature_k = 273.15,
    };
    const context: ResidualContext = .{
        .absorbed_shortwave_mj_per_m2 = 0,
        .atmospheric_longwave_mj_per_m2 = 0,
        .air_temperature_k = 260,
        .subsurface_temperature_k = 260,
        .previous_surface_temperature_k = 260,
        .ground_exposure_fraction = 0,
        .emissivity = 0,
        .sensible_heat_conductance_mj_per_m2_h_k = 1,
        .latent_heat_conductance_mj_per_m2_h_kpa = 0,
        .conductive_heat_conductance_mj_per_m2_h_k = 1,
        .storage_heat_conductance_mj_per_m2_h_k = 1,
