...
pub const Inputs = struct {
    ...
    minimum_canopy_turgor_potential_mpa: f64,
...
/// Exact GROSUB lines 535--544 canopy-water stress response.
///
...
pub fn calculate(inputs: Inputs) !Response {
    inline for (.{
        inputs.canopy_turgor_potential_mpa,
        inputs.minimum_canopy_turgor_potential_mpa,
        inputs.canopy_water_potential_mpa,
        inputs.stomatal_turgor_shape_per_mpa,
    ...
    const growth_factor = std.math.exp(
        growth_exponent * inputs.canopy_water_potential_mpa,
    );
...
test "GROSUB shallow-root response preserves source equations" {
    const response = try calculate(.{
        ...
        .canopy_turgor_potential_mpa = -0.4,
        .minimum_canopy_turgor_potential_mpa = -0.9,
        .canopy_water_potential_mpa = -2.0,
        .stomatal_turgor_shape_per_mpa = 12.0,
    });
...
test "GROSUB deeper-root response and turgor bounds preserve source equations" {
    const response = try calculate(.{
        ...
        .canopy_turgor_potential_mpa = -0.25,
        .minimum_canopy_turgor_potential_mpa = -2.0,
        .canopy_water_potential_mpa = -3.0,
        .stomatal_turgor_shape_per_mpa = 2.0,
    });
    ...
    const below_minimum = try calculate(.{
        .root_profile = .non_shallow,
        .canopy_turgor_potential_mpa = -2.1,
        .minimum_canopy_turgor_potential_mpa = -2.0,
        .canopy_water_potential_mpa = 0.0,
        .stomatal_turgor_shape_per_mpa = 0.0,
    });
...
test "GROSUB retains finite exponential responses above one" {
    const response = try calculate(.{
        .root_profile = .non_shallow,
        .canopy_turgor_potential_mpa = 0.5,
        .minimum_canopy_turgor_potential_mpa = -1.0,
        .canopy_water_potential_mpa = 2.0,
        .stomatal_turgor_shape_per_mpa = 2.0,
    });
...
test "non-finite canopy-water inputs and responses fail explicitly" {
    try std.testing.expectError(error.NonFiniteCanopyWaterStressInput, calculate(.{
        .root_profile = .shallow,
        .canopy_turgor_potential_mpa = std.math.nan(f64),
        .minimum_canopy_turgor_potential_mpa = 0.0,
        .canopy_water_potential_mpa = 0.0,
        .stomatal_turgor_shape_per_mpa = 0.0,
    }));
    try std.testing.expectError(error.NonFiniteCanopyWaterStressResponse, calculate(.{
        .root_profile = .non_shallow,
        .canopy_turgor_potential_mpa = 1000.0,
        .minimum_canopy_turgor_potential_mpa = 0.0,
        .canopy_water_potential_mpa = 0.0,
        .stomatal_turgor_shape_per_mpa = 1.0,
    }));
}
