...
pub const Admission = struct {
    combined_water_potential_megapascal: f64,
...
pub fn evaluate(inputs: Inputs) !Result {
    inline for (.{
        ...
        inputs.osmotic_potential_megapascal,
    ...
    const combined_water_potential_megapascal = inputs.matric_potential_megapascal + inputs.osmotic_potential_megapascal;
    if (!std.math.isFinite(combined_water_potential_megapascal))
    ...
    if (inputs.layer_thickness_m > inputs.minimum_active_layer_thickness_m) {
        if (inputs.layer_index == 0 or inputs.layer_index >= inputs.top_mineral_layer_index) {
            result.admission = .{
                .combined_water_potential_megapascal = combined_water_potential_megapascal,
...
test "NITRO surface admission preserves reset potential sum and KL selection" {
    const result = try evaluate(.{
        ...
        .matric_potential_megapascal = -0.4,
        .osmotic_potential_megapascal = -0.1,
    });
    try std.testing.expectEqual(@as(f64, 0), result.lignin_acidity_accumulator);
    try std.testing.expectEqual(@as(f64, -0.5), result.admission.?.combined_water_potential_megapascal);
...
test "NITRO mineral admission begins at the runtime top mineral layer" {
    const skipped = try evaluate(.{
        ...
        .matric_potential_megapascal = -0.2,
