...
/// `hour1.f` lines 2318--2328. This legacy CNDH calculation is retained as the
...
pub fn compute(inputs: Inputs) !Result {
    ...
    const saturated_conductivity_m2_h_megapascal =
        3.6e3 * source_pi *
    ...
    if (!std.math.isFinite(saturated_conductivity_m2_h_megapascal))
    ...
    return .{
        ...
        .saturated_conductivity_m2_h_megapascal = saturated_conductivity_m2_h_megapascal,
    };
}
...
test "legacy CNDH preserves count spacing viscosity and conductivity order" {
    ...
    try std.testing.expectApproxEqAbs(
        ...
        result.saturated_conductivity_m2_h_megapascal,
        1e-24,
    );
}
