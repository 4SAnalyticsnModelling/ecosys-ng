...
pub fn solve(allocator: std.mem.Allocator, state: *snow.State, parameters: Parameters, options: Options) !Report {
    inline for (.{ parameters.conductivity_scale_m_megajoules_per_h_k, parameters.conductivity_density_exponent_m3_per_megagram, parameters.conductivity_log10_intercept, parameters.maximum_effective_density_megagrams_per_m3, parameters.ice_density_megagrams_per_m3, options.timestep_h, options.full_snow_cover_depth_m, options.absolute_tolerance_k, options.relative_tolerance, options.picard_relaxation }) |value| if (!std.math.isFinite(value)) return error.NonFiniteSnowHeatParameter;
    if (parameters.conductivity_scale_m_megajoules_per_h_k <= 0 or parameters.maximum_effective_density_megagrams_per_m3 <= 0 or parameters.ice_density_megagrams_per_m3 <= 0 or options.timestep_h <= 0 or options.full_snow_cover_depth_m <= 0 or options.absolute_tolerance_k < 0 or options.relative_tolerance < 0 or options.picard_relaxation <= 0 or options.picard_relaxation > 1 or options.max_iterations == 0) return error.InvalidSnowHeatParameter;
    ...
    for (1..@as(usize, options.max_iterations) + 1) |iteration| {
        ...
        for (0..state.cell_count) |cell| {
            ...
            for (0..state.layer_capacity -| 1) |layer| {
                ...
                const first_conductivity = parameters.conductivity_scale_m_megajoules_per_h_k * std.math.pow(f64, 10, parameters.conductivity_density_exponent_m3_per_megagram * first_density + parameters.conductivity_log10_intercept);
                const second_conductivity = parameters.conductivity_scale_m_megajoules_per_h_k * std.math.pow(f64, 10, parameters.conductivity_density_exponent_m3_per_megagram * second_density + parameters.conductivity_log10_intercept);
            ...
            for (0..state.layer_capacity) |layer| {
                ...
                const capacity = state.heat_capacity_megajoules_per_k[index];
