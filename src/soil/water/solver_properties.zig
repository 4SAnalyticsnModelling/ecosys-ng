...
const conductivity = @import("hydraulic_conductivity.zig");
...
pub const State = struct {
    ...
    pub fn initMapped(
        ...
        inline for (@typeInfo(State).@"struct".fields) |field| {
            if (field.type == []f64) {
                const length = if (comptime std.mem.eql(u8, field.name, "matrix_hydraulic_conductivity_m2_per_h_megapascal")) try std.math.mul(usize, grid.layer_count, try std.math.mul(usize, 3, parameters.hydraulic_conductivity_class_count)) else grid.layer_count;
        ...
        for (catalog_index_by_cell, 0..) |catalog_index, cell| {
            ...
            for (0..entry.profile.total_layer_count) |layer| {
                ...
                @memcpy(result.matrix_hydraulic_conductivity_m2_per_h_megapascal[index * classes_per_cell ..][0..classes_per_cell], table.conductivity_m2_per_h_megapascal);
            }
        }
...
fn mpaToPressureHeadM(water_potential_megapascal: f64) !f64 {
...
test "mapped solver properties use runtime dimensions classes and profile science" {
    ...
    try std.testing.expectEqual(grid.layer_count * 3 * 37, state.matrix_hydraulic_conductivity_m2_per_h_megapascal.len);
    ...
    try std.testing.expect(state.matrix_hydraulic_conductivity_m2_per_h_megapascal[2 * parameters.hydraulic_conductivity_class_count] > 0);
