...
const science_module = @import("../runtime/process_science.zig");
...
pub fn advanceMapped(
    ...
    return advance(allocator, grid, hydrology, faces, .{
        .water_geometry = .{ .source_path_length_m = geometry.source_path_length_m, .destination_path_length_m = geometry.destination_path_length_m, .face_area_m2 = geometry.face_area_m2, .macropore_hydraulic_conductance_m_per_h_megapascal = geometry.macropore_hydraulic_conductance_m_per_h_megapascal },
        .water_properties = .{ .matrix_bulk_volume_m3 = properties.matrix_bulk_volume_m3, .retention_curve = properties.retention_curve, .mualem_van_genuchten_parameters = properties.mualem_van_genuchten_parameters, .macropore_mualem_van_genuchten_parameters = workspace.macropore_mualem_van_genuchten_parameters, .macropore_spacing_m = workspace.macropore_spacing_m, .macropore_radius_m = workspace.macropore_radius_m, .dual_domain_exchange_enabled = workspace.macropore_exchange_enabled, .dual_domain_geometry_factor = workspace.dual_domain_geometry_factor, .dual_domain_scaling_coefficient = workspace.dual_domain_scaling_coefficient, .frozen_hydraulic_impedance_exponent = workspace.frozen_hydraulic_impedance_exponent, .gravitational_water_potential_mpa_per_m = workspace.gravitational_water_potential_mpa_per_m, .gravitational_potential_megapascal = workspace.gravitational_potential_megapascal, .osmotic_potential_megapascal = workspace.osmotic_potential_megapascal, .matrix_hydraulic_conductivity_m2_per_h_megapascal = properties.matrix_hydraulic_conductivity_m2_per_h_megapascal, .rainfall_conductivity_multiplier = properties.rainfall_conductivity_multiplier, .matrix_external_source_m3_per_step = options.matrix_external_water_source_m3_per_step, .hydraulic_conductivity_class_count = properties.hydraulic_conductivity_class_count, .vertical_thickness_m = properties.layer_thickness_m, .osmotic_potential_multiplier = options.osmotic_reflection_coefficient, .boundary_topology = options.boundary_topology, .boundary_face_area_m2 = workspace.horizontal_face_area_m2, .boundary_macropore_hydraulic_conductivity_m2_per_h_megapascal = workspace.macropore_hydraulic_conductivity_m2_per_h_megapascal, .boundary_layer_volume_m3 = properties.layer_volume_m3, .boundary_layer_midpoint_depth_m = properties.layer_midpoint_depth_m, .boundary_layer_bottom_depth_m = properties.layer_bottom_depth_m },
...
pub fn advance(allocator: std.mem.Allocator, grid: *grid_module.GridState, hydrology: *hydrology_module.State, faces: *hydrology_module.SoilFaces, inputs: Inputs) !Result {
    ...
    const stage_enthalpy_megajoules_by_cell = try allocator.alloc(f64, if (diagnostics_enabled) grid.layer_count else 0);
    ...
    const richards_energy_change = if (diagnostics_enabled)
        try enthalpyChangeFromSnapshot(stage_enthalpy_megajoules_by_cell, grid, inputs.dry_solid_heat_capacity_megajoules_per_k, inputs.phase_properties.liquid_water_heat_capacity_megajoules_per_m3_k, inputs.phase_properties.ice_heat_capacity_megajoules_per_m3_k, inputs.phase_properties.freeze_thaw.latent_heat_of_fusion_megajoules_per_m3, inputs.phase_properties.freeze_thaw.pure_water_freezing_temperature_k)
    else
        EnergyChange{};
...
test "late WATSUB failure rolls back water vapor heat and shared faces" {
    ...
    try std.testing.expectError(error.InvalidSoilHeatCapacity, advance(std.testing.allocator, &grid, &hydrology, &faces, .{
        .water_geometry = .{ .source_path_length_m = &one_face, .destination_path_length_m = &one_face, .face_area_m2 = &one_face, .macropore_hydraulic_conductance_m_per_h_megapascal = &zero_face },
        .water_properties = .{ .matrix_bulk_volume_m3 = &one_cell, .retention_curve = &curves, .mualem_van_genuchten_parameters = &matrix_parameters, .macropore_mualem_van_genuchten_parameters = &macropore_parameters, .macropore_spacing_m = &macropore_spacing_m, .macropore_radius_m = &macropore_radius_m, .dual_domain_exchange_enabled = &pore_exchange_disabled, .gravitational_potential_megapascal = &zero_cell, .osmotic_potential_megapascal = &zero_cell, .matrix_hydraulic_conductivity_m2_per_h_megapascal = &conductivity, .vertical_thickness_m = &thickness, .osmotic_potential_multiplier = 1 },
