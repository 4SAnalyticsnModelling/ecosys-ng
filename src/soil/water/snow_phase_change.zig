...
pub const Report = struct {
    ...
    /// The change this solve makes to the quantity the landscape heat census
    /// actually stores for snow, namely `C*T` plus the frozen re-basing term
    /// `((C_l - C_i)*Tm - L)` per cubic metre of frozen water equivalent. This
    /// is the value the composition root must book so that a pure internal
    /// freeze/thaw leaves the audit unchanged.
    ///
    /// It is *not* the enthalpy above: the two differ by
    /// `(C_l - C_i)*Tm*d(frozen)`, which is nonzero whenever ice actually
    /// forms or melts.
...
pub fn solve(allocator: std.mem.Allocator, state: *snow.State, options: Options) !Report {
    ...
    // HEAT-001 booking. The census in `landscape_mass_inventory.aggregateSnow`
    // stores `C*T` plus a per-unit-frozen re-basing constant
    //     K = (C_l - C_i) * Tm - L,
    // so the change this solver makes to *census* heat is
    //     d(C*T) + K * d(frozen water equivalent),
    // not the `d(C*T) - L * d(frozen)` enthalpy above. Booking the enthalpy
    // left `(C_l - C_i) * Tm * d(frozen)` of storage unbooked, which showed up
    // as a growing `HeatMassBalanceLost` deviation once melt began.
    {
        var sensible_change_megajoules: f64 = 0;
        var frozen_water_equivalent_change_m3: f64 = 0;
        for (
            heat_capacity,
            temperature,
            solid,
            ...
            previous_ice,
        | {
            sensible_change_megajoules +=
                next_capacity * next_temperature -
                previous_capacity * previous_temperature;
            frozen_water_equivalent_change_m3 +=
                (next_solid + next_ice * options.ice_density_megagrams_per_m3) -
                (previous_solid + previous_ice * options.ice_density_megagrams_per_m3);
        }
        const frozen_rebasing_megajoules_per_m3 =
            (liquid_heat_capacity_megajoules_per_m3_k -
                ice_heat_capacity_megajoules_per_m3_k) * melting_temperature_k -
            options.latent_heat_of_fusion_megajoules_per_m3;
        report.sensible_energy_change_megajoules =
            sensible_change_megajoules +
            frozen_rebasing_megajoules_per_m3 * frozen_water_equivalent_change_m3;
        if (!std.math.isFinite(report.sensible_energy_change_megajoules))
            return error.NonFiniteSnowPhaseEnergyChange;
    }
