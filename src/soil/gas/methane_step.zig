...
const products = @import("../microbial/respiration_products_step.zig");
...
pub fn prepareTile(context: *PrepareContext, range: compute.CellRange) !void {
    ...
    if (range.first > range.end or range.end > n or context.gas_state.cell_count != n or context.respiration_products.layer_count != n or context.water_volume_m3.len != n or context.soil_temperature_k.len != n or context.matric_plus_osmotic_potential_megapascal.len != n) return error.InvalidSoilMethaneDimensions;
    ...
    for (range.first..range.end) |layer| {
        ...
        const temperature_water = try metabolism.growthTemperatureResponse(context.soil_temperature_k[layer], context.thermal_adaptation_offset_k) * @exp(0.1 * context.matric_plus_osmotic_potential_megapascal[layer]);
