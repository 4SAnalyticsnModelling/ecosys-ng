...
test "runtime soil maintenance separates microbial growth and deficit" {
    ...
    var context: ApplyContext = .{ .result = &result, .microbial_state = &microbial_state, .organic_state = &organic_state, .chemistry_state = &chemistry_state, .model_grid = &model_grid, .oxygen_state = &oxygen_state, .matric_plus_osmotic_potential_megapascal = &.{0}, .parameters = parsed, .timestep_h = 1 };
