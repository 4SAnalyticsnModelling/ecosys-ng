...
pub const State = struct {
    ...
    pub fn refresh(self: *State, grid: *const grid_module.GridState, properties: *const property_module.State, thermal: *const thermal_module.State, terrain: *const terrain_module.State, parameters: RuntimeParameters) !void {
        ...
        for (0..grid.cell_count) |cell| {
            ...
            for (0..grid.active_soil_layer_count[cell]) |layer| {
                ...
                self.saturated_lateral_matrix_conductivity_m2_per_h_megapascal[index] = properties.matrix_hydraulic_conductivity_m2_per_h_megapascal[(index * 3 + 1) * properties.hydraulic_conductivity_class_count];
    ...
    pub fn fillMacroporeFaceConductance(self: *const State, faces: *const hydrology_module.SoilFaces, geometry: *const geometry_module.State, output_m_per_h_megapascal: []f64) !void {
        if (faces.micropore_faces.len != output_m_per_h_megapascal.len or geometry.source_path_length_m.len != output_m_per_h_megapascal.len) return error.SoilHourlyWorkspaceDimensionMismatch;
        for (faces.micropore_faces, geometry.source_path_length_m, geometry.destination_path_length_m, output_m_per_h_megapascal) |face, source_path, destination_path, *output| {
            if (face.first_cell >= self.layer_count or face.second_cell >= self.layer_count) return error.InvalidSoilFaceTopology;
            const source = self.macropore_hydraulic_conductivity_m2_per_h_megapascal[face.first_cell];
            const destination = self.macropore_hydraulic_conductivity_m2_per_h_megapascal[face.second_cell];
...
test "hourly workspace refreshes gravity vapor fractions and extensive heat capacity" {
    ...
    var topography = try @import("../../state/topography.zig").parse(allocator, topo_source);
