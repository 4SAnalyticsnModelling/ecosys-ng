        ...
        for (0..grid.cell_count) |cell| {
            ...
            for (0..active_layers) |local_layer| {
                ...
                if (local_layer == 0) {
                    ...
                    const saturated_fraction = mualem_van_genuchten_parameters[above]
                        .effectiveSaturationAtWaterContent(water_fraction) catch 0;
...
test "runtime boundary topology preserves READI compass order and WATSUB signs" {
    ...
    var topography = try @import("../../state/topography.zig").parse(std.testing.allocator, topography_source);
...
test "internal water table refresh interpolates HOUR1 DPTHT from runtime saturation" {
    ...
    try state.refreshInternalWaterTable(&grid, &.{ 1, 1 }, &.{ parameters, parameters }, &.{ 0.1, 0.1 }, &.{ 0.05, 0.15 }, &.{ 0.1, 0.2 }, 1e-3, 1e-6);
