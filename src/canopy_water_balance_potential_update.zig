...
pub const Result = struct {
    ...
    next_canopy_water_potential_mpa: f64,
...
pub fn calculate(inputs: Inputs) !Result {
    ...
    const next_potential = @min(
        0.5 * inputs.current_canopy_water_potential_mpa,
    ...
    const result = Result{
        ...
        .potential_change_mpa = potential_change,
        ...
        .previous_canopy_water_potential_mpa = inputs.current_canopy_water_potential_mpa,
        .next_canopy_water_potential_mpa = next_potential,
...
test "UPTAKE water balance residual and potential update preserve source order" {
    ...
    try std.testing.expectEqual(potential_change, result.potential_change_mpa);
    try std.testing.expectEqual(next_potential, result.next_canopy_water_potential_mpa);
...
test "source upper potential limiter prevents positive relaxation" {
    ...
    try std.testing.expectEqual(
        0.5 * inputs.current_canopy_water_potential_mpa,
        result.next_canopy_water_potential_mpa,
    );
}
