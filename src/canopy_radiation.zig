...
pub const Partition = struct {
    ...
    horizontal_shortwave_mj_per_m2: f64,
...
pub const State = struct {
    ...
    pub fn init(allocator: std.mem.Allocator, cell_count: usize) !State {
        ...
        result.direct_shortwave_mj_per_m2 = try allocator.alloc(f64, cell_count);
        errdefer allocator.free(result.direct_shortwave_mj_per_m2);
        result.diffuse_shortwave_mj_per_m2 = try allocator.alloc(f64, cell_count);
        errdefer allocator.free(result.diffuse_shortwave_mj_per_m2);
        ...
        result.horizontal_shortwave_mj_per_m2 = try allocator.alloc(f64, cell_count);
        errdefer allocator.free(result.horizontal_shortwave_mj_per_m2);
    ...
    pub fn cellCount(self: State) usize {
        return self.horizontal_shortwave_mj_per_m2.len;
    }
};
...
pub const ApplyContext = struct {
    state: *State,
    horizontal_shortwave_mj_per_m2: f64,
    extraterrestrial_horizontal_shortwave_mj_per_m2: f64,
    solar_angle_sine: f64,
};

pub const MappedApplyContext = struct {
    state: *State,
    horizontal_shortwave_mj_per_m2: []const f64,
    extraterrestrial_horizontal_shortwave_mj_per_m2: []const f64,
...
pub fn applyUniformTile(context: *ApplyContext, range: CellRange) !void {
    ...
    const value = try partition(context.horizontal_shortwave_mj_per_m2, context.extraterrestrial_horizontal_shortwave_mj_per_m2, context.solar_angle_sine);
...
pub fn applyMappedTile(context: *MappedApplyContext, range: CellRange) !void {
    ...
    if (range.end > cells or context.horizontal_shortwave_mj_per_m2.len != cells or
        context.extraterrestrial_horizontal_shortwave_mj_per_m2.len != cells or
    ...
    for (range.first..range.end) |cell| {
        const value = try partition(
            context.horizontal_shortwave_mj_per_m2[cell],
            context.extraterrestrial_horizontal_shortwave_mj_per_m2[cell],
...
const par_micromol_per_m2_per_s_per_mj_per_m2 = 1269.4;
const maximum_direct_beam_mj_per_m2 = 4.167;
...
pub fn partition(
    ...
    if (!std.math.isFinite(horizontal_shortwave_mj_per_m2) or
        !std.math.isFinite(extraterrestrial_horizontal_shortwave_mj_per_m2) or
        !std.math.isFinite(solar_angle_sine)) return error.NonFiniteCanopyRadiation;
    if (horizontal_shortwave_mj_per_m2 < 0 or
        extraterrestrial_horizontal_shortwave_mj_per_m2 < 0 or
    ...
    const diffuse_horizontal_mj_per_m2 = std.math.clamp(
        0.5 * (extraterrestrial_horizontal_shortwave_mj_per_m2 - horizontal_shortwave_mj_per_m2),
        0.0,
        horizontal_shortwave_mj_per_m2,
    );
    const direct_beam_mj_per_m2 = @min(
        maximum_direct_beam_mj_per_m2,
        (horizontal_shortwave_mj_per_m2 - diffuse_horizontal_mj_per_m2) / solar_angle_sine,
    );
    const diffuse_beam_mj_per_m2 = diffuse_horizontal_mj_per_m2 / diffuse_sky_horizontal_projection;
    const direct_par = direct_beam_mj_per_m2 * direct_visible_fraction * par_micromol_per_m2_per_s_per_mj_per_m2;
    const diffuse_par = diffuse_beam_mj_per_m2 * diffuse_visible_fraction * par_micromol_per_m2_per_s_per_mj_per_m2;
    const reconstructed_shortwave = direct_beam_mj_per_m2 * solar_angle_sine + diffuse_beam_mj_per_m2 * diffuse_sky_horizontal_projection;
    ...
    return .{
        .direct_shortwave_mj_per_m2 = direct_beam_mj_per_m2,
        .diffuse_shortwave_mj_per_m2 = diffuse_beam_mj_per_m2,
        .direct_par_micromol_per_m2_per_s = direct_par,
        .diffuse_par_micromol_per_m2_per_s = diffuse_par,
        .horizontal_shortwave_mj_per_m2 = reconstructed_shortwave,
...
fn zeroPartition() Partition {
    return .{
        .direct_shortwave_mj_per_m2 = 0,
        .diffuse_shortwave_mj_per_m2 = 0,
        .direct_par_micromol_per_m2_per_s = 0,
        .diffuse_par_micromol_per_m2_per_s = 0,
        .horizontal_shortwave_mj_per_m2 = 0,
...
test "direct and diffuse beams reconstruct ordinary horizontal shortwave" {
    ...
    try std.testing.expectApproxEqAbs(@as(f64, 2.0), result.horizontal_shortwave_mj_per_m2, 1.0e-12);
    try std.testing.expectApproxEqAbs(@as(f64, 1.25), result.direct_shortwave_mj_per_m2, 1.0e-12);
    try std.testing.expect(result.diffuse_shortwave_mj_per_m2 > 0);
...
test "HOUR1 night branch zeros even a nonzero interpolated shortwave carrier" {
    ...
    try std.testing.expectEqual(@as(f64, 0), nonzero_carrier.horizontal_shortwave_mj_per_m2);
}
...
test "parallel tiles populate runtime-sized canopy radiation state" {
    ...
    var context: ApplyContext = .{
        .state = &state,
        .horizontal_shortwave_mj_per_m2 = 2,
        .extraterrestrial_horizontal_shortwave_mj_per_m2 = 4,
    ...
    try std.testing.expectApproxEqAbs(@as(f64, 2), state.horizontal_shortwave_mj_per_m2[10_002], 1.0e-12);
