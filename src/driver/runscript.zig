...
pub fn parse(allocator: std.mem.Allocator, source: []const u8) !RunScript {
    ...
    // Optional trailing fields. Older runscripts omit them and inherit
    // `absolute_tolerance`, preserving existing behaviour exactly; new decks can
    // set the mass-balance audit threshold and the negligible-quantity cutoff
    // independently of solver convergence.
    const mass_balance_tolerance = try optionalRecordFloat(&runtime_tokens, absolute_tolerance);
    ...
    if (!std.math.isFinite(mass_balance_tolerance) or mass_balance_tolerance <= 0.0 or
        !std.math.isFinite(negligible_quantity_threshold) or negligible_quantity_threshold <= 0.0) return error.InvalidRuntimeControls;
...
fn optionalRecordFloat(fields: *delimited_input.TokenIterator, fallback: f64) !f64 {
    const token = fields.next() orelse return fallback;
