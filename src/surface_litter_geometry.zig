...
pub fn calculate(inputs: Inputs, parameters: Parameters) !Result {
    ...
    inline for (.{ inputs.charcoal_carbon_g_c, inputs.water_m3, inputs.ice_m3, parameters.dry_mass_megagrams_per_g_c, parameters.particle_density_megagrams_per_m3, parameters.field_capacity_fraction_of_porosity, parameters.wilting_point_fraction_of_porosity }) |value| if (!std.math.isFinite(value)) return error.NonFiniteSurfaceLitterGeometryInput;
    ...
    if (inputs.charcoal_carbon_g_c < 0 or inputs.water_m3 < 0 or inputs.ice_m3 < 0 or parameters.dry_mass_megagrams_per_g_c < 0 or parameters.particle_density_megagrams_per_m3 <= 0 or parameters.field_capacity_fraction_of_porosity < 0 or parameters.field_capacity_fraction_of_porosity > 1 or parameters.wilting_point_fraction_of_porosity < 0 or parameters.wilting_point_fraction_of_porosity > parameters.field_capacity_fraction_of_porosity) return error.InvalidSurfaceLitterGeometryInput;
