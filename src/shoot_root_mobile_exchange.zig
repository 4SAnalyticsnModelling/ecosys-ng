...
fn validateInputs(inputs: Inputs) !void {
    ...
    const nonnegative = [_]f64{ inputs.total_root_sink_strength, inputs.total_root_carbon_g_c, inputs.branch_nonwoody_carbon_fraction, inputs.root_nonwoody_carbon_fraction, inputs.minimum_sink_ratio, inputs.base_exchange_rate_h_inv, inputs.leaf_petiole_allocation_fraction, inputs.salt_exchange_rate_h_inv, inputs.biological_timestep_h, inputs.minimum_pool_g_c };
...
fn exchangeSalt(workspace: State, inputs: Inputs, species: usize, branch: usize, layer: usize, branch_carbon_g_c: f64, root_carbon_g_c: f64, combined_carbon_g_c: f64) void {
    ...
    const transfer_mol = inputs.salt_exchange_rate_h_inv * difference_mol * inputs.biological_timestep_h;
...
pub fn apply(allocator: std.mem.Allocator, state: State, workspace: State, inputs: Inputs) !void {
    ...
    for (0..inputs.branch_count) |branch| {
        ...
        const carbon_exchange_rate_h_inv = if (inputs.growth_habit == .annual) @max(5.0e-3, inputs.base_exchange_rate_h_inv * std.math.pow(f64, inputs.leaf_petiole_allocation_fraction, 0.25)) else inputs.base_exchange_rate_h_inv;
        for (inputs.shared_planting_layer_index..inputs.deepest_rooted_layer_index + 1) |layer| {
            ...
            const carbon_transfer_g_c = carbon_exchange_rate_h_inv * carbon_difference_g_c * inputs.biological_timestep_h;
            ...
            if (combined_mobile_carbon_g_c > inputs.minimum_pool_g_c) {
                ...
                nitrogen_transfer_g_n = inputs.base_exchange_rate_h_inv * nitrogen_difference_g_n * inputs.biological_timestep_h;
                ...
                phosphorus_transfer_g_p = inputs.base_exchange_rate_h_inv * phosphorus_difference_g_p * inputs.biological_timestep_h;
            }
