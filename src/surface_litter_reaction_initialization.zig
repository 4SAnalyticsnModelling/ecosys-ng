...
pub const Inputs = struct {
    ...
    cation_exchange_capacity_floor_mol_charge_per_Mg_litter: f64,
};
...
pub fn calculate(inputs: Inputs) !Result {
    ...
    const litter_mass = inputs.litter_dry_mass_Mg;
    const litter_mass_per_water =
        litter_mass / inputs.litter_water_volume_m3;
    const topsoil_mass = inputs.topsoil_dry_mass_Mg;
    ...
    const cation_exchange_capacity = if (litter_mass >
        ...
        @max(
            inputs.cation_exchange_capacity_floor_mol_charge_per_Mg_litter,
            inputs.carboxyl_sites_mol_per_Mg_c *
                1.0e-6 *
    ...
    const result: WetState = .{
        ...
        .litter_dry_mass_Mg = litter_mass,
        .litter_mass_per_water_volume_Mg_per_m3 = litter_mass_per_water,
        .topsoil_dry_mass_Mg = topsoil_mass,
        .cation_exchange_capacity_mol_charge_per_Mg_litter = cation_exchange_capacity,
    };
...
fn validateInputs(inputs: Inputs) !void {
    ...
    if (inputs.cation_exchange_capacity_floor_mol_charge_per_Mg_litter == 0)
        return error.InvalidSurfaceLitterReactionInitializationInput;
}
...
fn testInputs() Inputs {
    return .{
        ...
        .litter_dry_mass_Mg = 0.5,
        .topsoil_dry_mass_Mg = 10,
        .ordinary_organic_carbon_g_c = 200_000,
        .charcoal_carbon_g_c = 50_000,
        .carboxyl_sites_mol_per_Mg_c = 250,
        ...
        .cation_exchange_capacity_floor_mol_charge_per_Mg_litter = 1.0e-32,
    };
}
...
test "SOLUTE wet surface-litter initialization preserves every source expression" {
    ...
    const expected_mass_per_water =
        inputs.litter_dry_mass_Mg / inputs.litter_water_volume_m3;
    ...
    try std.testing.expectEqual(
        inputs.litter_dry_mass_Mg,
        wet.litter_dry_mass_Mg,
    );
    try std.testing.expectEqual(
        expected_mass_per_water,
        wet.litter_mass_per_water_volume_Mg_per_m3,
    );
    try std.testing.expectEqual(
        inputs.topsoil_dry_mass_Mg,
        wet.topsoil_dry_mass_Mg,
    );
    try std.testing.expectEqual(
        expected_capacity,
        wet.cation_exchange_capacity_mol_charge_per_Mg_litter,
    );
}
...
test "surface-litter water and dry-mass gates retain strict source boundaries" {
    ...
    try std.testing.expectEqual(
        @as(f64, 0),
        threshold_wet.cation_exchange_capacity_mol_charge_per_Mg_litter,
    );
    ...
    try std.testing.expectEqual(
        inputs.cation_exchange_capacity_floor_mol_charge_per_Mg_litter,
        zero_carbon_wet.cation_exchange_capacity_mol_charge_per_Mg_litter,
    );
}
...
test "surface-litter initialization rejects invalid inputs and overflow" {
    ...
    inputs = testInputs();
    inputs.cation_exchange_capacity_floor_mol_charge_per_Mg_litter = 0;
    try std.testing.expectError(
        error.InvalidSurfaceLitterReactionInitializationInput,
        calculate(inputs),
    );
