...
fn stage(update: ZoneUpdate) !ZoneState {
    ...
    if (!std.math.isFinite(update.soil_mass_per_water_volume_Mg_per_m3) or
        update.soil_mass_per_water_volume_Mg_per_m3 <= 0)
        return error.InvalidSoilWaterRatio;

    const before_phosphorus = phosphorusMolPerMg(update.state.*, update.soil_mass_per_water_volume_Mg_per_m3);
    ...
    const water_volume_per_soil_mass_m3_per_Mg = 1.0 / update.soil_mass_per_water_volume_Mg_per_m3;
    var next = update.state.*;
    next.dissolved_h2po4_mol_p_per_m3 -=
        (f.h2po4_with_protonated_site_mol_p_per_Mg + f.h2po4_with_hydroxyl_site_mol_p_per_Mg) /
        water_volume_per_soil_mass_m3_per_Mg;
    next.dissolved_hpo4_mol_p_per_m3 -=
        f.hpo4_with_hydroxyl_site_mol_p_per_Mg / water_volume_per_soil_mass_m3_per_Mg;
    next.deprotonated_site_mol_per_Mg -= f.hydroxyl_to_deprotonated_site_mol_per_Mg;
    next.hydroxyl_site_mol_per_Mg += f.hydroxyl_to_deprotonated_site_mol_per_Mg -
        f.protonated_to_hydroxyl_site_mol_per_Mg -
        f.h2po4_with_hydroxyl_site_mol_p_per_Mg -
        f.hpo4_with_hydroxyl_site_mol_p_per_Mg;
    next.protonated_site_mol_per_Mg += f.protonated_to_hydroxyl_site_mol_per_Mg -
        f.h2po4_with_protonated_site_mol_p_per_Mg;
    next.adsorbed_h2po4_mol_p_per_Mg +=
        f.h2po4_with_protonated_site_mol_p_per_Mg + f.h2po4_with_hydroxyl_site_mol_p_per_Mg;
    next.adsorbed_hpo4_mol_p_per_Mg += f.hpo4_with_hydroxyl_site_mol_p_per_Mg;

    try validateState(next);
    try expectConserved(before_phosphorus, phosphorusMolPerMg(next, update.soil_mass_per_water_volume_Mg_per_m3));
    try expectConserved(before_sites, siteMolPerMg(next));
    return next;
}

fn phosphorusMolPerMg(state: ZoneState, soil_mass_per_water_volume_Mg_per_m3: f64) f64 {
    return (state.dissolved_h2po4_mol_p_per_m3 + state.dissolved_hpo4_mol_p_per_m3) /
        soil_mass_per_water_volume_Mg_per_m3 +
        state.adsorbed_h2po4_mol_p_per_Mg + state.adsorbed_hpo4_mol_p_per_Mg;
}

fn siteMolPerMg(state: ZoneState) f64 {
    return state.deprotonated_site_mol_per_Mg + state.hydroxyl_site_mol_per_Mg +
        state.protonated_site_mol_per_Mg + state.adsorbed_h2po4_mol_p_per_Mg +
        state.adsorbed_hpo4_mol_p_per_Mg;
}
...
fn sampleState() ZoneState {
    return .{
        ...
        .deprotonated_site_mol_per_Mg = 0.3,
        .hydroxyl_site_mol_per_Mg = 0.5,
        .protonated_site_mol_per_Mg = 0.4,
        .adsorbed_h2po4_mol_p_per_Mg = 0.1,
...
test "phosphate exchange conserves phosphorus and adsorption sites with explicit units" {
    ...
    try commit(.{ .state = &state, .soil_mass_per_water_volume_Mg_per_m3 = density, .flux = .{
        .protonated_to_hydroxyl_site_mol_per_Mg = 0.02,
        .hydroxyl_to_deprotonated_site_mol_per_Mg = -0.01,
        .h2po4_with_protonated_site_mol_p_per_Mg = 0.03,
        .h2po4_with_hydroxyl_site_mol_p_per_Mg = 0.02,
        .hpo4_with_hydroxyl_site_mol_p_per_Mg = 0.01,
    } });
...
test "paired phosphate exchange rolls both zones back when either update is invalid" {
    ...
    const valid_flux: phosphate_exchange.Flux = .{
        ...
        .h2po4_with_protonated_site_mol_p_per_Mg = 0.01,
        .h2po4_with_hydroxyl_site_mol_p_per_Mg = 0,
        .hpo4_with_hydroxyl_site_mol_p_per_Mg = 0,
    };
    var invalid_flux = valid_flux;
    invalid_flux.h2po4_with_protonated_site_mol_p_per_Mg = 1.0;
    try std.testing.expectError(error.NegativePhosphateState, commitPaired(
        .{ .state = &non_band, .flux = valid_flux, .soil_mass_per_water_volume_Mg_per_m3 = 1 },
        .{ .state = &band, .flux = invalid_flux, .soil_mass_per_water_volume_Mg_per_m3 = 1 },
    ));
