...
const fertilizer = @import("surface_litter_fertilizer.zig");
...
pub fn applyTile(context: *ApplyContext, range: compute.CellRange) !void {
    ...
    for (range.first..range.end) |cell| {
        ...
        const dry_mass_Mg = try context.surface_organic.totalCarbon_g_c(cell) * context.litter_dry_mass_Mg_per_g_c;
        ...
        context.diagnostics.fluxes[cell] = try fertilizer.calculateAndApplyToChemistry(context.state, context.chemistry, cell, .{
            ...
            .litter_dry_mass_Mg = dry_mass_Mg,
...
fn validateDimensions(context: ApplyContext, range: compute.CellRange) !void {
    ...
    if (context.state.formulation.len != count or !std.math.isFinite(context.litter_dry_mass_Mg_per_g_c) or context.litter_dry_mass_Mg_per_g_c <= 0 or !std.math.isFinite(context.step_duration_h) or context.step_duration_h <= 0) return error.InvalidSurfaceFertilizerStepInput;
    for (range.first..range.end) |cell| {
