...
pub fn calculate(inputs: Inputs) !Result {
    ...
    inline for (.{
        ...
        inputs.minimum_canopy_water_potential_megapascal,
    ...
    const trigger_value = switch (inputs.trigger) {
        ...
        .canopy_water_potential_megapascal => |value| value,
    };
    ...
    const triggered = switch (inputs.trigger) {
        ...
        .canopy_water_potential_megapascal => |threshold_megapascal| inputs.minimum_canopy_water_potential_megapascal < threshold_megapascal,
    };
...
test "partial evaluated layer uses authoritative volume and liquid plus ice" {
    const result = try calculate(.{
        ...
        .minimum_canopy_water_potential_megapascal = -0.5,
...
test "canopy criterion uses source PFT-one potential supplied by caller" {
    var inputs: Inputs = .{
        ...
        .refill_fraction_of_field_capacity = 1,
        .minimum_canopy_water_potential_megapascal = -1.5,
        .trigger = .{ .canopy_water_potential_megapascal = -1 },
        .first_source_hour = 1,
        .last_source_hour = 1,
    ...
    inputs.minimum_canopy_water_potential_megapascal = -0.5;
...
test "strict soil threshold equality does not trigger" {
    const result = try calculate(.{
        ...
        .evaluated_depth_m = 0.1,
        .refill_fraction_of_field_capacity = 1,
        .minimum_canopy_water_potential_megapascal = 0,
