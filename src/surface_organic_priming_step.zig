    ...
    for (range.first..range.end) |cell| {
        ...
        for (0..substrate_count) |complex| {
            ...
            for (0..respiration.source_population_count) |population| {
                ...
                microbial_response[complex * respiration.source_population_count + population] = context.environment.growth_temperature_response[cell] * @exp((if (population == 2) @as(f64, 0.05) else 0.10) * context.matric_plus_osmotic_potential_mpa[cell]);
            }
...
fn validate(context: ApplyContext, range: compute.CellRange) !void {
    ...
    if (range.first > range.end or range.end > cells or context.surface_organic.layer_count != cells or context.respiration.cell_count != cells or context.substrate_uptake.cell_count != cells or context.environment.growth_temperature_response.len != cells or context.environment.aqueous_diffusion_temperature_response.len != cells or context.matric_plus_osmotic_potential_mpa.len != cells) return error.SurfaceOrganicPrimingDimensionMismatch;
