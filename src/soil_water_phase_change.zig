...
pub fn endpointTemperatureFromPhaseEnthalpy(
    ...
    const freezing_heat_megajoules = parameters.fusion_latent_heat_megajoules_per_m3 * parameters.ice_density_megagrams_per_m3 * ((endpoint.matrix_ice_volume_m3 + endpoint.macropore_ice_volume_m3) - initial_ice_m3);
...
pub fn matrixFreezeThaw(temperature_k: f64, matric_plus_osmotic_potential_mpa: f64, liquid_water_m3: f64, ice_volume_m3: f64, heat_capacity_megajoules_per_k: f64, time_fraction: f64, parameters: FreezeThawParameters) !FreezeThaw {
    return freezeThaw(temperature_k, matric_plus_osmotic_potential_mpa, liquid_water_m3, ice_volume_m3, heat_capacity_megajoules_per_k, time_fraction, false, parameters);
}

pub fn macroporeFreezeThaw(temperature_k: f64, matric_plus_osmotic_potential_mpa: f64, liquid_water_m3: f64, ice_volume_m3: f64, liquid_heat_capacity_megajoules_per_m3_k: f64, ice_heat_capacity_megajoules_per_m3_k: f64, time_fraction: f64, parameters: FreezeThawParameters) !FreezeThaw {
...
fn freezeThaw(temperature_k: f64, potential_mpa: f64, liquid_m3: f64, ice_m3: f64, capacity_megajoules_per_k: f64, time_fraction: f64, macropore: bool, parameters: FreezeThawParameters) !FreezeThaw {
    inline for (.{ temperature_k, potential_mpa, liquid_m3, ice_m3, capacity_megajoules_per_k, time_fraction }) |value| if (!std.math.isFinite(value)) return error.NonFiniteSoilPhaseInput;
    ...
    if (temperature_k <= 0 or liquid_m3 < 0 or ice_m3 < 0 or capacity_megajoules_per_k < 0 or time_fraction <= 0 or time_fraction > 1 or parameters.freezing_potential_numerator_k_mpa <= 0 or parameters.latent_heat_of_fusion_megajoules_per_m3 <= 0 or parameters.ice_density_megagrams_per_m3 <= 0 or parameters.heat_capacity_temperature_feedback_per_k < 0 or parameters.pure_water_freezing_temperature_k <= 0 or potential_mpa == parameters.latent_heat_of_fusion_megajoules_per_m3) return error.InvalidSoilPhaseInput;
    const freezing_temperature = -parameters.freezing_potential_numerator_k_mpa / (potential_mpa - parameters.latent_heat_of_fusion_megajoules_per_m3);
...
fn freezeParameters() FreezeThawParameters {
    return .{ .freezing_potential_numerator_k_mpa = 9.0959e4, .latent_heat_of_fusion_megajoules_per_m3 = 333, .ice_density_megagrams_per_m3 = 0.917, .heat_capacity_temperature_feedback_per_k = 6.2913e-3, .pure_water_freezing_temperature_k = 273.15 };
}
...
test "endpoint phase enthalpy reproduces WATSUB heat-capacity and latent-energy update" {
    const parameters: PhaseEnthalpyParameters = .{ .liquid_heat_capacity_megajoules_per_m3_k = 4.19, .ice_heat_capacity_megajoules_per_m3_k = 1.9274, .vaporization_latent_heat_megajoules_per_m3 = 2450.0, .fusion_latent_heat_megajoules_per_m3 = 333.0, .ice_density_megagrams_per_m3 = 0.917 };
