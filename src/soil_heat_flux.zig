...
pub const CellConductivityInputs = struct {
    ...
    solid_conductivity_numerator_m_mj_per_h_k: f64,
...
pub fn calculateCellConductivity(inputs: CellConductivityInputs, parameters: TurbulenceParameters) !f64 {
    ...
    const numerator = inputs.solid_conductivity_numerator_m_mj_per_h_k + inputs.liquid_water_fraction * water_conductivity + 0.611 * inputs.ice_fraction * 7.844e-3 + air_weight * inputs.air_fraction * air_conductivity;
...
pub const FaceInputs = struct {
    ...
    source_heat_capacity_mj_per_k: f64,
    destination_heat_capacity_mj_per_k: f64,
    source_minimum_heat_capacity_mj_per_k: f64,
    destination_minimum_heat_capacity_mj_per_k: f64,
    ...
    source_thermal_conductivity_m_mj_per_h_k: f64,
    destination_thermal_conductivity_m_mj_per_h_k: f64,
...
pub const FaceFlux = struct {
    conductive_unlimited_mj: f64,
    conductive_limited_mj: f64,
    convective_mj: f64,
    total_mj: f64,
...
pub fn calculateFaceFlux(inputs: FaceInputs) !FaceFlux {
    ...
    if (inputs.source_temperature_k <= 0 or inputs.destination_temperature_k <= 0 or inputs.source_heat_capacity_mj_per_k <= 0 or inputs.destination_heat_capacity_mj_per_k <= 0 or inputs.source_minimum_heat_capacity_mj_per_k < 0 or inputs.destination_minimum_heat_capacity_mj_per_k < 0 or inputs.top_snow_heat_capacity_mj_per_k < 0 or inputs.maximum_negligible_snow_heat_capacity_mj_per_k < 0 or inputs.liquid_water_heat_capacity_mj_per_m3_k <= 0 or inputs.source_thermal_conductivity_m_mj_per_h_k < 0 or inputs.destination_thermal_conductivity_m_mj_per_h_k < 0 or inputs.source_path_length_m <= 0 or inputs.destination_path_length_m <= 0 or inputs.face_area_m2 < 0 or inputs.time_fraction <= 0 or inputs.time_fraction > 1) return error.InvalidSoilHeatInput;
    ...
    const vapor_convective = inputs.liquid_water_heat_capacity_mj_per_m3_k * vapor_donor_temperature * inputs.vapor_flux_m3;
    const liquid_convective = inputs.liquid_water_heat_capacity_mj_per_m3_k * liquid_donor_temperature * inputs.liquid_water_flux_m3;
    const macro_convective = inputs.liquid_water_heat_capacity_mj_per_m3_k * macro_donor_temperature * inputs.macropore_water_flux_m3;
    var source_interim = inputs.source_temperature_k;
    if (inputs.source_heat_capacity_mj_per_k > inputs.source_minimum_heat_capacity_mj_per_k) {
        source_interim -= if (inputs.source_is_top_soil_layer and inputs.top_snow_heat_capacity_mj_per_k <= inputs.maximum_negligible_snow_heat_capacity_mj_per_k)
            (vapor_convective - inputs.snow_storage_heat_flux_mj) / inputs.source_heat_capacity_mj_per_k
        else
            vapor_convective / inputs.source_heat_capacity_mj_per_k;
    }
    var destination_interim = inputs.destination_temperature_k;
    if (inputs.destination_heat_capacity_mj_per_k > inputs.destination_minimum_heat_capacity_mj_per_k) destination_interim += vapor_convective / inputs.destination_heat_capacity_mj_per_k;
    const equilibrium = (inputs.source_heat_capacity_mj_per_k * source_interim + inputs.destination_heat_capacity_mj_per_k * destination_interim) / (inputs.source_heat_capacity_mj_per_k + inputs.destination_heat_capacity_mj_per_k);
    ...
    const equilibration_limit =
        ...
        inputs.source_heat_capacity_mj_per_k;
    const denominator = inputs.source_thermal_conductivity_m_mj_per_h_k * inputs.destination_path_length_m + inputs.destination_thermal_conductivity_m_mj_per_h_k * inputs.source_path_length_m;
    const conductance = if (denominator > 0) 2.0 * inputs.source_thermal_conductivity_m_mj_per_h_k * inputs.destination_thermal_conductivity_m_mj_per_h_k / denominator else 0.0;
    ...
    return .{ .conductive_unlimited_mj = unlimited, .conductive_limited_mj = limited, .convective_mj = convective, .total_mj = total, .equilibrium_temperature_k = equilibrium };
}
...
test "WATSUB conductive face is limited to pair equilibration" {
    const flux = try calculateFaceFlux(.{ .source_temperature_k = 300, .destination_temperature_k = 280, .source_heat_capacity_mj_per_k = 2, .destination_heat_capacity_mj_per_k = 2, .source_minimum_heat_capacity_mj_per_k = 0, .destination_minimum_heat_capacity_mj_per_k = 0, .source_is_top_soil_layer = false, .top_snow_heat_capacity_mj_per_k = 0, .maximum_negligible_snow_heat_capacity_mj_per_k = 0, .snow_storage_heat_flux_mj = 0, .liquid_water_flux_m3 = 0, .vapor_flux_m3 = 0, .macropore_water_flux_m3 = 0, .liquid_water_heat_capacity_mj_per_m3_k = 4.19, .source_thermal_conductivity_m_mj_per_h_k = 100, .destination_thermal_conductivity_m_mj_per_h_k = 100, .source_path_length_m = 1, .destination_path_length_m = 1, .face_area_m2 = 1, .time_fraction = 1 });
    try std.testing.expectApproxEqAbs(@as(f64, 290), flux.equilibrium_temperature_k, 1e-12);
    try std.testing.expectApproxEqAbs(@as(f64, 20), flux.conductive_limited_mj, 1e-12);
}

test "convective heat uses the upstream temperature for each phase" {
    const flux = try calculateFaceFlux(.{ .source_temperature_k = 300, .destination_temperature_k = 280, .source_heat_capacity_mj_per_k = 2, .destination_heat_capacity_mj_per_k = 2, .source_minimum_heat_capacity_mj_per_k = 0, .destination_minimum_heat_capacity_mj_per_k = 0, .source_is_top_soil_layer = false, .top_snow_heat_capacity_mj_per_k = 0, .maximum_negligible_snow_heat_capacity_mj_per_k = 0, .snow_storage_heat_flux_mj = 0, .liquid_water_flux_m3 = 0.01, .vapor_flux_m3 = -0.02, .macropore_water_flux_m3 = 0.03, .liquid_water_heat_capacity_mj_per_m3_k = 4.2, .source_thermal_conductivity_m_mj_per_h_k = 0, .destination_thermal_conductivity_m_mj_per_h_k = 0, .source_path_length_m = 1, .destination_path_length_m = 1, .face_area_m2 = 1, .time_fraction = 1 });
    const expected = 4.2 * (300 * 0.01 + 280 * -0.02 + 300 * 0.03);
    try std.testing.expectApproxEqAbs(expected, flux.convective_mj, 1e-12);
}
