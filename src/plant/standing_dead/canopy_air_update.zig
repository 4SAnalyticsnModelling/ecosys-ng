...
pub const Inputs = struct {
    ...
    minimum_air_solver_heat_capacity_megajoules_per_k: f64,
    ...
    surface_sensible_heat_megajoules_per_step: f64,
    ...
    lateral_sensible_heat_rate_megajoules_per_h: f64,
...
pub fn compute(inputs: Inputs) !Result {
    ...
    if (inputs.dry_heat_capacity_megajoules_per_k >
        inputs.minimum_air_solver_heat_capacity_megajoules_per_k and
        inputs.combined_area_radiation_fraction >
            inputs.minimum_combined_area_radiation_fraction)
    {
        try validateAdmitted(inputs);
        result.atmosphere_sensible_heat_megajoules_per_step =
        ...
        result.net_sensible_heat_megajoules_per_step =
            result.atmosphere_sensible_heat_megajoules_per_step -
            inputs.surface_sensible_heat_megajoules_per_step;
        ...
        result.ground_vapor_flux_m3_per_step =
            ...
            inputs.combined_area_radiation_fraction;
        result.lateral_sensible_heat_megajoules_per_step =
            inputs.lateral_sensible_heat_rate_megajoules_per_h *
...
fn validateAlwaysUsed(inputs: Inputs) !void {
    inline for (.{
        ...
        inputs.minimum_air_solver_heat_capacity_megajoules_per_k,
        inputs.combined_area_radiation_fraction,
        inputs.minimum_combined_area_radiation_fraction,
        inputs.surface_evaporation_m3_per_step,
    ...
    if (inputs.dry_heat_capacity_megajoules_per_k < 0 or
        inputs.minimum_air_solver_heat_capacity_megajoules_per_k < 0 or
...
fn validateAdmitted(inputs: Inputs) !void {
    ...
    if (inputs.total_aerodynamic_resistance_h_per_m <= 0 or
        inputs.sensible_boundary_conductance_megajoules_per_m_k_step < 0 or
        inputs.latent_boundary_conductance_m2_per_step < 0 or
        inputs.canopy_air_vapor_capacity_m3 <= 0 or
        inputs.ground_sensible_conductance_megajoules_per_k_step < 0 or
        ...
        inputs.timestep_h < 0 or inputs.canopy_air_heat_capacity_megajoules_per_k <= 0)
        return error.InvalidStandingDeadCanopyAirInput;
}
...
fn testInputs() Inputs {
    return .{
        ...
        .minimum_air_solver_heat_capacity_megajoules_per_k = 0.1,
        ...
        .surface_sensible_heat_megajoules_per_step = 0.5,
        .surface_transpiration_m3_per_step = 0,
        .surface_evaporation_m3_per_step = -0.001,
        .ground_sensible_conductance_megajoules_per_k_step = 0.1,
        ...
        .lateral_sensible_heat_rate_megajoules_per_h = 0.4,
...
test "admitted standing-dead canopy air update preserves source order" {
    ...
    try std.testing.expectEqual(@as(f64, 2), result.atmosphere_sensible_heat_megajoules_per_step);
    try std.testing.expectEqual(@as(f64, 1.5), result.net_sensible_heat_megajoules_per_step);
    try std.testing.expectEqual(@as(f64, 0.1), result.ground_sensible_heat_megajoules_per_step);
    try std.testing.expectEqual(@as(f64, 0.05), result.lateral_sensible_heat_megajoules_per_step);
...
test "strict threshold fallback resets fluxes and selects ambient state" {
    ...
    try std.testing.expectEqual(@as(f64, 0), result.net_sensible_heat_megajoules_per_step);
}
...
test "later invalid runtime species leaves destination unchanged" {
    ...
    invalid.canopy_air_heat_capacity_megajoules_per_k = 0;
