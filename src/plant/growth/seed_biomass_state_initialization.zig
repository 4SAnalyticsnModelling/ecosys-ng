...
pub const Inputs = struct {
    ...
    total_canopy_water_potential_megapascal: f64,
...
pub const Parameters = struct {
    ...
    water_potential_dry_matter_numerator_per_megapascal: f64,
    water_potential_dry_matter_denominator_per_megapascal: f64,
...
/// Translates `startq.f` lines 859--885 for fresh seed morphology and biomass.
pub fn initialize(
    inputs: Inputs,
    ...
    const absolute_water_potential_megapascal = @abs(inputs.total_canopy_water_potential_megapascal);
    const dry_matter_fraction = parameters.fully_hydrated_dry_matter_fraction +
        parameters.water_potential_dry_matter_numerator_per_megapascal *
            absolute_water_potential_megapascal /
            (parameters.water_potential_dry_matter_denominator_per_megapascal *
                absolute_water_potential_megapascal +
...
fn validate(inputs: Inputs, parameters: Parameters) InitializationError!void {
    inline for (std.meta.fields(Inputs)) |field| {
        ...
        if (!std.mem.eql(u8, field.name, "total_canopy_water_potential_megapascal") and
            value < 0.0)
        {
...
test "seed morphology preserves STARTQ C N P and water equations" {
    const inputs = Inputs{
        ...
        .total_canopy_water_potential_megapascal = -0.001,
    ...
    const parameters = Parameters{
        ...
        .water_potential_dry_matter_numerator_per_megapascal = 0.10,
        .water_potential_dry_matter_denominator_per_megapascal = 0.05,
