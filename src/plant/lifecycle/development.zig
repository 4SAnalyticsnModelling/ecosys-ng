...
test "HFUNC development kernel advances arbitrary runtime species without shared state" {
    ...
    @memset(dormancy_parameters, .{ .required_leafout_h = 2, .required_leafoff_h = 2, .leafout_temperature_threshold_c = 5, .leafoff_temperature_threshold_c = 0, .chilling_temperature_c = -5, .drought_leafout_total_water_potential_megapascal = -0.1, .combined_leafout_turgor_potential_megapascal = 0.1, .leafoff_total_water_potential_megapascal = -1.5, .maximum_photoperiod_counter_h = 3600, .evergreen_leafoff_remobilization_start_fraction = 0.75, .deciduous_leafoff_remobilization_start_fraction = 0.5, .full_senescence_duration_h = 480 });
