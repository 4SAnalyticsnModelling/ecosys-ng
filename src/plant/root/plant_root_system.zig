...
pub const State = struct {
    ...
    pub fn refreshLayerMorphology(self: *State, plant: usize, domain: usize, layer: usize, plant_population_count: f64, layer_thickness_m: f64, woody_root_fraction: f64, root_porosity_fraction: f64, volume_m3_per_g_c: f64, root_geometry_pi: f64, vascular_growth_habit: bool, parameters: MorphologyParameters) !LayerTopology {
        ...
        if (has_live_roots) {
            self.primary_radius_m[index] = @max(reference_primary_radius_m, (1.0 + turgor / parameters.root_elastic_modulus_megapascal) * reference_primary_radius_m);
            self.secondary_radius_m[index] = @max(reference_secondary_radius_m, (1.0 + turgor / parameters.root_elastic_modulus_megapascal) * reference_secondary_radius_m);
