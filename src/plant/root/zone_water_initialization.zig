...
pub const Inputs = struct {
    landscape_total_water_potential_megapascal: []const f64,
...
pub fn initialize(state: *State, inputs: Inputs) !void {
    ...
    for (0..state.layer_count) |layer| {
        staged.root_referenced_total_water_potential_megapascal[layer] =
            inputs.landscape_total_water_potential_megapascal[layer] -
...
fn validate(state: *const State, inputs: Inputs) !void {
    ...
    inline for (.{
        inputs.landscape_total_water_potential_megapascal,
...
test "UPTAKE initializes mineral and non-mineral layers in source order" {
    ...
    try initialize(&state, .{
        .landscape_total_water_potential_megapascal = &.{ 0.2, -0.4 },
    ...
    try std.testing.expectApproxEqAbs(@as(f64, 0.102), state.root_referenced_total_water_potential_megapascal[0], 1e-15);
    try std.testing.expectApproxEqAbs(@as(f64, -0.498), state.root_referenced_total_water_potential_megapascal[1], 1e-15);
...
test "root-zone initialization preserves negative source available water" {
    ...
    defer state.deinit();
    try initialize(&state, .{
        .landscape_total_water_potential_megapascal = &.{0},
