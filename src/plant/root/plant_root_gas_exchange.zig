...
pub fn soilWaterFilmThicknessM(parameters: RuntimeParameters, matric_potential_megapascal: f64) !f64 {
    try validateRuntimeParameters(parameters);
    ...
    const film = parameters.soil_water_film_scale_m * @exp(parameters.soil_water_film_log_intercept + parameters.soil_water_film_potential_exponent * @log(suction_megapascal));
...
test "WATSUB water film and UPTAKE radial conductance retain source equations" {
    ...
    const film = try soilWaterFilmThicknessM(parameters, potential_megapascal);
