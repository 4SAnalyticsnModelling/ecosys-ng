...
const SoilMaterial = @import("soil_initialization.zig").SoilMaterial;
...
pub const SoilHydrology = struct {
    ...
    pub fn init(
        ...
        for (0..result.layer_count) |layer| {
            ...
            }, profile.field_capacity_potential_mpa, profile.wilting_point_potential_mpa);
...
test "initialize water and ice from reference sentinel rules" {
    ...
    var material = try SoilMaterial.init(std.testing.allocator, profile, @import("soil_profile_derivation.zig").compatibilityParameters());
...
test "negative field capacity and wilting sentinels use HOUR1 runtime estimators" {
    ...
    var material = try SoilMaterial.init(allocator, profile, @import("soil_profile_derivation.zig").compatibilityParameters());
    defer material.deinit();
