...
pub const Parameters = struct {
    ...
    liquid_evaporation_latent_heat_mj_per_m3: f64,
    snow_sublimation_latent_heat_mj_per_m3: f64,
};
...
fn candidateAt(transfer_to_vapor_m3: f64, solid_m3: f64, liquid_m3: f64, vapor_m3: f64, temperature_k: f64, heat_capacity_mj_per_k: f64, ice_m3: f64, air_m3: f64, parameters: Parameters) !LayerCandidate {
    ...
    const latent_heat_mj = parameters.liquid_evaporation_latent_heat_mj_per_m3 * liquid_change_m3 + parameters.snow_sublimation_latent_heat_mj_per_m3 * solid_change_m3;
    ...
    const next_temperature = (heat_capacity_mj_per_k * temperature_k + latent_heat_mj) / next_capacity;
    ...
    return .{ .solid_m3 = @max(0, next_solid), .liquid_m3 = @max(0, next_liquid), .vapor_m3 = @max(0, next_vapor), .temperature_k = next_temperature, .heat_capacity_mj_per_k = next_capacity, .residual_m3 = residual };
}
...
pub fn solve(allocator: std.mem.Allocator, state: *snow.State, parameters: Parameters, options: Options) !Report {
    ...
    var report: Report = .{ .iterations = 0, .converged = false, .maximum_vapor_residual_m3 = 0, .sensible_energy_change_mj = 0 };
    for (0..state.cell_count) |cell| {
        ...
        for (1..state.layer_capacity) |layer| {
            ...
            heat_capacity[index] = current.heat_capacity_mj_per_k;
        }
    }
...
test "internal vapor equilibrium conserves total water and responds below surface" {
    ...
    try std.testing.expect(report.sensible_energy_change_mj < 0);
}
