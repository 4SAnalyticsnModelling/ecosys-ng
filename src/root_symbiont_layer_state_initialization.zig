...
/// Translates STARTQ lines 741-790 with runtime population and soil-layer
...
pub fn initialize(
    ...
    try validate(
        populations,
        leaf_osmotic_potential_at_zero_total_mpa,
    ...
    for (populations, 0..) |population, population_index| {
        for (0..layer_count) |layer_index| {
            const total_potential_mpa = parameters.initial_total_water_potential_mpa;
            const osmotic_potential_mpa =
                leaf_osmotic_potential_at_zero_total_mpa + total_potential_mpa;
            const turgor_potential_mpa =
                @max(0.0, total_potential_mpa - osmotic_potential_mpa);
            const state = PopulationLayerState{
                .water = .{
                    ...
                    .turgor_potential_mpa = turgor_potential_mpa,
                },
...
fn validate(
    ...
    inline for (.{
        leaf_osmotic_potential_at_zero_total_mpa,
        maximum_root_protein_concentration_g_g_c,
        parameters.initial_total_water_potential_mpa,
    ...
    if (maximum_root_protein_concentration_g_g_c < 0.0 or
        ...
        parameters.initial_total_water_potential_mpa > 0.0)
    {
...
test "runtime populations and layers preserve STARTQ initial states" {
    ...
    const totals = try initialize(
        ...
        -0.4,
        0.2,
        .{ .initial_total_water_potential_mpa = -0.01, .initial_axial_length_m = 1.0e-3 },
        &states,
    );
    ...
    try std.testing.expectApproxEqAbs(
        @as(f64, -0.41),
        states[0].water.osmotic_potential_mpa,
        1.0e-15,
    );
    try std.testing.expectApproxEqAbs(
        @as(f64, 0.4),
        states[0].water.turgor_potential_mpa,
        1.0e-15,
    );
