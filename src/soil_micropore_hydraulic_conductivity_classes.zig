...
pub const Inputs = struct {
    ...
    vertical_saturated_conductivity_m2_h_mpa: f64,
    lateral_saturated_conductivity_m2_h_mpa: f64,
...
/// HOUR1 lines 2252--2304. Runtime classes retain K, K->M, then direction
...
pub fn compute(inputs: Inputs, outputs: Outputs) !?AirEntry {
    ...
    for (0..count) |class| {
        ...
        outputs.water_potential_mpa[class] = psi;
    ...
    for (0..count) |class| {
        ...
        for (class..count) |summed| {
            ...
            const psi = outputs.water_potential_mpa[summed];
            numerator += (2.0 * x_m + 1.0 - 2.0 * x_k) / (psi * psi);
        }
        outputs.lateral_x_conductivity_m2_h_mpa[class] =
            inputs.lateral_saturated_conductivity_m2_h_mpa *
            shape * numerator / denominator;
        outputs.lateral_y_conductivity_m2_h_mpa[class] =
            inputs.lateral_saturated_conductivity_m2_h_mpa *
            shape * numerator / denominator;
        outputs.vertical_conductivity_m2_h_mpa[class] =
            inputs.vertical_saturated_conductivity_m2_h_mpa *
            shape * numerator / denominator;
        if (class > 0 and
            outputs.vertical_conductivity_m2_h_mpa[class] <
                inputs.air_entry_conductivity_fraction *
                    inputs.vertical_saturated_conductivity_m2_h_mpa and
...
fn potential(inputs: Inputs, theta: f64) f64 {
    if (theta < inputs.wilting_point_m3_m3)
        return @max(inputs.minimum_potential_mpa, -@exp(
    ...
    return inputs.saturation_potential_mpa;
}
...
fn validate(inputs: Inputs, outputs: Outputs) !usize {
    ...
    if (count == 0 or outputs.water_potential_mpa.len != count or
        ...
        outputs.lateral_y_conductivity_m2_h_mpa.len != count or
        outputs.vertical_conductivity_m2_h_mpa.len != count)
    ...
    if (inputs.porosity_m3_m3 <= 0 or inputs.field_capacity_m3_m3 <= 0 or
        ...
        inputs.minimum_potential_mpa >= 0 or inputs.saturation_potential_mpa >= 0)
...
test "runtime soil classes produce two lateral and one vertical conductivity" {
    ...
    _ = try compute(.{
        ...
        .vertical_saturated_conductivity_m2_h_mpa = 0.02,
        .lateral_saturated_conductivity_m2_h_mpa = 0.01,
        .air_entry_conductivity_fraction = 0.1,
    }, .{
        .water_content_m3_m3 = &theta,
        .water_potential_mpa = &psi,
