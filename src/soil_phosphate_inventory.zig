...
const SoilChemistry = @import("solute_chemistry_state.zig");
const SurfaceChemistry = @import("surface_litter_chemistry.zig");
...
pub fn exchangeablePhosphorus_g_p(
    ...
    if (cell >= grid.cell_count or soil.cell_count != grid.layer_count or surface.cells.len != grid.cell_count or matrix_bulk_volume_m3.len != grid.layer_count or bulk_density_Mg_per_m3.len != grid.layer_count or litter_dry_mass_Mg.len != grid.cell_count) return error.PhosphateInventoryDimensionMismatch;
    inline for (.{ non_band_fraction, band_fraction, phosphorus_molar_mass_g_per_mol }) |value| if (!std.math.isFinite(value)) return error.InvalidPhosphateInventory;
    if (non_band_fraction < 0 or band_fraction < 0 or @abs(non_band_fraction + band_fraction - 1) > 1e-12 or phosphorus_molar_mass_g_per_mol <= 0) return error.InvalidPhosphateInventory;
    const litter_mass = litter_dry_mass_Mg[cell];
    ...
    var phosphorus_mol = litter_mass * (litter_sites.adsorbed_hpo4_mol_p_per_Mg + litter_sites.adsorbed_h2po4_mol_p_per_Mg);
    for (0..grid.active_soil_layer_count[cell]) |local_layer| {
        const layer = try grid.layerIndex(cell, local_layer);
        const volume = matrix_bulk_volume_m3[layer];
        const density = bulk_density_Mg_per_m3[layer];
        ...
        phosphorus_mol += volume * density * (non_band_fraction * (non_band.adsorbed_hpo4_mol_p_per_Mg + non_band.adsorbed_h2po4_mol_p_per_Mg) + band_fraction * (band.adsorbed_hpo4_mol_p_per_Mg + band.adsorbed_h2po4_mol_p_per_Mg));
    }
    const result = phosphorus_mol * phosphorus_molar_mass_g_per_mol;
    if (!std.math.isFinite(result) or result < 0) return error.InvalidPhosphateInventory;
    return result;
...
test "UPX4 uses distinct litter and soil masses with runtime phosphate zones" {
    ...
    var surface = try SurfaceChemistry.State.init(std.testing.allocator, grid.cell_count);
    defer surface.deinit();
    surface.cells[0].phosphate_surface.adsorbed_hpo4_mol_p_per_Mg = 2;
    soil.non_band_phosphate[0].adsorbed_h2po4_mol_p_per_Mg = 4;
    soil.band_phosphate[1].adsorbed_hpo4_mol_p_per_Mg = 8;
