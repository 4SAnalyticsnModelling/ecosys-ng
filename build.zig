...
pub fn build(b: *std.Build) void {
    ...
    // aggregator module rather than `src/module_index.zig`, which lane A1 owns, so that
    ...
    const legacy_comparison_module = b.addModule("legacy_comparison", .{
        .root_source_file = b.path("src/legacy_comparison_index.zig"),
        .target = target,
        .optimize = optimize,
    });
    ...
    const compare_legacy_tests = b.addTest(.{
        .root_module = compare_legacy_executable.root_module,
    });
    test_step.dependOn(&b.addRunArtifact(compare_legacy_tests).step);

    // Built from the whole-tree module, not from the validation file as its own
    // module root: that file reaches siblings with `../` imports, which land
    // outside the module path if the module starts at src/validation/.
